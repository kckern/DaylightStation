...
describe('isInsignificantEffort', () => {
  ...
  it('is true for a near-idle strap regardless of duration', () => {
    expect(isInsignificantEffort({ rings: 1, activeWarmZoneSeconds: 0, hrSampleCount: 2 }, cfg)).toBe(true);
  });
  it('is false when any effort signal exceeds its bound', () => {
    expect(isInsignificantEffort({ rings: 5, activeWarmZoneSeconds: 0, hrSampleCount: 2 }, cfg)).toBe(false);
    expect(isInsignificantEffort({ rings: 0, activeWarmZoneSeconds: 30, hrSampleCount: 2 }, cfg)).toBe(false);
    expect(isInsignificantEffort({ rings: 0, activeWarmZoneSeconds: 0, hrSampleCount: 50 }, cfg)).toBe(false);
  });
});
