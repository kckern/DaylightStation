...
Cycle challenges are a single-rider, equipment-specific challenge type where one participant must reach and maintain target RPMs on a specific bike across multiple phases. Unlike zone or vibration challenges, cycle challenges are tied to a particular piece of equipment (via `equipment.id`) and require the rider to remain committed to that bike for the duration of the challenge.

### How Cycle Challenges Differ
- **Single rider**: One assigned participant drives the challenge; group requirements do not apply.
- **Equipment-bound**: Each selection targets one `equipment.id`; eligible riders are declared on the equipment entry (`eligible_users`).
- **Multi-phase**: Each challenge consists of 3-5 RPM segments that the rider must complete in sequence.
- **Dedicated sub-state machine**: Instead of a single success/fail outcome, a cycle challenge progresses through `init → ramp → maintain → (next ramp | success)`, with `locked` as a recovery state.

### Sub-State Machine

- **`init`** - Rider has been assigned but is not yet engaged. They must get on the bike and reach `init.min_rpm` (while also satisfying the policy's base requirement for themselves) within `init.time_allowed_seconds`. Failing to start in time fails the challenge.
- **`ramp`** - Start of a phase. The rider has `ramp_seconds` to climb from their current RPM up to the phase's `hi_rpm`.
- **`maintain`** - Rider must hold RPM at or above `hi_rpm`. Phase progress fills toward `maintain_seconds`. A dip into `[lo_rpm, hi_rpm)` dims the video (see below) and pauses progress accrual but does not lock. Dropping below `lo_rpm` transitions to `locked`.
- **`locked`** - Video is paused. The rider must reach the relevant threshold to resume: `init.min_rpm` for an init-lock, or `hi_rpm` for a ramp/maintain-lock.

When the final phase's `maintain_seconds` completes, the challenge succeeds.

### Progressive Dimming

While the rider's RPM is between `lo_rpm` and `hi_rpm` during `maintain`, a `dimFactor` value ramps from 0 (at `hi_rpm`) to 1 (at `lo_rpm`). The frontend applies this via the CSS custom property `--cycle-dim`, layering blur, grayscale, sepia, and brightness adjustments on the video. This gives the rider immediate visual feedback that they are slipping before a hard lock occurs.

### Boost Mechanic

Other participants (and the rider themselves) contribute to the challenge's pace:
- Any user in the `hot` or `fire` zone contributes a configured multiplier (`boost.zone_multipliers`) to phase progress accrual.
- Multipliers from all contributing users sum together, capped at `boost.max_total_multiplier` (default `3.0`).
- Boosters appear as avatars in the challenge overlay.

The rider can also boost themselves by pushing resistance high enough to enter `hot` or `fire` zones.

### Rider Swap

The assigned rider is not immediately locked in. During the `init` phase or phase-1 `ramp`, tapping the rider avatar opens a swap modal listing eligible alternates (users in `equipment.eligible_users`, not the current rider, and not on cooldown). Once phase-1's `hi_rpm` has been touched for the first time, the rider is committed and the challenge can no longer be swapped.

### Cooldown

When a cycle challenge ends - whether success, failure, or abandonment - every participant who ever held the `rider` role during it is placed on a `user_cooldown_seconds` cooldown (default `600s`). That user cannot be assigned another cycle challenge until the cooldown expires. This prevents the same person being drafted for back-to-back cycle challenges.

### Sequence Types

Phase RPMs can be generated by one of several patterns:
- **`random`** - Each phase independently samples from `hi_rpm_range`.
- **`progressive`** - Phase RPMs increase across the challenge.
- **`regressive`** - Phase RPMs decrease across the challenge.
- **`constant`** - All phases share the same RPM.

An explicit `phases: [...]` array on the selection overrides the generated sequence.

### Audio Cues

Cycle challenges emit their own edge-triggered audio cues:
- `cycle_challenge_init` - Rider assigned, entering `init`
- `cycle_phase_complete` - A phase's `maintain_seconds` completed and the next phase begins
- `cycle_success` - Final phase completed
- `cycle_locked` - Transitioned into `locked`
...
Full design reference: `docs/_wip/plans/2026-04-17-cycle-challenge-design.md`.
