...
export function normalizePressureMatMessage(message, previous = null) {
  ...
  return {
    ...
    firmwareBuild: message.firmwareBuild || previous?.firmwareBuild || null,
    detectionState: message.detectionState || previous?.detectionState || null,
    occupancyKnown: typeof message.occupancyKnown === 'boolean' ? message.occupancyKnown : null,
    rearmCount: optionalNonnegative(message.rearmCount, previous?.rearmCount),
