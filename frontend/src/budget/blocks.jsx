...
function BudgetTable({ setDrawerContent, budget }) {
  ...
  const loadAnticipatedTransactions = (month, key) => {
    ...
    switch (key) {
      ...
      case "fixed":
        return Object.keys(activeBudget["monthlyBudget"][month].categories).map((cat) => ({
          date,
          accountName,
          amount: activeBudget["monthlyBudget"][month].categories[cat].amount,
          expenseAmount: activeBudget["monthlyBudget"][month].categories[cat].amount,
          description: cat,
          tagNames: []
        }));
      case "day":
        return [{
          date,
          accountName,
          amount: activeBudget["dayToDayBudget"][month].spent,
          expenseAmount: activeBudget["dayToDayBudget"][month].spent,
  ...
  const loadTransactions = (month, key) => {
    ...
    switch (key) {
      ...
      case "fixed":
        return Object.keys(activeBudget["monthlyBudget"][month].categories).flatMap(cat => activeBudget["monthlyBudget"][month].categories[cat].transactions) || [];
      case "day":
        return activeBudget["dayToDayBudget"][month].transactions || [];
      case "income":
        return activeBudget["monthlyBudget"][month].paychecks.map(paycheck => ({
          date: paycheck.date,
          accountName: "Anticipated",
          amount: paycheck.amount,
          expenseAmount: paycheck.amount,
          description: "Paycheck",
          tagNames: []
  ...
  const rows = (() => {
    ...
    const rows = months.map((month) => {
      ...
      return (
        <tr key={month} className={rowClassName}>
          ...
          <td>{formatAsCurrency(surplus || 0)}</td>
        </tr>
      );
    });
    ...
    const sumRow = (
      <tr>
        ...
        <td>{formatAsCurrency(months.reduce((acc, month) => acc + (monthlyBudget[month]?.surplus || 0), 0))}</td>
      </tr>
    );
