...
const menuMusicSpy = vi.fn();
vi.mock('./useMenuMusic.js', () => ({
  __esModule: true,
  default: (opts) => menuMusicSpy(opts)
}));
...
describe('MenuMusicController — ducking', () => {
  ...
  it('stays active when nothing is recording', () => {
    ...
    expect(menuMusicSpy).toHaveBeenCalledWith(expect.objectContaining({ isActive: true }));
  });

  it('ducks while the voice-memo overlay is open', () => {
    mockCtx.voiceMemoOverlayState = { open: true };
    render(<MenuMusicController isActive trackChangeKey="a" volume={0.1} trackUrls={['x.mp3']} />);
    expect(menuMusicSpy).toHaveBeenCalledWith(expect.objectContaining({ isActive: false }));
  });
  ...
  it('resumes once feedback recording ends', () => {
    ...
    expect(menuMusicSpy).toHaveBeenLastCalledWith(expect.objectContaining({ isActive: false }));
    ...
    expect(menuMusicSpy).toHaveBeenLastCalledWith(expect.objectContaining({ isActive: true }));
  });
