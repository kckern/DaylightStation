...
const build = (over = {}) => {
  timers = fakeTimers();
  return createArcadeGameSessionReporter({
...
describe('arcadeGameSessionReporter', () => {
  ...
  it('requires its essentials rather than silently reporting nothing', () => {
    expect(() => createArcadeGameSessionReporter({ post: () => {} })).toThrow(/deviceId/);
    expect(() => createArcadeGameSessionReporter({ deviceId: 'd' })).toThrow(/post/);
  });
});
