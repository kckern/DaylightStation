...
export function createChartDataSource(session) {
  ...
  if (allUserIds.size > 0) {
    roster = [...allUserIds]
      ...
      const rawName = meta.displayName || meta.display_name || userId;
      // Capitalize first letter if name matches userId (backend sometimes stores lowercase ids as names)
      const displayName = rawName === userId ? userId.charAt(0).toUpperCase() + userId.slice(1) : rawName;
      return {
        id: userId,
        profileId: userId,
        name: displayName,
