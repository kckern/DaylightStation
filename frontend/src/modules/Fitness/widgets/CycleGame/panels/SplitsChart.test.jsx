...
describe('SplitsChart', () => {
  it('renders one column per rider and one row per completed lap', () => {
    const { getAllByTestId } = render(
      <SplitsChart riderIds={['user_2','user_3']} riders={riders} lapLengthM={100} elapsedS={120} />
    );
  ...
  it('shows the per-lap delta (not cumulative) for completed laps', () => {
    const { getAllByTestId } = render(
      <SplitsChart riderIds={['user_2']} riders={{ user_2: riders.user_2 }} lapLengthM={100} elapsedS={120} />
    );
  ...
  it('renders a current-lap row counting up from the last crossing', () => {
    const { getByTestId } = render(
      <SplitsChart riderIds={['user_2']} riders={{ user_2: riders.user_2 }} lapLengthM={100} elapsedS={100} />
    );
  ...
  it('shows an empty state when laps are disabled', () => {
    const { getByTestId } = render(
      <SplitsChart riderIds={['user_2']} riders={{ user_2: { displayName: 'User_2', cumulativeDistanceM: 50 } }} lapLengthM={0} elapsedS={10} />
    );
