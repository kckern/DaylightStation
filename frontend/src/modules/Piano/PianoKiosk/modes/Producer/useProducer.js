...
import { ProducerEngine } from './producerEngine.js';

const DJ_ROOT = 'audio/dj';
const streamUrl = (rel) => `/api/v1/local/stream/${rel}`;

/**
 * Producer data + audio engine. Discovers kits under media/audio/dj (each a folder
...
export function useProducer() {
  ...
  if (!engineRef.current) engineRef.current = new ProducerEngine();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        logger.info('piano.producer.discovered', { count: found.length });
      } catch (e) {
        if (!cancelled) { setKits([]); setError(e.message); }
        logger.warn('piano.producer.discover-failed', { error: e.message });
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        logger.info('piano.producer.kit-loaded', { kit: meta.id, bpm: parsed?.bpm });
      } catch (e) {
        if (!cancelled) setError(e.message);
        logger.warn('piano.producer.kit-failed', { kit: meta.id, error: e.message });
      }
    })();
    return () => { cancelled = true; };
  ...
  const toggleLoop = useCallback((id) => {
    ...
    logger.info('piano.producer.loop', { id, on });
  }, [engine, logger]);
