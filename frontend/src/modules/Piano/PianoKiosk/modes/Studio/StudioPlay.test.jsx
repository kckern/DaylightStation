...
describe('StudioPlay top pane', () => {
  it('always renders the theory triptych (circle · staff · chord) — no layout toggle', () => {
    const { container } = render(<StudioPlay />);
  ...
  it('no longer renders the Record button — it moved to the tab bar', () => {
    const { container } = render(<StudioPlay />);
    expect(container.querySelector('.piano-studio__record')).toBeNull();
    expect(container.querySelector('.piano-studio-play__record')).toBeNull();
  });
});
