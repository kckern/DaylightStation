...
| video | skip-back-15 | `video/skip-back-15.svg` | solar:rewind-back-bold |
| video | skip-forward-15 | `video/skip-forward-15.svg` | solar:rewind-forward-bold |
...
| video | loop-a | `video/loop-a.svg` | solar:repeat-one-bold |
| video | loop-b | `video/loop-b.svg` | solar:repeat-one-bold |
| video | clear-loop | `video/clear-loop.svg` | solar:repeat-one-minimalistic-bold |
...
## Notes
- **skip-back-15/30** share the `rewind-back` glyph; **skip-forward-15/30** share `rewind-forward`. The 15/30 numerals are intended as a UI text overlay.
- **loop-a / loop-b** share the `repeat-one` glyph; overlay the A/B letter in the UI. **clear-loop** uses the minimalistic repeat-one variant.
