...
import { advanceHeroRun, applyHeroPress, createHeroRun, HERO_DEFAULTS } from './heroChart.js';
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (
        ...
        setPhase('complete');
        logger.info('hero.completed', { ...next.score, targets: next.targets.length });
      }
    }, TICK_MS);
    return () => clearInterval(id);
  }, [phase, timing, chart?.durationMs, logger]);

  return { phase, elapsedMs, run, timing, start };
}
