...
import { stemDirectionFor } from '../model/stems.js';
 ...
 * and the widest group — and every card drawn to exactly that box. On the chess
 * rim that took the line spacing from 8px to 13–14px in the same card.
 *
 ...
 *     The lines run past the viewBox to the card's edges.
 *   - The clef is cue-size (3/4), the size notation already uses for a clef that
 *     is not at the head of a system.
 *   - No stems. A stem says how long a note lasts, which the rim does not; it
 *     cost three and a half spaces of height the pitch needed.
...
export function rimClefRight(clef) {
  return CLEF_X + CLEFS[clef === 'bass' ? 'bass' : 'treble'].width * RIM_LINE_SPACING * RIM_CLEF_SCALE;
}
 ...
 */
export function layoutRimCard(pitches, accidental) {
  ...
  let lo = Math.min(STAFF_LO, clefLo);
  let hi = Math.max(STAFF_HI, clefHi);

  if (!sorted.length) {
    const baseX = rimClefRight(clef) + CLEF_GAP + NOTEHEAD_RX;
    return { clef, sorted, offsets: [], accDx: [], baseX, width: baseX + NOTEHEAD_RX + RIGHT_AIR, lo, hi };
  }

  const positions = sorted.map((np) => np.position);
  const offsets = noteheadOffsets(positions, stemDirectionFor(positions))
  ...
  const baseX = rimClefRight(clef) + CLEF_GAP - inkLeft;
  ...
  return { clef, sorted, offsets, accDx, baseX, width: baseX + inkRight + RIGHT_AIR, lo, hi };
}
 ...
 * @param {{accidental?: 'sharp'|'flat'}} [options]
 * @returns {{lo: number, hi: number, width: number}}
 */
export function rimStaffExtent(tokens = [], { accidental } = {}) {
  const cards = (tokens.length ? tokens : [[]]).map((token) => layoutRimCard(pitchesOf(token), accidental));
  return {
...
export function RimStaffRenderer({
  targetPitches = [], activeNotes = null, matched = false, accidental = undefined, extent = null,
}) {
  ...
  const card = useMemo(() => layoutRimCard(pitches, accidental), [pitchKey, accidental]);
  const { lo, hi, width } = extent ?? card;
  ...
  const anchor = CLEFS[card.clef === 'bass' ? 'bass' : 'treble'].anchorSpacesAboveBottomLine;
  ...
  return (
    ...
    >
      ...
      <ClefGlyph clef={card.clef} lineSpacing={cueSpacing} bottomLineY={clefBottomLineY} />

      {card.sorted.map((np, i) => {
        const noteY = yOf(np.position);
        const noteX = card.baseX + card.offsets[i];
        return (
          <g key={np.pitch}>
            {ledgerLineYs(np.position, bottomLineY, STEP).map((ly, li) => (
              <line
                key={`ledger-${li}`}
                x1={card.baseX - LEDGER_HALF}
                x2={card.baseX + LEDGER_HALF}
                y1={ly}
                y2={ly}
                stroke={INK}
                strokeWidth="1"
            ...
            {card.accDx[i] !== null && (
              <g
                ...
                transform={`translate(${card.baseX + card.accDx[i]}, ${noteY})`}
              >
      ...
      {ghosts.map((ghost) => {
        ...
        const noteX = ghostX + ghost.column * 2 * NOTEHEAD_RX;
        return (
          <g key={`ghost-${ghost.pitch}`} className="action-staff__ghost">
            {ledgerLineYs(ghost.position, bottomLineY, STEP).map((ly, li) => (
              <line key={`ghost-ledger-${li}`} x1={ghostX - LEDGER_HALF} x2={ghostX + LEDGER_HALF} y1={ly} y2={ly} {...GHOST_INK.ledger} />
            ))}
      ...
      {beyond.above && (
        <path
          ...
          d={`M ${ghostX - 7} 9 L ${ghostX + 7} 9 L ${ghostX} 1 Z`}
          {...GHOST_INK.head}
        />
      )}
      {beyond.below && (
        <path
          ...
          d={`M ${ghostX - 7} ${height - 9} L ${ghostX + 7} ${height - 9} L ${ghostX} ${height - 1} Z`}
          {...GHOST_INK.head}
        />
      )}
    </svg>
