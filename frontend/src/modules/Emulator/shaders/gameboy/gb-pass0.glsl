...
// DaylightStation: dot colour source. 0 = the palette's foreground colour (a
// DMG), 1 = the game's own colour (a Game Boy Color). Ported from v1.1's
// color_toggle (Matt Akins, 2024) — see README.md.
#pragma parameter color_toggle "Color Mode" 0.0 0.0 1.0 1.0
...
void main()
{
	float video_scale_factor = floor(outsize.y / OrigInputSize.y);
...
#define prev0_rgb abs(1.0 - COMPAT_TEXTURE(PrevTexture,  history_coord(PrevInputSize.xy, PrevTextureSize.xy)).rgb)
#define prev1_rgb abs(1.0 - COMPAT_TEXTURE(Prev1Texture, history_coord(Prev1InputSize.xy, Prev1TextureSize.xy)).rgb)
#define prev2_rgb abs(1.0 - COMPAT_TEXTURE(Prev2Texture, history_coord(Prev2InputSize.xy, Prev2TextureSize.xy)).rgb)
#define prev3_rgb abs(1.0 - COMPAT_TEXTURE(Prev3Texture, history_coord(Prev3InputSize.xy, Prev3TextureSize.xy)).rgb)
#define prev4_rgb abs(1.0 - COMPAT_TEXTURE(Prev4Texture, history_coord(Prev4InputSize.xy, Prev4TextureSize.xy)).rgb)
#define prev5_rgb abs(1.0 - COMPAT_TEXTURE(Prev5Texture, history_coord(Prev5InputSize.xy, Prev5TextureSize.xy)).rgb)
#define prev6_rgb abs(1.0 - COMPAT_TEXTURE(Prev6Texture, history_coord(Prev6InputSize.xy, Prev6TextureSize.xy)).rgb)
...
void main()
{
    ...
    float lum_alpha = (input_rgb.r + input_rgb.g + input_rgb.b) / grey_balance;
    // Gambatte's DMG "white" is (255,251,255), not pure white, so a blank pixel
    // carries ~0.5% alpha and pass4 treats it as a dot: the paper under it gets
    // flattened and a grid appears over empty screen. Snap near-zero to zero.
    if (lum_alpha < 0.03) lum_alpha = 0.0;
    float rgb_to_alpha = lum_alpha + (is_on_dot * baseline_alpha);
    ...
    // Colour mode (Game Boy Color): the dot takes the game's own colour instead
    // of the palette foreground. Opacity still comes from how dark it is.
    vec3 dot_rgb = mix(foreground_color, COMPAT_TEXTURE(Source, vTexCoord).rgb, color_toggle);
    vec4 out_color = vec4(dot_rgb, rgb_to_alpha);
