...
// DaylightStation: `color_toggle` (1 = Game Boy Color — the dots keep the
// game's own colours instead of being tinted by the palette background) is
// read here too, but its `#pragma parameter` lives in gb-pass0.glsl ONLY.
// Declaring the same parameter in two passes makes the web core's RetroArch
// drop the preset's override for it (verified 2026-09-25: the pragma default
// applied, the .glslp value did not). RetroArch sets every registered
// parameter on every pass's program by name, so the uniform below still
// receives the value.
...
void main()
{
    ...
    // Palette mode tints the dots with the background colour; in colour mode
    // (Game Boy Color) the game's own colours pass through untinted.
    foreground *= mix(bg_color, vec4(1.0), color_toggle);
