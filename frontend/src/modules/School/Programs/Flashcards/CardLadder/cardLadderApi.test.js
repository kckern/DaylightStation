...
vi.mock('./cardLadderLog.js', () => ({ cardLadderLog: { apiRejected: vi.fn(), apiFailed: vi.fn() } }));

afterEach(() => { vi.unstubAllGlobals(); });

describe('createCardLadderApi', () => {
  it('live mode calls the plain base and never sends a scenario', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/v1/school/card-ladder/open', expect.objectContaining({
  ...
  it('test mode calls the /test base and forwards a scenario', async () => {
    const fetchMock = vi.fn(async () => ({ ok: true, status: 200, json: async () => ({ sittingId: 's1' }) }));
    vi.stubGlobal('fetch', fetchMock);
    const api = createCardLadderApi({ test: true });
    await api.open({ userId: 'kid', deckId: 'language/korean/week-01', scenario: 'exhausted' });
    expect(fetchMock).toHaveBeenCalledWith('/api/v1/school/card-ladder/test/open', expect.objectContaining({
  ...
  it('intro is a GET on the mount\'s own base; the test mount forwards a scenario', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    await createCardLadderApi({ test: false }).intro({ userId: 'kid', deckId: 'language/korean/week-01', scenario: 'fresh' });
    expect(fetchMock).toHaveBeenLastCalledWith('/api/v1/school/card-ladder/intro?userId=kid&deckId=language%2Fkorean%2Fweek-01', expect.objectContaining({ method: 'GET' }));
    await createCardLadderApi({ test: true }).intro({ userId: 'kid', deckId: 'd', scenario: 'fresh' });
    expect(fetchMock).toHaveBeenLastCalledWith('/api/v1/school/card-ladder/test/intro?userId=kid&deckId=d&scenario=fresh', expect.objectContaining({ method: 'GET' }));
  });
  ...
  it('respond, get and close hit the right paths with the right verbs', async () => {
    ...
    expect(fetchMock).toHaveBeenLastCalledWith('/api/v1/school/card-ladder/sittings/sit1/items/r1%3As%3A0', expect.objectContaining({ method: 'POST', body: JSON.stringify({ userId: 'kid', response: { seen: true } }) }));
    await api.get('sit1', 'kid');
    expect(fetchMock).toHaveBeenLastCalledWith('/api/v1/school/card-ladder/sittings/sit1?userId=kid', expect.objectContaining({ method: 'GET' }));
    await api.close('sit1', { userId: 'kid', reason: 'idle' });
    expect(fetchMock).toHaveBeenLastCalledWith('/api/v1/school/card-ladder/sittings/sit1/close', expect.objectContaining({ method: 'POST', body: JSON.stringify({ userId: 'kid', reason: 'idle' }) }));
  });
  ...
  it('open forwards capabilities when given, and omits them when not', async () => {
    ...
    expect(fetchMock).toHaveBeenLastCalledWith('/api/v1/school/card-ladder/open', expect.objectContaining({
  ...
  it('uploadRecording sends the raw blob with its own Content-Type and userId/ext on the query', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/v1/school/card-ladder/sittings/sit1/recordings/r1%3Asay%3A0?userId=kid&ext=webm',
  ...
  it('uploadRecording omits ext from the query when not given', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/v1/school/card-ladder/sittings/sit1/recordings/r1%3Asay%3A0?userId=kid',
      expect.anything(),
    );
  });
  ...
  it('practice posts mode/help/filter/chosen/frontSide', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/v1/school/card-ladder/sittings/sit1/practice', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({
  ...
  it('words GETs with userId and deckId on the query', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/v1/school/card-ladder/words?userId=kid&deckId=language%2Fkorean%2Fweek-01',
  ...
  it('words passes the sitting id when given (the test mount requires it)', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/v1/school/card-ladder/test/words?userId=kid&deckId=d&sittingId=test.p.t.1',
      expect.objectContaining({ method: 'GET' }),
    );
  });
});
