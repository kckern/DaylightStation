...
vi.mock('./CardLadderStage.jsx', () => ({ default: ({ children }) => <div data-testid="stage">{children}</div> }));
...
vi.mock('./cardLadderLog.js', async (importOriginal) => {
  const actual = await importOriginal();
  const wrapped = { ...actual.cardLadderLog, sessionReopened: logs.sessionReopened };
  return { ...actual, cardLadderLog: wrapped, default: wrapped };
});
...
describe('CardLadderProgram — Start screen', () => {
  it('does not open the sitting until Start is tapped', async () => {
    const api = fakeApi();
    render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
  ...
  it('shows the descriptor title when one is known', () => {
    render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner', title: 'Korean words' }} api={fakeApi()} />);
  ...
  it('Space and Enter start the sitting too', async () => {
    for (const key of [' ', 'Enter']) {
      const api = fakeApi();
      const { unmount } = render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
  ...
  it('a test sitting shows the banner on the Start screen', () => {
    render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner', test: true }} api={fakeApi(true)} />);
  ...
  it('Exit before Start exits without opening or closing anything', () => {
    const api = fakeApi();
    const onExit = vi.fn();
    render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} onExit={onExit} />);
...
describe('CardLadderProgram — the start screen is a launch card', () => {
  it('draws poster, class, unit, today and words learned — and opens nothing before Start', async () => {
    const api = introApi();
    const shown = vi.spyOn(cardLadderLog, 'introShown');
    render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
  ...
  it('a poster that fails to load becomes the calm placeholder, never a substitute', async () => {
    render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={introApi()} />);
  ...
  it('a done day says so on the card', async () => {
    ...
    render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={introApi(done)} />);
  ...
  it('when the card facts are unavailable it falls back to the title and Start still opens the day', async () => {
    ...
    render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner', title: 'Korean words' }} api={api} />);
  ...
  it('a test sitting asks the test intro with its scenario, and keeps the banner', async () => {
    ...
    render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner', test: true, scenario: 'fresh' }} api={api} />);
    expect(await screen.findByRole('heading', { name: 'Test Class' })).toBeInTheDocument();
...
describe('CardLadderProgram — the sitting header', () => {
  ...
  it('a step\'s hint shows the first time the step appears, goes on the first input, and never returns for that step', async () => {
    const api = fakeApi();
    const entered = vi.spyOn(cardLadderLog, 'stepEntered');
  ...
  it('Exit is an icon + label button that behaves as Leave did', async () => {
    const api = fakeApi();
    const onExit = vi.fn();
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} onExit={onExit} />);
    await screen.findByText('가위');
...
describe('CardLadderProgram', () => {
  ...
  it('a test sitting shows the banner', async () => {
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner', test: true }} api={fakeApi(true)} />);
    expect(await screen.findByText(/TEST — nothing is saved/)).toBeInTheDocument();
  });

  it('a live sitting shows no banner', async () => {
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={fakeApi()} />);
    await screen.findByText('가위');
  ...
  it('opens with the descriptor scenario', async () => {
    ...
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner', test: true, scenario: 'round-end' }} api={api} />);
    await screen.findByText('가위');
  ...
  it('Exit closes once as leave — unmount afterwards sends nothing more', async () => {
    const api = fakeApi();
    const { unmount } = renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
    await screen.findByText('가위');
    fireEvent.click(screen.getByRole('button', { name: /exit/i }));
...
describe('CardLadderProgram — dispatches every item type', () => {
...
describe('CardLadderProgram — spec §8 trace and events', () => {
  afterEach(() => { vi.useRealTimers(); });

  it('sitting.opened fires on open, item.shown carries a derived layout/media and fontPx:null', async () => {
    const shownSpy = vi.spyOn(cardLadderLog, 'itemShown');
    const openedSpy = vi.spyOn(cardLadderLog, 'sittingOpened');
    const api = fakeApi();
    api.open.mockResolvedValue(openWith(choice));
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
    await screen.findByText('Glue');
  ...
  it('item.layout follows item.shown once with the main FitText\'s computed fontPx', async () => {
    const layoutSpy = vi.spyOn(cardLadderLog, 'itemLayout');
    const api = fakeApi();
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
    await screen.findByText('가위');
  ...
  it('sitting.closed on Leave carries the reason and the progress the sitting closed on', async () => {
    const closedSpy = vi.spyOn(cardLadderLog, 'sittingClosed');
    const api = fakeApi();
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
    await screen.findByText('가위');
    fireEvent.click(screen.getByRole('button', { name: /exit/i }));
  ...
  it('item.answered carries the response that was sent, and a numeric ms', async () => {
    const answeredSpy = vi.spyOn(cardLadderLog, 'itemAnswered');
    const api = fakeApi();
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
    await screen.findByText('가위');
  ...
  it('round.started fires for the opening round, and round.ended/round.started fire again when progress.round.index advances', async () => {
    const startedSpy = vi.spyOn(cardLadderLog, 'roundStarted');
    const endedSpy = vi.spyOn(cardLadderLog, 'roundEnded');
    const api = fakeApi();
  ...
  it('item.stalled fires at 45s of no input on the current item', async () => {
    vi.useFakeTimers();
    const stalledSpy = vi.spyOn(cardLadderLog, 'itemStalled');
    const api = fakeApi();
    render(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
  ...
  it('item.prompt-fallback logs when a cue asks for an image but the server sent no asset for it', async () => {
    const fallbackSpy = vi.spyOn(cardLadderLog, 'promptFallback').mockImplementation(() => {});
    const api = fakeApi();
  ...
  it('visibility is logged on a document visibilitychange', async () => {
    const visSpy = vi.spyOn(cardLadderLog, 'visibility');
    const api = fakeApi();
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
    await screen.findByText('가위');
...
describe('CardLadderProgram — spec §8 input, verdict panel and hints', () => {
  ...
  it('a held verdict logs result.shown, and Next past it logs result.dismissed with how', async () => {
    const shownSpy = vi.spyOn(cardLadderLog, 'resultShown');
    const dismissedSpy = vi.spyOn(cardLadderLog, 'resultDismissed');
    const api = fakeApi();
    api.open.mockResolvedValue(openWith(choice));
    api.respond.mockResolvedValue({ ok: true, status: 200, data: { result: { correct: false, answer: 'Scissors' }, item: stream, progress } });
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
  ...
  it('a step hint shown for the first time logs hint.shown', async () => {
    const hintSpy = vi.spyOn(cardLadderLog, 'hintShown');
    const api = fakeApi();
    renderStarted(<CardLadderProgram descriptor={{ deckId: 'd', userId: 'test-learner' }} api={api} />);
    await screen.findByText('가위');
