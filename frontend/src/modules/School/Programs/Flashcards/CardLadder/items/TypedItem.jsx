...
const KEYBOARD_LOGGED_KEY = 'card-ladder.keyboard-detected';
...
function logKeyboardOnce(itemId) {
  ...
  cardLadderLog.keyboardDetected({ itemId });
}
...
export default function TypedItem({ item, mode, langs, resolveAssetUrl, onRespond, result = null, onContinue, busy = false, stageRef = null, pending = false, onLayout }) {
  ...
  useEffect(() => {
    ...
    autoTimerRef.current = setTimeout(() => {
      ...
      input.current?.focus();
      cardLadderLog.keypadToggled({ auto: true, open: true });
    }, KEYPAD_AUTO_OPEN_MS);
  ...
  useEffect(() => {
    const onPhysicalKeydown = (event) => {
      ...
      if (keypadOpenRef.current) {
        setKeypadOpen(false);
        cardLadderLog.keypadToggled({ auto: true, open: false });
      }
    };
  ...
  const startLongPress = () => {
    ...
    longPress.current = setTimeout(() => {
      ...
      keypadUsedRef.current = true;
      setKeypadOpen(true);
      cardLadderLog.keypadToggled({ auto: false, open: true, via: 'long-press' });
      input.current?.focus();
    }, LONG_PRESS_MS);
  };
  ...
  const toggleKeypad = () => {
    ...
    setKeypadOpen(next);
    cardLadderLog.keypadToggled({ auto: false, open: next });
    input.current?.focus();
  };
  ...
  const showMe = () => {
    ...
    cardLadderLog.showMeUsed({ itemId: item.id, via: currentInput(), ms: Date.now() - shownAtRef.current });
    onRespond({ typed: '' });
  ...
  const hear = answerAudio
    ...
    : graded && anchorCueAudio(item, resolveAssetUrl) ? () => playClip(anchorCueAudio(item, resolveAssetUrl), 'gloss') : null;
