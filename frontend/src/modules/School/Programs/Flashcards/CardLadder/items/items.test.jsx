...
describe('FlashcardItem', () => {
  ...
  it('logs card.flipped on flip, card.sorted with the pile on a sort, and card.undone on Undo', () => {
    const flipped = vi.spyOn(cardLadderLog, 'cardFlipped').mockImplementation(() => {});
    const sorted = vi.spyOn(cardLadderLog, 'cardSorted').mockImplementation(() => {});
    const undone = vi.spyOn(cardLadderLog, 'cardUndone').mockImplementation(() => {});
    const onRespond = vi.fn();
...
describe('image cues', () => {
  ...
  it('ChoiceItem: an image that fails to load drops out (the text stays) and logs', () => {
    const spy = vi.spyOn(cardLadderLog, 'mediaFailed').mockImplementation(() => {});
...
describe('TypedItem keypad toggle', () => {
  ...
  it('auto-opens once after 10 s of idle focus, and logs keypad.toggled {auto:true}', () => {
    const spy = vi.spyOn(cardLadderLog, 'keypadToggled').mockImplementation(() => {});
    vi.useFakeTimers();
...
describe('TypedItem — busy keeps the keypad, Show me', () => {
  ...
  it('Show me is logged as showme.used with how it was asked for (spec §8)', () => {
    const spy = vi.spyOn(cardLadderLog, 'showMeUsed').mockImplementation(() => {});
