...
export default function FlashcardItem({ item, langs, resolveAssetUrl, onRespond, busy = false, onLayout }) {
  ...
  const flip = () => {
    ...
    cardLadderLog.cardFlipped({ itemId: item.id, ms: Date.now() - shownAt.current });
  ...
  const act = (response) => {
    if (busy) return;
    if (response.sort) cardLadderLog.cardSorted({ itemId: item.id, pile: response.sort });
    if (response.undo) cardLadderLog.cardUndone({ itemId: item.id });
    onRespond(response);
  };
