 ...
 * SERVER actually sent (`CardLadderSittingService#publicItem`) — not just a
...
export default function SayItem({
  ...
  const logTake = useCallback((phase, extra = {}) => {
    ...
    cardLadderLog.sayRecording({ itemId: item.id, itemMode: mode, phase, ms: Date.now() - shownAtRef.current, ...extra });
  }, [item.id, mode]);
  ...
  // Mount-only (CardLadderProgram keys each item to a fresh instance, so
  ...
  useEffect(() => {
    ...
    cardLadderLog.noticeShown({ itemId: item.id, itemMode: mode, reason: recorder.verdict });
  ...
  const cueClip = showCue ? anchorCueAudio(item, resolveAssetUrl) : null;
