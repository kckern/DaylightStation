...
| A student's **Words** tab — one table per word-ladder deck (term, gloss, state chip, stage, due, streak, tricky, excluded, typed answers), Reset / Mark mastered / Exclude / Re-grade, and the deck pool's Drop from pool | `WordsView` → `WordLadderWordsPanel` | `WorkspaceViews.jsx`, `panels/WordLadderWordsPanel.jsx` (enrollments filtered from the learner's `programs` — `wordLadderEnrollments`; writes through `wordLadderAdminApi.js`, a separate client from `teacherWorkspaceApi.js`/`schoolApi.js` because the admin routes are mounted at `/api/v1/school/word-ladder/admin`, not `/api/v1/school/teacher`) |
