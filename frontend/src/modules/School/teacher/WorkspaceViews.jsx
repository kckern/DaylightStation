...
function AgendaPreview({ learnerId, learnerName }) {
  ...
  const dispatch = () => {
    const idempotencyKey = newIdempotencyKey(`agenda:${learnerId}`);
    run(`agenda:${learnerId}`, ({ actorId, pin }) => teacherWorkspaceApi.agendaDispatch(learnerId, {
      learnerName, dispatchedBy: actorId, pin,
    }, idempotencyKey), { onSuccess: (receipt) => { setDispatchReceipt(receipt); setDispatchOpen(false); } });
  };
...
export function OperationsView({ kids }) {
  return <div className="teacher-view"><div className="teacher-view__heading"><div><p className="teacher-view__eyebrow">School operations</p><h2>Health, gates, and exceptions</h2><p>Find systematic blockers before changing a student record.</p></div></div><StaleSessions kids={kids} /><ActiveOverrides kids={kids} /><PeriodsTimeline /><CapabilityNotice>Device health, retained-artifact audit, and bulk regrade will appear here when their teacher read models are available.</CapabilityNotice></div>;
}
...
function GradeCorrection({ sessionId, revision, currentPercent, onApplied }) {
  ...
  const applyChange = () => run(key, (auth) => teacherWorkspaceApi.adjustGrade(sessionId, body(auth, true)), {
...
export function SessionInspector({ learnerId, sessionId, kids, onBack }) {
  ...
  const artifactId = session?.artifactIds?.[0] ?? sessionState?.artifactId ?? sessionState?.document?.artifactId ?? sessionState?.issued?.artifactId ?? null;
  ...
  const offerRetake = () => run(sessionId, () => schoolApi.offerRetake(sessionId), { onSuccess: () => setAttempt((n) => n + 1) });
      ...
      {session && (
        <>
          ...
          <section className="teacher-panel">
            ...
            {artifactId ? <div className="teacher-artifact-actions"><a target="_blank" rel="noreferrer" href={`/api/v1/school/teacher/artifacts/${encodeURIComponent(artifactId)}/original.pdf`}>Issued PDF</a><button type="button" disabled title="Postview rendering is not enabled on this install">Postview PDF</button><span>{artifactId}</span></div> : <CapabilityNotice>No retained artifact is linked to this session. Legacy issues may only have event metadata.</CapabilityNotice>}
          </section>
