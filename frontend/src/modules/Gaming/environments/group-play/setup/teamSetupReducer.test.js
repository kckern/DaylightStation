...
const PRESET = {
  ...
  teams: [
    { name: 'Kids', color: '#e6b325', members: [{ id: 'learner2', name: 'learner2', avatar: null }] },
...
describe('teamSetupReducer', () => {
  ...
  it('init with presets loads the first preset', () => {
    ...
    expect(s.teams[0].members[0].id).toBe('learner2');
  });
  ...
  it('ASSIGN_MEMBER moves a member between teams (no duplicates)', () => {
    ...
    const learnerTwo = { id: 'learner2', name: 'learner2', avatar: null };
    s = teamSetupReducer(s, { type: 'ASSIGN_MEMBER', teamId: 'team_2', member: learnerTwo });
    expect(s.teams[0].members.find((m) => m.id === 'learner2')).toBeUndefined();
    expect(s.teams[1].members.some((m) => m.id === 'learner2')).toBe(true);
    s = teamSetupReducer(s, { type: 'REMOVE_MEMBER', teamId: 'team_2', memberId: 'learner2' });
    expect(s.teams[1].members.some((m) => m.id === 'learner2')).toBe(false);
  });
