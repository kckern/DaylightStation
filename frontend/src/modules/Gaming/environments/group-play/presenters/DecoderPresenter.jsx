...
const logger = getChildLogger({ component: 'group-play-decoder' });

export function DecoderText({ children }) {
  return <span className="group-play-decoder-text" style={{ color: '#29b36b', background: 'repeating-linear-gradient(0deg,#d71920 0 3px,#ef6b70 3px 6px)' }}>{children}</span>;
}
...
export function HighContrastMask({ src, alt = '' }) {
  ...
  return failed
    ? <img src={src} alt={alt} className="group-play-decoder-mask" style={{ filter: 'grayscale(1) contrast(4)' }} />
    : <canvas ref={canvasRef} role="img" aria-label={alt} className="group-play-decoder-mask" />;
}
