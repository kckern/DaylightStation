...
import TitleCard from '../ui/TitleCard.jsx';
import GroupPlayResults from '../ui/GroupPlayResults.jsx';
import { EXPERIENCE_REGISTRY } from './experienceRegistry.js';
import './GroupPlayApp.scss';
import '../ui/fonts.js';
...
function HostQr({ sessionId }) {
  ...
  const hostUrl = `${window.location.origin}/group-play/host/${sessionId}`;
  const src = `/api/v1/qrcode?data=${encodeURIComponent(hostUrl)}&size=180`;
  return (
    <div className="group-play__hostqr" title={hostUrl}>
...
function BuzzerBind({ teams, onDone }) {
  ...
  return (
    <div className="group-play__bind">
...
export default function GroupPlayApp({ dismiss, clear }) {
  ...
  return (
    <div className="group-play" data-phase={flow.phase}>
      {flow.error && <div className="group-play__error">{flow.error}</div>}
      {!connected && <div className="group-play__ws-warn" title="Buzzers offline — keyboard still works">⚡</div>}

      {flow.phase === 'loading' && <TitleCard title="Group Play" subtitle="Loading…" />}

      {flow.phase === 'set-picker' && (
        <div className="group-play__sets">
          <TitleCard title="Group Play" subtitle="Pick a game" />
          {flow.sets.map((s) => (
          ...
          {flow.sets.length === 0 && <p>No mounted group-play experiences are available.</p>}
        </div>
      )}

      {flow.phase === 'team-setup' && (
        <div className="group-play__team-and-host">
          {(flow.setupProfile.host_modes || []).length > 0 && <fieldset className="group-play__host-mode"><legend>Host</legend>{flow.setupProfile.host_modes.map((mode) => <button key={mode} type="button" aria-pressed={flow.hostMode === mode} onClick={() => dispatchFlow({ type: 'SET_HOST_MODE', hostMode: mode })}>{mode.replace('-', ' ')}</button>)}</fieldset>}
      ...
      {flow.phase === 'playing' && !flow.sessionId && <TitleCard title="Group Play" subtitle="Creating session…" />}

      {flow.phase === 'playing' && flow.sessionId && !Game && (
        <div className="group-play__error" role="alert">Mounted presenter unavailable: {flow.presenterId || 'missing'}</div>
      )}

      {flow.phase === 'results' && (
        <GroupPlayResults teams={flow.teams} scores={finalScores}
