...
import { createSession, fetchBoot, finishSession } from './sessionClient.js';

describe('group-play session client', () => {
  beforeEach(() => DaylightAPI.mockReset());

  it('loads an entirely mounted group-play catalog', async () => {
    ...
    expect(DaylightAPI).toHaveBeenCalledWith('api/v1/gaming/environments/group-play/profile');
    expect(DaylightAPI).toHaveBeenCalledWith('api/v1/gaming/environments/group-play/catalog');
  });
  ...
  it('creates a session by mounted definition identity only', async () => {
    ...
    expect(DaylightAPI).toHaveBeenCalledWith('api/v1/gaming/sessions', {
      definition_id: 'quiz:s1', participants: [], seats: [{ id: 'red', members: [] }], setup: { teams: [{ id: 'red', members: [] }] },
    }, 'POST');
  });
  ...
  it('closes through the canonical endpoint', async () => {
    DaylightAPI.mockResolvedValueOnce({ header: { status: 'complete' } });
