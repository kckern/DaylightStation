...
export class AudioCueEngine {
  constructor({ pack = 'classic', mute = false, audioFactory = (src) => new Audio(src) } = {}) {
    ...
    this.log = getLogger().child({ component: 'group-play-audio' });
  }
  ...
  play(cue, { channel = 'sfx', loop = false } = {}) {
    if (this.mute) return;
    try {
      // served by the group-play router's /media route (raw /media/* is not served)
      ...
      const p = audio.play();
      p?.catch?.((err) => this.log.warn('group-play.audio.play_failed', { cue, error: err.message }));
    } catch (err) {
      this.log.warn('group-play.audio.error', { cue, error: err.message });
    }
  }
