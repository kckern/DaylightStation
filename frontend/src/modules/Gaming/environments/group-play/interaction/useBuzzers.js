...
const log = () => getLogger().child({ component: 'group-play-buzzers' });

/**
 * Wires buzz sources into a BuzzerArbiter:
 *  - WS `group-play`/`buzz` events (MQTT relay or the debug POST /buzz endpoint)
  ...
  const handleSlot = useCallback((slot, source) => {
    ...
    if (arbiter.bindingTeamId) {
      if (arbiter.handleBindPress(slot)) {
        log().info('group-play.buzzer.bound', { slot, source });
    ...
    if (teamId) {
      log().info('group-play.buzz.locked', { slot, teamId, source });
      setLocked(teamId);
