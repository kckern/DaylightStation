...
export default function PartyGamesResults({ seats = [], result = null, competition = true, onPlayAgain, onExit }) {
  const casual = competition === false;
  const scoreMap = Object.fromEntries((casual ? [] : result?.scores || []).map((score) => [score.subject_id, score.value]));
  const ranked = [...seats].sort((left, right) => (scoreMap[right.id] ?? 0) - (scoreMap[left.id] ?? 0));
  const explicitWinnerIds = casual ? [] : result?.outcome?.winner_ids || [];
  ...
  return (
    <div className={`party-games-results${casual ? " party-games-results--casual" : ""}`} data-testid="results">
        ...
        {ranked.length > 0 && <ol className="party-games-results__list">
          {ranked.map((seat) => (
            <li key={seat.id} style={{ '--team-color': seat.color || 'var(--gp-neutral-team)' }}>
              {!casual && <span className="party-games-results__rank" aria-hidden="true" />}
