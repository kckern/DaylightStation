...
function harness({ resumeStarted = false, definition = cardBattleFixture, mutateState = null, runtimeResult = null, clock = () => 1000 } = {}) {
  let session = {
    session_id: 'game_test', game_id: 'fixture-card-battle', status: 'active', revision: 0,
  ...
  const provider = { id: 'test', createRuntime: vi.fn(async () => runtime) };
  const logger = { info: vi.fn(), warn: vi.fn(), error: vi.fn(), child() { return this; } };
  const controller = new GamingController({
    api, providerRegistry: createProviderRegistry([provider]), gameId: 'fixture-card-battle',
...
describe('GamingController', () => {
  it('resumes the session against its pinned definition instead of current mounted content', async () => {
    ...
    expect(controller.getSnapshot().session.session_id).toBe('game_stale');
    controller.dispose();
  });
  ...
  it('surfaces a failed pass-gated result as consumed with no musical effect', async () => {
    const definition = structuredClone(cardBattleFixture);
    Object.values(definition.cards).forEach((card) => {
      card.resolution = { effect: 'score', requires_pass: true, failure: 'consume-no-effect' };
    });
    const { controller, logger } = harness({
      definition,
      runtimeResult: {
        status: 'completed', score: 0.9, metrics: {}, provider_version: 'test', attempt_id: 'attempt-failed-gate',
        verdict: { score: 0.9, passed: false, failed_criteria: ['placement'], failed_gates: ['pace'] },
      },
    });
    await controller.start();
    const opening = controller.getSnapshot().session;
    const enemyHealth = opening.state.enemy.health;
    await controller.chooseAction(opening.state.zones.hand[0].instance_id);
    expect(controller.getSnapshot().session.state.enemy.health).toBe(enemyHealth);
    expect(controller.getSnapshot().combatResult).toMatchObject({
      damage: 0,
      passed: false,
      failedCriteria: ['placement'],
      failedGates: ['pace'],
      effectiveness: 'No effect — requirements not met',
    });
    expect(logger.info).toHaveBeenCalledWith('gaming.challenge.completed', expect.objectContaining({
      passed: false, failedCriteria: ['placement'], failedGates: ['pace'],
    }));
    controller.dispose();
  });
  ...
  it('refunds a pending card when provider preparation fails', async () => {
    ...
    runtime.prepare.mockRejectedValueOnce(new Error('provider unavailable'));
    await controller.start();
  ...
  it('starts a clean rematch from the terminal battle result', async () => {
    ...
    const rematchState = createInitialState(cardBattleFixture, { seed: 11, participants: [{ user_id: 'guest' }] });
    api.createSession.mockImplementationOnce(async () => ({
      session_id: 'game_rematch', game_id: 'fixture-card-battle', status: 'active', revision: 0,
      definition_hash: 'definition-current', definition: cardBattleFixture,
      state: rematchState, interaction: deriveInteraction(rematchState, cardBattleFixture, 'guest'), events: [],
    }));
