...
const SIGNAL_COLORS = Object.freeze(['#fff34d', '#ff83d5', '#ff7438', '#ffffff']);
const MASK_COLORS = Object.freeze(['#24dff2', '#2453ff', '#4de34c']);
...
function segmentPoints([x1, y1, x2, y2]) {
...
export default function SegmentedSecretText({ text, label = 'Secret clue' }) {
  const value = String(text || '').toUpperCase();
  const words = value.split(/(\s+)/);
  let glyphIndex = 0;
  return (
    <div className="segmented-secret-text" role="img" aria-label={`${label}: ${value}`}>
