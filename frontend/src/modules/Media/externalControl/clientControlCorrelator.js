...
export function createClientControlCorrelator({ controlClientId, service, timeoutMs = 10_000 }) {
  ...
  const unsubscribe = service.subscribe((message) => message?.topic === CLIENT_ACK_TOPIC(controlClientId), (ack) => {
    ...
    if (entry.command.command === 'handoff' && !validateHandoffCommandAck(entry.command, ack, { target: { kind: 'client', id: entry.targetControlClientId } }).valid) return;
  ...
  return {
    send({ targetControlClientId, command }) {
      ...
      if (!validateCommandEnvelope(command).valid) return Promise.reject(new Error('control-invalid-command'));
      ...
      return new Promise((resolve, reject) => {
        ...
        pending.set(command.commandId, { targetControlClientId, command, resolve, reject, timer });
