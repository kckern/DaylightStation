...
export function usePortionDraft(day, date) {
  ...
  const control = {
    ...
    begin(row) {
      ...
      if (!portion.value) return false;
      write({ row, portion, date, operationId: crypto.randomUUID(), status: 'editing' });
      logger.debug('begin', { id: entryId(row), unit: portion.unit });
      return true;
    },
    preview(value) {
      const state = current.current;
      if (!state || state.status !== 'editing' || !Number.isFinite(value) || value <= 0) return;
      write({ ...state, portion: { ...state.portion, value } });
    },
    ...
    async retry() {
      ...
      if (state.conflict) {
        ...
        try {
          ...
          if (!freshPortion.value || freshPortion.unit !== state.portion.unit) throw new Error('The portion unit changed or is unknown. Discard this draft and review the entry.');
    ...
    async commit() {
      const state = current.current;
      if (!state || !['editing', 'error'].includes(state.status)) return;
      if (state.portion.value === foodPortion(state.row).value) { write(null); return; }
      write({ ...state, status: 'saving' });
      try {
        const result = await updateEntry(state.row, { portion: state.portion }, state.operationId);
        ...
        day.reload();
        logger.info('saved', { id: entryId(state.row), portion: state.portion });
      } catch (error) {
        ...
        day.reload();
        logger.warn('failed', { id: entryId(state.row), error: error.message });
      }
    },
  };
