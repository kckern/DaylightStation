...
export function AuditorConfig({ resource, spend }) {
  ...
  const [cap, setCap] = useState(capText(settings?.dailyCapUsd));
  useEffect(() => { setCap(capText(settings?.dailyCapUsd)); }, [settings?.dailyCapUsd]);

  const refresh = () => { resource.reload(); invalidateApiResources(path => path === settingsLogPath); };
  const save = async (field, change) => {
    if (busy) return;
    setBusy(field); setError(null);
    try {
      await DaylightAPI(`${cleanupPath}/settings`, { expectedVersion: resource.data.version, ...change }, 'PATCH');
      logger.info('health-auditor.settings.saved', { field });
  ...
  const commitCap = () => {
    ...
    if (next === undefined) { setError(`Daily cap must be between $0 and $${MAX_CAP}.`); return; }
    if (next === (settings.dailyCapUsd ?? null)) return;
  ...
  const models = AUDITOR_MODELS.map(model => ({
    value: model,
    label: `${model} · ${byModel[model]?.runs ? `≈ ${formatUsd(byModel[model].avgUsd, 3)} / run` : 'no runs yet'}`,
  }));
  ...
  return <SectionCard title="Auditor settings">
    <Stack gap="md">
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="sm">
        ...
        <NumberInput label="Daily cap ($)" description="Blank for no cap" value={cap} min={0} max={MAX_CAP} decimalScale={2} step={0.25}
