...
describe('fkb launchIntent', () => {
  ...
  // Keys are ours (ROM/LIBRETRO/CONFIGFILE), never user input — so a key
  // carrying an intent separator means something upstream is building keys
  // dynamically, which is a bug to surface rather than a value to sanitize.
  // Values are the opposite and do get encoded (see the cases above).
  it('rejects a structural extra key instead of encoding it', () => {
    const { startIntent } = mockBridge();
    expect(launchIntent(PKG, ACT, { 'BAD;KEY': 'x' })).toBe(false);
    expect(launchIntent(PKG, ACT, { 'BAD=KEY': 'x' })).toBe(false);
