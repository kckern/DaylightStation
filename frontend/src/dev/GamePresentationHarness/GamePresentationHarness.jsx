...
import InteractiveSceneSurface from '../../lib/presentation/scenes/InteractiveSceneSurface.jsx';
...
import {
  ...
  statesForAssembly,
} from '../../lib/presentation/scenes/interactiveModel.js';
import { PRESENTATION_DPAD } from '../../lib/presentation/scenes/usePresentationInput.js';
import './GamePresentationHarness.scss';
...
function AssetPreview({ catalog, assetId, reducedMotion }) {
  ...
  return <canvas ref={canvasRef} className="game-presentation-harness__asset-canvas" role="img" aria-label={`${assetId} sprite preview`} />;
}

function Toggle({ checked, onChange, children }) {
  return <label className="game-presentation-harness__toggle"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} /><span>{children}</span></label>;
}

export default function GamePresentationHarness({ clear = null }) {
  useDocumentTitle('Game Presentation Harness');
  ...
  if (loading && !catalog) return <main className="game-presentation-harness game-presentation-harness--loading"><div className="game-presentation-harness__loader" role="status">Loading mounted game catalog…</div></main>;
  if (error && !catalog) return <main className="game-presentation-harness game-presentation-harness--loading"><div className="game-presentation-harness__fatal" role="alert"><h1>Game framework unavailable</h1><p>{error.message}</p><button type="button" onClick={() => window.location.reload()}>Retry</button></div></main>;

  return (
    <main className="game-presentation-harness">
      <header className="game-presentation-harness__header">
        <div><p className="game-presentation-harness__eyebrow">Interactive presentation laboratory</p><h1>Game Presentation Harness</h1><p>One runtime · {sceneIndex.length} environments · {coverage?.assets ?? 0} reviewed assets</p></div>
        <div className="game-presentation-harness__header-actions"><span className={`game-presentation-harness__status ${error ? 'is-error' : ''}`}>{error ? error.message : loading ? 'Loading scene…' : 'Runtime ready'}</span>{clear && <button type="button" onClick={clear}>Close</button>}</div>
      </header>

      <section className="game-presentation-harness__scene-nav" aria-label="Environment selection">
        ...
        <div className="game-presentation-harness__scene-pips" aria-hidden="true">{sceneIndex.map((entry) => <span key={entry.id} className={entry.id === sceneId ? 'is-active' : ''} />)}</div>
      </section>

      <div className="game-presentation-harness__workspace">
        <section className="game-presentation-harness__stage-card">
          <div className="game-presentation-harness__stage-toolbar">
            <div><strong>{titleCase(sceneId)}</strong><span>{scene?.style_profile} · {scene?.logical_size?.join('×')} logical px</span></div>
            <div className="game-presentation-harness__toggles"><Toggle checked={!paused} onChange={(value) => setPaused(!value)}>Running</Toggle><Toggle checked={animateWorld} onChange={setAnimateWorld}>World motion</Toggle><Toggle checked={showGrid} onChange={setShowGrid}>Grid</Toggle><Toggle checked={reducedMotion} onChange={setReducedMotion}>Reduced motion</Toggle></div>
          </div>
          <div className="game-presentation-harness__stage">
            ...
            <div className="game-presentation-harness__stage-hint">Move near a chest, sign, or mechanism and press A to interact. Click any sprite to inspect it.</div>
          </div>

          <div className="game-presentation-harness__controls">
            <div className="game-presentation-harness__dpad" aria-label="Directional controls">{PRESENTATION_DPAD.map(({ action, label, glyph }) => <button key={action} type="button" className={`game-presentation-harness__dpad-${action.split('.')[1]}`} aria-label={label} onPointerDown={(event) => { event.currentTarget.setPointerCapture?.(event.pointerId); dispatchTouch(action, 'press'); }} onPointerUp={() => dispatchTouch(action, 'release')} onPointerCancel={() => dispatchTouch(action, 'release')}>{glyph}</button>)}</div>
            <div className="game-presentation-harness__action-pad"><button type="button" className="game-presentation-harness__button-b" onPointerDown={() => dispatchTouch('action.secondary', 'press')}>B<small>Next action</small></button><button type="button" className="game-presentation-harness__button-a" onPointerDown={() => dispatchTouch('action.primary', 'press')}>A<small>Interact</small></button></div>
            <div className="game-presentation-harness__keyboard-help"><kbd>WASD</kbd><span>or arrows to move</span><kbd>Space</kbd><span>interact</span><kbd>Shift</kbd><span>next action</span><kbd>P</kbd><span>pause</span></div>
          </div>
        </section>

        <aside className="game-presentation-harness__runtime-panel">
          <section><h2>Playable actor</h2><label>Actor / rig<select value={actor?.id ?? ''} onChange={(event) => { setActorId(event.target.value); setAssemblyId(''); setActorState('idle'); }}>{actors.map((choice) => <option key={choice.id} value={choice.id}>{titleCase(choice.label)}</option>)}</select></label>
            ...
            <div className="game-presentation-harness__layer-list"><span>{Object.keys(actorEquipment).length ? `${Object.keys(actorEquipment).length} equipped semantic layers` : 'Base layer only'}</span>{Object.keys(actorEquipment).map((slot) => <code key={slot}>{slot}</code>)}</div>
          </section>
