...
function applyTerminalResult(state, command, definition) {
  ...
  const passGatedFailure = card.resolution?.requires_pass && result.verdict?.passed !== true;
  if (!passGatedFailure) {
  ...
  if (isTactical(definition)) {
    if (!passGatedFailure) return success(next, events);
    const continuation = endTurn(next, definition);
    return success(continuation.state, [...events, ...continuation.events]);
  }
