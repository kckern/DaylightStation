...
const tacticalDefinition = {
  ...
  card_battle: {
    ...
    deck: ['gated', 'jab', 'guard', 'focus'],
  },
  cards: {
    jab: {
      title: 'Jab', type: 'attack', cost: 1, damage: 2,
      challenge: { domain: 'assessment', kind: 'sequence', pool: 'exercises' },
    ...
    gated: {
      title: 'Gated', type: 'attack', cost: 1, damage: 9,
      challenge: { domain: 'assessment', kind: 'timed-sequence', pool: 'exercises' },
      resolution: { effect: 'score', requires_pass: true, failure: 'consume-no-effect' },
      outcomes: [{ id: 'full', min_score: 1, multiplier: 1 }, { id: 'miss', min_score: 0, multiplier: 0 }],
    },
  },
};
...
function resolveCard(state, definition, definitionId, result, id) {
  const card = state.zones.hand.find((item) => item.definition_id === definitionId);
  let outcome = transition(state, command('choose_action', 0, { card_instance_id: card.instance_id }, `${id}-choose`), definition);
  const challengeId = outcome.state.pending_action.id;
  outcome = transition(outcome.state, command('prepare_challenge', 1, { challenge_id: challengeId, prepared: { id } }, `${id}-prepare`), definition);
  outcome = transition(outcome.state, command('start_challenge', 2, { challenge_id: challengeId }, `${id}-start`), definition);
  return transition(outcome.state, command('submit_challenge_result', 3, { challenge_id: challengeId, result }, `${id}-result`), definition);
}

describe('gaming card-slice reducer', () => {
  it('exposes legal actions without requiring the view to duplicate rules', () => {
    const state = createInitialState(cardBattleFixture, { seed: 7, participants: [{ user_id: 'guest' }] });
    const interaction = deriveInteraction(state, cardBattleFixture, 'guest');
  ...
  it('persists each recoverable challenge lifecycle boundary', () => {
    let state = createInitialState(cardBattleFixture, { seed: 7, participants: [{ user_id: 'guest' }] });
    const card = state.zones.hand[0];
    let result = transition(state, command('choose_action', 0, { card_instance_id: card.instance_id }, 'choose'), cardBattleFixture);
    ...
    result = transition(result.state, command('start_challenge', 2, { challenge_id: challengeId }), cardBattleFixture);
  ...
  it('refunds an interrupted challenge without consuming the card', () => {
    const initial = createInitialState(cardBattleFixture, { seed: 7, participants: [{ user_id: 'guest' }] });
    const card = initial.zones.hand[0];
    let result = transition(initial, command('choose_action', 0, { card_instance_id: card.instance_id }, 'choose'), cardBattleFixture);
    const challengeId = result.state.pending_action.id;
    result = transition(result.state, command('prepare_challenge', 1, { challenge_id: challengeId, prepared: { id: 'p' } }), cardBattleFixture);
    result = transition(result.state, command('start_challenge', 2, { challenge_id: challengeId }), cardBattleFixture);
    result = transition(result.state, command('submit_challenge_result', 3, {
      challenge_id: challengeId,
  ...
  it('authoritatively abandons a session and interrupts any pending challenge', () => {
    const initial = createInitialState(cardBattleFixture, { seed: 7, participants: [{ user_id: 'guest' }] });
    const card = initial.zones.hand[0];
    const chosen = transition(initial, command('choose_action', 0, { card_instance_id: card.instance_id }, 'choose-abandon'), cardBattleFixture);
    const result = transition(chosen.state, command('abandon_session', 1, { reason: 'player_closed' }, 'abandon'), cardBattleFixture);
  ...
  it('selects challenge content from the pool after the tactical card is chosen', () => {
    ...
    expect(result.state.pending_action.request.context.challenge_pool).toBe('exercises');
  ...
  it('consumes a failed pass-gated tactical card with no effect and continues the enemy turn', () => {
    const initial = createInitialState(tacticalDefinition, { seed: 7, participants: [{ user_id: 'guest' }] });
    const enemyHealth = initial.enemy.health;
    const result = resolveCard(initial, tacticalDefinition, 'gated', {
      status: 'completed', score: 0.9, metrics: {}, provider_version: 'test', attempt_id: 'gated-fail',
      verdict: { score: 0.9, passed: false, failed_criteria: ['placement'], failed_gates: [] },
    }, 'gated-fail');
    expect(result.state.enemy.health).toBe(enemyHealth);
    expect(result.state.player.health).toBe(initial.player.health - 4);
    expect(result.state.turn).toBe(2);
    expect(result.state.player.energy).toBe(result.state.player.max_energy);
    expect(result.events).toContainEqual(expect.objectContaining({
      type: 'challenge_resolved', passed: false, failed_criteria: ['placement'],
    }));
    expect(result.events).not.toContainEqual(expect.objectContaining({ type: 'damage_dealt', target: 'enemy' }));
  });

  it('continues retaliation after a failed pass-gated single-card move', () => {
    const definition = structuredClone(cardBattleFixture);
    const initial = createInitialState(definition, { seed: 7, participants: [{ user_id: 'guest' }] });
    const cardId = initial.zones.hand[0].definition_id;
    definition.cards[cardId].resolution = { effect: 'score', requires_pass: true, failure: 'consume-no-effect' };
    const result = resolveCard(initial, definition, cardId, {
      status: 'completed', score: 0.9, metrics: {}, provider_version: 'test', attempt_id: 'single-fail',
      verdict: { score: 0.9, passed: false, failed_criteria: [], failed_gates: ['pace'] },
    }, 'single-fail');
    expect(result.state.enemy.health).toBe(initial.enemy.health);
    expect(result.state.player.health).toBe(initial.player.health - definition.card_battle.enemy.attack);
    expect(result.state.turn).toBe(2);
    expect(result.events).toContainEqual(expect.objectContaining({ type: 'challenge_resolved', failed_gates: ['pace'] }));
  });
});
