...
function validateResolution(owner, resolution, errors) {
  ...
  if (resolution.requires_pass !== true) errors.push(`${owner} resolution.requires_pass must be true`);
  if (resolution.failure !== 'consume-no-effect') errors.push(`${owner} resolution.failure must be consume-no-effect`);
}
...
export function validateDefinition(definition) {
  ...
  const cardBattle = definition.view_id === 'card-battle-v1' && definition.ruleset === 'card-battle-v1';
  if (!cardBattle) errors.push('view_id and ruleset must both be card-battle-v1');
  if (!cardBattle) return { valid: errors.length === 0, errors };
