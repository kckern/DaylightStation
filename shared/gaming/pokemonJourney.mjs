...
function makeEnemy(definition, encounterIndex, rngState) {
  const opponent = definition.journey.opponents[encounterIndex];
...
export function computePokemonJourneyScore(state, definition) {
  ...
  const firstPassRate = attempts.length > 0
    ...
    : 0;
  const opponentRate = (state.completed_encounters?.length || 0) / definition.journey.opponents.length;
  ...
  const qualified = state.completed_encounters.length === definition.journey.opponents.length
...
export function createPokemonJourneyInitialState(definition, { seed = 1, participants = [], setup = {} } = {}) {
  ...
  const enemy = makeEnemy(definition, 0, Number(seed) >>> 0);
  return {
    schema_version: 1,
    ...
    enemy,
    zones: { deck: [], hand: moveInstances(definition, partner.id), discard: [] },
    pending_action: null,
    practice_attempts: [],
...
function resolvePractice(state, command, definition) {
  ...
  if (next.enemy.health === 0) {
    ...
    if (next.completed_encounters.length === definition.journey.opponents.length) {
...
function continueEncounter(state, definition) {
  ...
  next.player.block = 0;
  next.player.focus = 0;
  next.enemy = makeEnemy(definition, next.current_encounter, next.rng_state);
