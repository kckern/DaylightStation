...
describe('topic builders', () => {
  it('builds per-device topics with the deviceId suffix', () => {
    ...
    expect(ARCADE_SESSION_TOPIC('tv-living-room')).toBe('arcade-session:tv-living-room');
  });
  ...
  it('exposes the broadcast topic as a constant', () => {
    ...
    expect(ARCADE_SESSIONS_TOPIC).toBe('arcade-game-sessions');
  });
  it('parses a per-device topic back into { kind, deviceId }', () => {
    ...
    expect(parseDeviceTopic('arcade-session:tv-1')).toEqual({ kind: 'arcade-session', deviceId: 'tv-1' });
