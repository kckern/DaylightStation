 ...
 * 
 * Express handlers for direct API inputs (UPC, image URL).
 * These bypass Telegram and allow programmatic food logging.
 */

import { createLogger } from '../../_lib/logging/index.mjs';

const logger = createLogger({ source: 'handler', app: 'nutribot-direct' });

/**
 * Get default user/chat config for direct API calls
 * @param {Object} container
 * @param {Object} body - Request body with optional user_id, chat_id, bot_id
 * @returns {{ userId: string, conversationId: string, chatId: string }}
 */
function resolveUserContext(container, body) {
  const config = container.getConfig?.() || {};
  
  // Support legacy format: bot_id + user_id
  const botId = body.bot_id || config.telegram?.botId || process.env.NUTRIBOT_TELEGRAM_BOT_ID || '6898194425';
  const userId = body.user_id || config.users?.defaultUserId || '575596036';
  ...
  const chatId = body.chat_id || `b${botId}_u${userId}`;
 ...
 */
export function directUPCHandler(container) {
  return async (req, res) => {
    ...
    try {
      ...
      const { userId, conversationId, chatId } = resolveUserContext(container, req.body);
 ...
 * @returns {Function} Express handler
 */
export function directImageHandler(container) {
  return async (req, res) => {
 ...
 * @returns {Function} Express handler
 */
export function directTextHandler(container) {
  return async (req, res) => {
