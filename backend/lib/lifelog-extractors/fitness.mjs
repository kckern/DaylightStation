...
export const fitnessExtractor = {
  ...
  summarize(entry) {
    ...
    if (entry.steps) {
      const hr = entry.stepsAvgHR ? `, avg HR ${entry.stepsAvgHR}, max HR ${entry.stepsMaxHR}` : '';
    ...
    if (entry.activities?.length) {
      ...
      entry.activities.forEach(a => {
        ...
        lines.push(`    - ${a.startTime}-${a.endTime}: ${a.title} - ${a.duration} minutes, ${a.calories} calories${hr}`);
      });
    }
