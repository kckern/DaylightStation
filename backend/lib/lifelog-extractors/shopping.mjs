...
export const shoppingExtractor = {
  source: 'shopping',
  category: 'spending',
  filename: 'shopping',
  ...
  extractForDate(data, date) {
    ...
    const totalItems = dayReceipts.reduce((sum, r) => sum + (r.items?.length || 0), 0);
  ...
  summarize(entry) {
    ...
    // Summary line
    const merchantText = entry.merchantCount === 1 
      ? entry.receipts[0].merchant
      : `${entry.merchantCount} retailers`;
    lines.push(`  ${entry.receipts.length} order${entry.receipts.length > 1 ? 's' : ''} from ${merchantText}`);
    ...
    if (entry.totalItems > 0) {
      lines.push(`  ${entry.totalItems} item${entry.totalItems > 1 ? 's' : ''} purchased`);
    }
    
    // Detail each order
    entry.receipts.forEach(receipt => {
      const total = receipt.total ? `$${receipt.total.toFixed(2)}` : 'unknown total';
      lines.push(`  - ${receipt.merchant}: ${total} (${receipt.itemCount} item${receipt.itemCount !== 1 ? 's' : ''})`);
