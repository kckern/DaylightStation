 ...
 * @module infrastructure/ai/OpenAIGateway
 */

import axios from 'axios';
import { ExternalServiceError, RateLimitError, TimeoutError } from '../../_lib/errors/index.mjs';
...
export class OpenAIGateway {
  ...
  constructor(config, options = {}) {
    ...
    this.#logger = options.logger || createLogger({ source: 'openai', app: 'gateway' });
  ...
  async #callApi(endpoint, data, options = {}) {
    ...
    if (this.#rateLimiter) {
      ...
      if (!canProceed) {
        throw new RateLimitError('OpenAI', 60, { endpoint });
      }
    }
    ...
    try {
      ...
      if (error.response?.status === 429) {
        ...
        this.#logger.warn('openai.api.rateLimit', { endpoint, retryAfter });
        throw new RateLimitError('OpenAI', retryAfter, { endpoint });
      }

      // Handle timeout
      if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
        this.#logger.error('openai.api.timeout', { endpoint, timeout: this.#timeout });
        throw new TimeoutError('OpenAI API call', this.#timeout, { endpoint });
      }

      // Handle other API errors
      const message = error.response?.data?.error?.message || error.message;
      this.#logger.error('openai.api.error', { endpoint, error: message });
  ...
  async chatWithJson(messages, options = {}) {
    ...
    try {
      ...
      try {
        ...
        throw new ExternalServiceError('OpenAI', 'Failed to parse JSON response after retry', {
  ...
  async transcribe(audioBuffer, options = {}) {
    ...
    try {
      ...
      const message = error.response?.data?.error?.message || error.message;
      this.#logger.error('openai.transcribe.error', { error: message });
      throw new ExternalServiceError('OpenAI', `Transcription failed: ${message}`);
    }
  }
