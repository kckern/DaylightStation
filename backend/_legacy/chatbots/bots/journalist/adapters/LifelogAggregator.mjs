 ...
 * @module journalist/adapters/LifelogAggregator
 * 
 * Aggregates yesterday's data from all harvested sources using
 * source-specific extractors that know how to parse each format.
 */

import { userLoadFile } from '../../../../lib/io.mjs';
import moment from 'moment-timezone';
import { extractors } from '../../../../lib/lifelog-extractors/index.mjs';
...
export class LifelogAggregator {
  ...
  /**
   * Aggregate yesterday's lifelog data for a user
   * 
  ...
  #loadSource(username, filename) {
    try {
      const data = userLoadFile(username, filename);
      return data || null;
    } catch (error) {
