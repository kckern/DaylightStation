...
export default async (job_id) => {
    ...
    const calendarItems = calendarEvents.map(event => {
        const { start, end, summary, description, location,  organizer: {displayName: calendarName} } = event;
        const domain = location && location.match(/https?:\/\/([^\/]+)/) ? location.match(/https?:\/\/([^\/]+)/)[1] : null;
        const allday = !!(start.date && !start.dateTime);
        return { start: start.dateTime || start.date, end: end.dateTime || end.date, duration: (new Date(end.dateTime || end.date) - new Date(start.dateTime || start.date)) / 1000 / 60 / 60, summary, description, type: 'calendar', calendarName, location, domain, allday };
    ...
    const todoistItems = todoItems.map(item => {
        ...
        return { start: due ? new Date(due).toISOString() : null, summary: extractedContent, description, type: 'todoist', domain, url: extractedUrl };
    });
