...
export class MediaJudge {
  ...
  async pick({ query, candidates }) {
    ...
    try {
      ...
      this.#logger.warn?.('brain.media.judge.timeout_or_error', { error: err.message });
    ...
    if (!parsed || typeof parsed.index !== 'number') {
      this.#logger.warn?.('brain.media.judge.parse_failed', { raw: String(raw).slice(0, 200) });
