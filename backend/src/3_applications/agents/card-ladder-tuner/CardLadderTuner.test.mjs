...
import { CardLadderTuner, TUNING_SCHEMA, validateTuning, systemPrompt } from './index.mjs';
import { TUNABLE, GROWN_UP_SETTINGS } from '#domains/school/cardLadder/index.mjs';
...
describe('CardLadderTuner', () => {
  it('runs one tool-less structured step over the digest and returns the proposal', async () => {
    ...
    expect(CardLadderTuner.id).toBe('card-ladder-tuner');
  });

  it('accepts `runtime` as the dep name, like the nutrition auditor', async () => {
    const tuner = new CardLadderTuner({ runtime: fakeRuntime(good), logger: quietLogger });
  ...
  it.each([
    ...
    const tuner = new CardLadderTuner({ agentRuntime: fakeRuntime(structured), logger: quietLogger });
  ...
  it('the system prompt names every tunable, the brakes and the grown-up-only settings', () => {
    ...
    expect(new CardLadderTuner({ agentRuntime: fakeRuntime(good) }).getSystemPrompt()).toBe(systemPrompt);
  });
});
