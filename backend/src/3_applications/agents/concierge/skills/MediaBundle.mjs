 ...
 * to the concierge agent. The actual orchestration (search → judge → resolve →
 * play) lives in PlayMediaUseCase. This bundle owns:
 ...
 * query keys, ranking field names) come from the operator's concierge.yml.
 * This file names no specific content sources — defaults are vendor-neutral.
 *
 * Migrated from MediaSkill (Task 6). Old MediaSkill.mjs remains until Task 10.
 */
...
- If nothing matches, decline politely; do not invent titles.`;
  ...
  createTools() {
    ...
    const nameAliases = this.#nameAliases;
    ...
    return [
      {
        ...
        execute: async ({ query, media_class }, ctx) => {
          ...
          let normalisedQuery = applyPrefix(query, media_class, config.prefix_aliases);
          const aliased = nameAliases.lookup(normalisedQuery);
          ...
          log.info?.('concierge.skill.media.tool_complete', {
...
async function voiceSearch(cq, text, cfg) {
  // Build the query verbatim from operator config. The concierge names no
  // specific source or library — operators populate concierge.yml.media.
  const query = {
    text,
