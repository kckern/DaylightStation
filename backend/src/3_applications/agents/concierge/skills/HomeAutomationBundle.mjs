    ...
    return [
      {
        ...
        async execute({ name, action }) {
          ...
          if (!resolved.entityId) {
            log.warn?.('concierge.skill.ha.resolve_failed', { friendly_name: name, candidates: resolved.candidates ?? [] });
          ...
          log.info?.('concierge.skill.ha.action', { tool: 'ha_toggle_entity', entity_id: resolved.entityId, ok: !!result?.ok });
      ...
      },
      {
        ...
        async execute({ name }) {
          ...
          log.info?.('concierge.skill.ha.action', { tool: 'ha_activate_scene', entity_id: resolved.entityId, ok: !!result?.ok });
      ...
      },
      {
        ...
        async execute({ name }) {
          ...
          log.info?.('concierge.skill.ha.action', { tool: 'ha_run_script', entity_id: resolved.entityId, ok: !!result?.ok });
