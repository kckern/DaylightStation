  ...
  async execute({ query, satellite, contentType = 'music' }) {
    ...
    if ((search.items?.length ?? 0) === 0) {
      ...
      if (simplified && simplified !== query) {
        ...
        log.info?.('concierge.skill.media.search_retry', {
    ...
    if (items.length > 0 && !items.some(isContainerCandidate)) {
      ...
      if (split && split !== query) {
        ...
        if (extraItems.length > 0) {
          ...
          log.info?.('concierge.skill.media.compound_split', {
    ...
    if (items.length === 0) {
      log.warn?.('concierge.skill.media.no_match', { query, attempts });
    ...
    if (items.length > 1 && this.#judge) {
      try {
        ...
        if (judgement?.index >= 0 && judgement.index < candidates.length) {
          ...
          log.info?.('concierge.skill.media.judge', {
            query, picked_id: candidates[pickIndex].id,
            reason: judgement.reason, latencyMs: judgement.latencyMs,
          });
        }
      } catch (err) {
        log.warn?.('concierge.skill.media.judge_failed', { error: err.message });
      }
    }
    ...
    if (!played) {
      log.warn?.('concierge.skill.media.all_resolve_failed', {
    ...
    log.info?.('concierge.skill.media.play', {
      content_id: top.id,
