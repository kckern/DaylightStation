    ...
    return [
      {
        ...
        async execute() {
          ...
          log.info?.('concierge.skill.finance.balances', { count: accounts.length });
          return { accounts };
        },
      },
      {
        ...
        async execute({ days = 7, account, tag, limit = 25 }) {
          ...
          log.info?.('concierge.skill.finance.transactions', { days, count: tx.length });
      ...
      },
      {
        ...
        async execute({ period_start }) {
          ...
          log.info?.('concierge.skill.finance.budget', { periodStart: summary.asOf });
          return summary;
        },
      },
    ];
