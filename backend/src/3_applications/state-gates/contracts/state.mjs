...
import { PeriodRef, PolicyGraph, SubjectRef, instanceKey } from '#domains/state-gates/index.mjs';
...
export function enumerateInstances({ graph, snapshot, subjects, now, timezone }) {
  ...
  for (const gate of graph.gates.values()) {
    for (const pair of pairs.values()) {
      if (gate.subjectKinds.includes(pair.subject.kind) && gate.periodKinds.includes(pair.period.kind)) {
        instances.push({ gateId: gate.id, ...pair, key: instanceKey(gate.id, pair.subject, pair.period) });
      }
    }
  }
  return instances;
