...
test('counts every completed eligible board game, regardless of result', () => {
  ...
  ]) service.record({ learnerId: 'milo', gameId, gameSessionId, completed: true, result });
  assert.equal(service.current('milo').completedGames, 3);
});

test('is idempotent by learner and game session, and ignores abandoned/ineligible games', () => {
  const service = fixture();
  const record = { learnerId: 'felix', gameId: 'chess', gameSessionId: 'same', completed: true, result: 'loss' };
  assert.equal(service.record(record).counted, true);
  assert.equal(service.record(record).duplicate, true);
  service.record({ learnerId: 'felix', gameId: 'checkers', gameSessionId: 'abandoned', completed: false, result: 'loss' });
  service.record({ learnerId: 'felix', gameId: 'solitaire', gameSessionId: 'other', completed: true, result: 'win' });
  assert.equal(service.current('felix').completedGames, 1);
});
...
test('uses the household four-AM study-day boundary', () => {
  ...
  assert.equal(service.current('soren').studyDate, '2026-08-27');
});
