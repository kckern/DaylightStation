...
import { LibbyBootstrapService } from './LibbyBootstrapService.mjs';
...
describe('LibbyBootstrapService', () => {
  it('maps the structural gateway port to an opened immutable spine', async () => {
    const bootstrapLoan = vi.fn(async () => spine);
    const signal = new AbortController().signal;
    const result = await new LibbyBootstrapService({ bootstrapGateway: { bootstrapLoan } }).open(input, { signal });
  ...
  it.each([['BOOTSTRAP_TIMEOUT', 'timeout'], ['BOOTSTRAP_BUSY', 'busy'], ['BOOTSTRAP_ABORTED', 'aborted'],
    ...
    const service = new LibbyBootstrapService({ bootstrapGateway: { bootstrapLoan: async () => { throw Object.assign(new Error('secret'), { code }); } } });
  ...
  it('rejects extra gateway capabilities even for a structurally valid port', async () => {
    const service = new LibbyBootstrapService({ bootstrapGateway: { bootstrapLoan: async () => ({ ...spine, storage: 'secret' }) } });
  ...
  it('never forwards an unrelated input field', async () => {
    ...
    const service = new LibbyBootstrapService({ bootstrapGateway: { bootstrapLoan } });
