 ...
 */
export class ReconcileArcadeGameSessions {
  ...
  constructor({ sessions, logReader, resolveContent = null, matchToleranceMs = 120_000, logger = console }) {
    if (!sessions) throw new Error('ReconcileArcadeGameSessions requires a sessions repository');
    if (!logReader?.listRecentSessions) throw new Error('ReconcileArcadeGameSessions requires a logReader');
  ...
  async execute({ deviceId, since }) {
    ...
    for (const entry of onDevice) {
      ...
      if (!match) {
        ...
        this.#logger.warn?.('arcade.session.unrecorded', {
      ...
      if (!match.content?.contentId && entry.contentPath) {
        ...
        if (content) {
          ...
          this.#logger.info?.('arcade.session.attribution_repaired', {
  ...
  #resolve(contentPath) {
    ...
    try {
      ...
      this.#logger.debug?.('arcade.reconcile.resolve_failed', { contentPath, error: error.message });
      return null;
    }
  }
}

export default ReconcileArcadeGameSessions;
