...
import { ReconcileArcadeGameSessions } from './ReconcileArcadeGameSessions.mjs';
...
const recordedSession = (id, startSec, content = { contentId: 'game:a' }) => {
  const s = ArcadeGameSession.open({ id, deviceId: 'tv', surface: 'console-emulator', content, trustedGapMs: 25_000 });
  s.observe({ state: ArcadeGameSessionState.PLAYING, observedAt: at(startSec) });
  return s;
};
...
const build = (over = {}) => new ReconcileArcadeGameSessions({
  sessions, logReader,
...
describe('ReconcileArcadeGameSessions', () => {
  ...
  it('reports a session the meter never saw — loudly, and without billing it', async () => {
    ...
    expect(warns.map((w) => w[0])).toContain('arcade.session.unrecorded');
  });
  ...
  it('persists device-log evidence so the same missed session warns only once across restarts', async () => {
    ...
    expect(warns.filter(([event]) => event === 'arcade.session.unrecorded')).toHaveLength(1);
  });
