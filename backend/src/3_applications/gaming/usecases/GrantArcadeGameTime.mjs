 ...
 */
export class GrantArcadeGameTime {
  #ledger; #today; #logger;

  constructor({ ledger, today = () => new Date().toISOString().slice(0, 10), logger = console }) {
    if (!ledger?.append) throw new Error('GrantArcadeGameTime requires a grant ledger');
    this.#ledger = ledger;
    this.#today = today;
    this.#logger = logger;
  }
  ...
  async execute({ userId, minutes, by = null, reason = null, on = null }) {
    if (!userId) throw new Error('GrantArcadeGameTime requires a userId');
    if (!Number.isFinite(minutes) || minutes === 0) {
      throw new Error('GrantArcadeGameTime requires a non-zero number of minutes');
    }
    ...
    this.#logger.info?.('arcade.grant.recorded', {
...
export default GrantArcadeGameTime;
