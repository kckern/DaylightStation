...
import { assessBudget, describeRemaining } from '#domains/gaming/services/arcadeGameBudget.mjs';
 ...
 */
export class EnforceArcadeGameBudget extends IArcadeGameSessionAnnouncer {
  ...
  constructor({ grants, terminator, speaker = null, notify = null, sessions = null, announcer = null, logger = console }) {
    super();
    if (!grants?.forSession) throw new Error('EnforceArcadeGameBudget requires a grants port');
    if (!terminator?.endPlay) throw new Error('EnforceArcadeGameBudget requires a terminator port');
    this.#grants = grants;
  ...
  async progress(session) {
    ...
    try {
      ...
      this.#logger.warn?.('arcade.budget.grant_unavailable', { sessionId: session.id, error: error.message });
      return;
    }
    if (!grant) return;
  ...
  async #warn(session, assessment) {
    ...
    this.#logger.info?.('arcade.budget.warning', {
  ...
  async #expire(session) {
    this.#logger.warn?.('arcade.budget.expired', {
    ...
    if (!result?.ok) {
      ...
      this.#logger.error?.('arcade.budget.terminate_failed', {
    ...
    if (this.#sessions && !session.isEnded()) {
      session.end({ endedAt: new Date().toISOString(), reason: ArcadeGameSessionEndReason.EXPIRED });
  ...
  async #safely(what, fn) {
    try { await fn?.(); } catch (error) {
      this.#logger.warn?.(`arcade.budget.${what}_failed`, { error: error.message });
    }
  }
}

export default EnforceArcadeGameBudget;
