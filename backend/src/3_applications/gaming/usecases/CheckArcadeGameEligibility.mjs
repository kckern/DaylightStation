import { assessEligibility } from '#domains/gaming/services/arcadeGameEligibility.mjs';
 ...
 */
export class CheckArcadeGameEligibility {
  ...
  async execute({ userId = null, deviceId = null, contentId = null }) {
    ...
    if (!result.allowed) {
      this.#logger.info?.('arcade.eligibility.refused', { userId, deviceId, contentId, reasons: result.reasons });
    }
    return result;
  }
  ...
  async #safely(what, fn, fallback) {
    try {
      ...
      this.#logger.warn?.('arcade.eligibility.input_failed', { input: what, error: error.message });
      return fallback;
    }
  }
}

export default CheckArcadeGameEligibility;
