 ...
 */
export class ReconcileOpenArcadeGameSessions {
  #sessions; #announcer; #logger;

  constructor({ sessions, announcer = null, logger = console }) {
    if (!sessions) throw new Error('ReconcileOpenArcadeGameSessions requires a sessions repository');
  ...
  async execute({ deviceIds = [], now, staleAfterMs }) {
    ...
    try {
      ...
      this.#logger.warn?.('arcade.reconcile.discovery_failed', { error: error.message });
    }
    ...
    for (const deviceId of allDeviceIds) {
      ...
      try {
        ...
        this.#logger.warn?.('arcade.reconcile.read_failed', { deviceId, error: error.message });
        continue;
      }
      if (!session) continue;
      ...
      if (Number.isFinite(age) && age <= staleAfterMs) {
        ...
        this.#logger.info?.('arcade.session.resumed', {
          sessionId: session.id, deviceId, playedMs: session.playedMs, ageMs: age,
        });
        continue;
      }

      session.end({ endedAt: now, reason: ArcadeGameSessionEndReason.LOST });
  ...
  async #announce(session) {
    ...
    try {
      ...
      this.#logger.warn?.('arcade.session.announce_failed', { kind: 'ended', sessionId: session.id, error: error.message });
    }
  }
}

export default ReconcileOpenArcadeGameSessions;
