...
import { GroupPlayCatalog } from './GroupPlayCatalog.mjs';
...
function makeService({ cfg = HOUSEHOLD_CFG } = {}) {
  const configService = { getHouseholdAppConfig: () => cfg };
  const userService = {
  ...
  const definitions = new Map([
    ...
    ['solo:private', { definition: { experience: { id: 'solo', version: 1 } }, parts: { content: { artifact: {}, title: 'Not Group Play' } } }],
  ...
  const manifests = new Map([
    ['quiz@1', { id: 'quiz', version: 1, native_surface_id: 'group-play', presenters: { primary: 'quiz-board' }, setup: { kind: 'teams' } }],
    ['drawing@2', { id: 'drawing', version: 2, native_surface_id: 'group-play', presenters: { primary: 'drawing-stage' }, setup: { kind: 'individuals-or-teams' } }],
    ['solo@1', { id: 'solo', version: 1, native_surface_id: 'solo-screen', presenters: { primary: 'solo-view' } }],
  ]);
  const manifestStore = { get: (id, version) => manifests.get(`${id}@${version}`) || null };
  return new GroupPlayCatalog({ configService, userService, definitionStore, manifestStore, logger: NOOP });
}
