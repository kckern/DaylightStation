...
import { ReconcileOpenArcadeGameSessions } from './ReconcileOpenArcadeGameSessions.mjs';
...
class FakeSessions {
  ...
  seed(deviceId, lastObservedSec) {
    const s = ArcadeGameSession.open({ id: `ps_${deviceId}`, deviceId, surface: 'console-emulator', content: { contentId: 'g' }, trustedGapMs: 25_000 });
    s.observe({ state: ArcadeGameSessionState.PLAYING, observedAt: at(0) });
    s.observe({ state: ArcadeGameSessionState.PLAYING, observedAt: at(lastObservedSec) });
...
beforeEach(() => {
  ...
  useCase = new ReconcileOpenArcadeGameSessions({ sessions, logger: quiet });
});

describe('ReconcileOpenArcadeGameSessions', () => {
  ...
  it('announces the ending so consumers see the settlement', async () => {
    const calls = [];
    const uc = new ReconcileOpenArcadeGameSessions({ sessions, announcer: { ended: async (s) => calls.push(s.id) }, logger: quiet });
  ...
  it('a failing announcer never blocks the settlement', async () => {
    const uc = new ReconcileOpenArcadeGameSessions({ sessions, announcer: { ended: async () => { throw new Error('bus down'); } }, logger: quiet });
    const s = sessions.seed('livingroom-tv', 20);
    await uc.execute({ deviceIds: ['livingroom-tv'], now: at(3600), staleAfterMs: 60_000 });
    expect(s.isEnded()).toBe(true);
  });
