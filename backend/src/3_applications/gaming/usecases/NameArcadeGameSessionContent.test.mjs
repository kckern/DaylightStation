...
import { NameArcadeGameSessionContent } from './NameArcadeGameSessionContent.mjs';
...
function openSession({ content = null, startedAt = '2026-09-11T18:00:00.000Z' } = {}) {
  const session = ArcadeGameSession.open({
...
function build({ entries, saved = [], resolve = (p) => CATALOG[p] || null } = {}) {
  return new NameArcadeGameSessionContent({
...
describe('NameArcadeGameSessionContent', () => {
  ...
  it('does not name a session that has not started', async () => {
    const pending = ArcadeGameSession.open({ id: 's2', deviceId: 'tv', surface: 'console', trustedGapMs: TRUSTED_GAP_MS });
  ...
  it('does nothing without a log source', async () => {
    const useCase = new NameArcadeGameSessionContent({ sessions: {}, logReader: null, resolveContent: null, logger: quiet });
