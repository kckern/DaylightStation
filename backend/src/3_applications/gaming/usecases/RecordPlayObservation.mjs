...
export class RecordPlayObservation {
  ...
  async execute({ deviceId, surface, userId = null, grantRef = null, observation }) {
    ...
    if (session && (loaded === false || loadChanged)) {
      session.end({
        endedAt: observedAt,
        reason: loadChanged ? PlaySessionEndReason.SWITCHED : PlaySessionEndReason.QUIT,
      });
    ...
    if (!session) {
      ...
      if (state !== PlayState.PLAYING || loaded !== true) return result;
      ...
      if (contentAttribution.attributed || userAttribution.attributed || loadAttribution.attributed) {
        this.#logger.info?.('play.session.attributed', {
