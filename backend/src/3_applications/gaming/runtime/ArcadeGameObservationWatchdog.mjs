 ...
 * nobody can observe.
 */
export class ArcadeGameObservationWatchdog {
  ...
  constructor({
    ...
    if (typeof scheduler?.after !== 'function') throw new Error('ArcadeGameObservationWatchdog requires a scheduler with after()');
    if (typeof now !== 'function') throw new Error('ArcadeGameObservationWatchdog requires an injected now()');
    if (!Number.isFinite(staleAfterMs) || staleAfterMs <= 0) throw new Error('ArcadeGameObservationWatchdog requires a positive staleAfterMs');
    if (!Number.isFinite(intervalMs) || intervalMs <= 0) throw new Error('ArcadeGameObservationWatchdog requires a positive intervalMs');
  ...
  #schedule() {
    ...
    this.#cancel = this.#scheduler.after(this.#intervalMs, () => {
      try { this.check(); } catch (error) {
        this.#logger.error?.('arcade.watchdog.failed', { error: error.message });
      }
      this.#schedule();
    });
  }
    ...
    for (const tracker of this.#trackers) {
      for (const health of tracker.getHealth()) {
        ...
        if (prolonged && !this.#blocked.has(deviceId)) {
          ...
          this.#logger.error?.('arcade.watchdog.blind_escalated', {
            deviceId, ageMs: age,
          ...
          this.#logger.info?.('arcade.watchdog.blind_cleared', { deviceId });
        } else if (prolonged) {
          found.push({ deviceId, condition: 'blind' });
        }
  ...
  async #raise(deviceId, condition, message) {
    ...
    try {
      ...
      this.#logger.warn?.('arcade.watchdog.alert_failed', { deviceId, error: error.message });
    }
  }
  ...
  #edge(deviceId, condition, isActive, level, detail) {
    ...
    if (isActive) {
      this.#logger[level]?.(`arcade.watchdog.${condition}`, { deviceId, ...detail });
    } else {
      this.#logger.info?.(`arcade.watchdog.${condition}_cleared`, { deviceId });
    }
  }
}

export default ArcadeGameObservationWatchdog;
