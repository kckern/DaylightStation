 ...
 */
export class OverlayArcadeGameSessionAnnouncer extends IArcadeGameSessionAnnouncer {
  #overlay; #buildUrl; #logger;

  /**
   * @param {Object} config
   * @param {import('../ports/IArcadeGameOverlay.mjs').IArcadeGameOverlay} config.overlay
   * @param {(deviceId: string) => string} config.buildUrl
   */
  constructor({ overlay, buildUrl, logger = console }) {
    super();
    if (!overlay?.arm) throw new Error('OverlayArcadeGameSessionAnnouncer requires an overlay port');
    if (typeof buildUrl !== 'function') throw new Error('OverlayArcadeGameSessionAnnouncer requires buildUrl()');
  ...
  async disarmIdle(deviceIds = []) {
    for (const deviceId of deviceIds) {
      try {
        ...
        this.#logger.warn?.('arcade.overlay.disarm_failed', { deviceId, error: error.message });
      }
    }
  }
}

export default OverlayArcadeGameSessionAnnouncer;
