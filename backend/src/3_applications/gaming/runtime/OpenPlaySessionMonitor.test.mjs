...
import { OpenPlaySessionMonitor } from './OpenPlaySessionMonitor.mjs';
...
function session(deviceId, lastObservedSeconds) {
  ...
  opened.observe({ state: PlayState.PLAYING, observedAt: at(0) });
  opened.observe({ state: PlayState.PLAYING, observedAt: at(lastObservedSeconds) });
  return opened;
}
...
const build = ({ items = [], now = () => at(100), announcer = null, scheduler = null, logger = quiet } = {}) => {
  ...
  return {
    sessions,
    monitor: new OpenPlaySessionMonitor({
      sessions,
      announcer,
...
describe('OpenPlaySessionMonitor', () => {
  ...
  it('does not overlap sweeps when storage is slow', async () => {
    ...
    const monitor = new OpenPlaySessionMonitor({
