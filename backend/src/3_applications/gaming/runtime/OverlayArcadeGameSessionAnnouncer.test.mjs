...
import { OverlayArcadeGameSessionAnnouncer } from './OverlayArcadeGameSessionAnnouncer.mjs';
...
const build = () => new OverlayArcadeGameSessionAnnouncer({
  overlay, buildUrl: (d) => `https://host/arcade-film.html?device=${d}`, logger: { warn() {} },
});

beforeEach(() => { calls = []; });

describe('OverlayArcadeGameSessionAnnouncer', () => {
  ...
  it('one failing disarm does not stop the rest of the sweep', async () => {
    ...
    const a = new OverlayArcadeGameSessionAnnouncer({ overlay: flaky, buildUrl: () => 'u', logger: { warn() {} } });
    await a.disarmIdle(['bad', 'good']);
    expect(calls).toEqual([['disarm', 'good']]);
  });

  it('requires its dependencies', () => {
    expect(() => new OverlayArcadeGameSessionAnnouncer({ buildUrl: () => '' })).toThrow(/overlay/);
    expect(() => new OverlayArcadeGameSessionAnnouncer({ overlay })).toThrow(/buildUrl/);
  });
});
