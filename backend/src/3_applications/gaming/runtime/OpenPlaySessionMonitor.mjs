 ...
 */
export class OpenPlaySessionMonitor {
  ...
  constructor({
    ...
    if (!sessions?.listOpen || !sessions?.findOpenForDevice || !sessions?.save) {
      throw new Error('OpenPlaySessionMonitor requires a sessions repository');
    }
    if (typeof scheduler?.after !== 'function') {
      throw new Error('OpenPlaySessionMonitor requires a scheduler with after()');
    }
    if (typeof now !== 'function') throw new Error('OpenPlaySessionMonitor requires an injected now()');
    if (!Number.isFinite(staleAfterMs) || staleAfterMs <= 0) {
      throw new Error('OpenPlaySessionMonitor requires a positive staleAfterMs');
    }
    if (!Number.isFinite(intervalMs) || intervalMs <= 0) {
      throw new Error('OpenPlaySessionMonitor requires a positive intervalMs');
    }
  ...
  async #runSweep() {
    ...
    for (const candidate of open) {
      try {
        ...
        session.end({ endedAt: observedNow, reason: PlaySessionEndReason.LOST });
        ...
        this.#logger.warn?.('play.session.stale_settle_failed', {
  ...
  #recordSweepFailure(error) {
    ...
    this.#logger.error?.('play.session.monitor_failed', { error: error.message });
  }
...
export default OpenPlaySessionMonitor;
