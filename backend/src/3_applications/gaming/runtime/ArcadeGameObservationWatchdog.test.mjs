...
import { ArcadeGameObservationWatchdog } from './ArcadeGameObservationWatchdog.mjs';
...
const build = (health, over = {}) => new ArcadeGameObservationWatchdog({
...
describe('ArcadeGameObservationWatchdog — staleness', () => {
  it('raises an error when a tracker stops ticking', () => {
    ...
    expect(logs.find((l) => l[1] === 'arcade.watchdog.stale')[0]).toBe('error');
  });
...
describe('ArcadeGameObservationWatchdog — failing and degraded', () => {
  it('warns once consecutive errors cross the threshold', () => {
    ...
    expect(events()).toContain('arcade.watchdog.failing');
  });
  ...
  it('flags a degraded meter separately from a failing one', () => {
    ...
    expect(events()).toContain('arcade.watchdog.degraded');
  });
  ...
  it('identifies a loaded game that the recorder cannot persist', () => {
    ...
    expect(events()).toContain('arcade.watchdog.unrecordable');
  });
});

describe('ArcadeGameObservationWatchdog — alarms are edge-triggered', () => {
  it('announces a condition once, not on every sweep', () => {
    ...
    expect(events().filter((e) => e === 'arcade.watchdog.stale')).toHaveLength(1);
  });
  ...
  it('announces recovery when the condition clears', () => {
    ...
    expect(w.check()).toEqual([]);
    expect(events()).toContain('arcade.watchdog.stale_cleared');
  });
...
describe('ArcadeGameObservationWatchdog — lifecycle', () => {
  it('requires injected scheduling and clock', () => {
    expect(() => new ArcadeGameObservationWatchdog({ now, staleAfterMs: 1, intervalMs: 1 })).toThrow(/scheduler/);
    expect(() => new ArcadeGameObservationWatchdog({ scheduler: { after() {} }, staleAfterMs: 1, intervalMs: 1 })).toThrow(/now/);
  });
  ...
  it('a throwing tracker does not kill the sweep loop', () => {
    const w = new ArcadeGameObservationWatchdog({
    ...
    w.start();
    expect(events()).toContain('arcade.watchdog.failed');
  });
});

describe('ArcadeGameObservationWatchdog — prolonged blindness escalates to a person', () => {
  ...
  it('marks the device so no NEW play is granted, and never stops a running game', () => {
    ...
    expect(events()).not.toContain('arcade.terminated');
  });
