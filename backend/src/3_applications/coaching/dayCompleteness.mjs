...
export function classifyDay(entry, closure, minCalories = DEFAULT_MIN_CALORIES) {
  ...
  // Like a day closure it outranks a reconstructed estimate: a day backfilled
  // BEFORE a meal was marked skipped keeps the estimate in its total until the
  // backfill is removed (DELETE /nutrition/reconstruction).
