...
function makeFakes() {
  ...
  const publications = {
    locked: payload => eventBus.broadcast('fitness.emergency.locked', payload),
    released: payload => eventBus.broadcast('fitness.emergency.released', payload),
  };
  return { repo, haGateway, haCalls, publications, broadcasts };
}

test('trigger persists state, broadcasts locked, then fires HA script', async () => {
  const { repo, haGateway, haCalls, publications, broadcasts } = makeFakes();
  const uc = new TriggerEmergencyLockdown({ repo, haGateway, publications, scriptId: 'garage_deactivate', defaultDurationSec: 1800, shutdownBufferMs: 0 });
...
test('HA failure compensates by releasing (no "locked but garage running")', async () => {
  const { repo, publications, broadcasts } = makeFakes();
...
test('release clears state and broadcasts released', async () => {
  const { repo, haGateway, publications, broadcasts } = makeFakes();
  await new TriggerEmergencyLockdown({ repo, haGateway, publications, scriptId: 'garage_deactivate', defaultDurationSec: 1800, shutdownBufferMs: 0 }).execute({ lockedBy: 'alice', now: 1000 });
  await new ReleaseEmergencyLockdown({ repo, publications }).execute({ by: 'admin', now: 1500 });
...
test('GetLockdownState returns null and self-clears once expired', async () => {
  const { repo, haGateway, publications } = makeFakes();
  await new TriggerEmergencyLockdown({ repo, haGateway, publications, scriptId: 'garage_deactivate', defaultDurationSec: 100, shutdownBufferMs: 0 }).execute({ lockedBy: 'alice', now: 1000 });
...
test('trigger requires repo, haGateway, publications', () => {
  assert.throws(() => new TriggerEmergencyLockdown({ haGateway: {}, publications: {} }));
  assert.throws(() => new TriggerEmergencyLockdown({ repo: {}, publications: {} }));
