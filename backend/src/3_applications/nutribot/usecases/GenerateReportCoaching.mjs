 ...
 * @module nutribot/usecases/GenerateReportCoaching
 *
...
export class GenerateReportCoaching {
  ...
  constructor(deps) {
    ...
    if (!deps.nutriCoachStore) throw new Error('nutriCoachStore is required');
    if (!deps.config) throw new Error('config is required');

    this.#messagingGateway = deps.messagingGateway;
    this.#aiGateway = deps.aiGateway;
    this.#nutriListStore = deps.nutriListStore;
    this.#nutriCoachStore = deps.nutriCoachStore;
  ...
  async execute(input) {
    ...
    try {
      ...
      const isFirstOfDay = await this.#nutriCoachStore.isFirstOfDay(userId, date);

      // 2. Load context data
      const goals = this.#config.getUserGoals(userId);
      const todayItems = await this.#nutriListStore.findByDate(userId, date);
      ...
      const previousCoaching = await this.#nutriCoachStore.getRecent(userId, 10);

      // 4. Build appropriate prompt
      const prompt = isFirstOfDay ? this.#buildFirstOfDayPrompt(todaySummary, nutriDayHistory, previousCoaching, goals) : this.#buildSubsequentPrompt(todaySummary, todayItems, previousCoaching, goals);
      ...
      await this.#nutriCoachStore.save({
        userId,
        date,
      ...
      const { messageId } = await this.#messagingGateway.sendMessage(conversationId, formattedMessage, {});
  ...
  #calculateSummary(items, goals) {
    ...
    summary.calorieGoal = calorieMax;
...
- Average protein: ${avgProtein}g (goal: ${goals.protein}g)`;
    ...
    if (previousCoaching.length > 0) {
      const recentAdvice = previousCoaching
        .slice(0, 3)
        .map((c) => c.message)
        .join(' | ');
...
Give brief, encouraging feedback on this update.`;
  ...
  #getFallbackMessage(summary, goals) {
    ...
    if (summary.calories < calorieMin) {
      return FALLBACK_MESSAGES.underMin.replace('{calories}', summary.calories).replace('{min}', calorieMin);
    } else if (summary.calories > calorieMax) {
      return FALLBACK_MESSAGES.overGoal.replace('{calories}', summary.calories).replace('{over}', summary.calories - calorieMax).replace('{max}', calorieMax);
    } else {
      return FALLBACK_MESSAGES.inRange.replace('{calories}', summary.calories).replace('{min}', calorieMin).replace('{max}', calorieMax);
    }
  }
