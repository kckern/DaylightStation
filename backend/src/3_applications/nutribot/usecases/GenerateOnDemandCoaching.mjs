 ...
 * @module nutribot/usecases/GenerateOnDemandCoaching
 *
...
export class GenerateOnDemandCoaching {
  ...
  async execute(input) {
    ...
    try {
      ...
      const summary = await this.#foodLogStore.getDailySummary(userId, today);
  ...
  async #loadRecentDays(userId, days) {
    ...
    for (let i = 0; i < days; i++) {
      ...
      const summary = await this.#foodLogStore.getDailySummary(userId, dateStr);
