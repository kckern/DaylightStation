 ...
 * GratitudeService
 *
 * Application service for managing gratitude and hopes items.
...
export class GratitudeService {
  ...
  async addOption(householdId, category, text) {
    const item = new GratitudeItem({ id: uuidv4(), text });
  ...
  async addSelection(householdId, category, userId, item, timestamp) {
    ...
    const selection = Selection.create(userId, item, timestamp, uuidv4());
  ...
  async addSelections(householdId, category, userId, items, timestamp) {
    ...
    for (const item of items) {
      ...
      const selection = Selection.create(userId, itemWithId, timestamp, uuidv4());
      await this.#store.addSelection(householdId, category, selection);
  ...
  async discardItem(householdId, category, item) {
    const gratitudeItem = new GratitudeItem({ ...item, id: item.id || uuidv4() });
