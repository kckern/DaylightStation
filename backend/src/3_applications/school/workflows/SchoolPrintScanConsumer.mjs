// Application workflow for translating scan outcomes into follow-up actions.
//
 ...
 *   outcome that bypasses the semantic `speak` funnel would be invisible to
 *   that check — exactly how a sheet once went unanswered. Add outcomes by
 *   calling `speak`.
...
export function createSchoolPrintScanConsumer({
  ...
  const onPayload = (payload) => {
    const { testId, answers, testIdCandidates } = payload;
  ...
  const unsubscribe = realtime.onPrintSheet(config, onPayload);
  logger.info?.('school.print.scan-consumer.ready', {});
