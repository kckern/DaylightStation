...
function harness() { let state = { schema: 'school.flashcard-progress/v1', cards: {}, sessions: {} }; const now = Date.parse('2026-08-24T12:00:00.000Z'); return { service: new FlashcardStudyService({ progressStore: { update: (_u, fn) => { state = fn(state); return state; }, read: () => structuredClone(state) }, decks: { getFlashcardDeck: async () => deck, listFlashcardDecks: async () => [deck] }, now: () => now, id: () => 'session-1' }), read: () => state }; }
describe('FlashcardStudyService', () => {
  it('creates a resumable due/new study session and persists ratings', async () => { const { service, read } = harness(); const opened = await service.open({ userId: 'kid', deckId: deck.id, policy: { newCardLimit: 1 } }); expect(opened.cards).toHaveLength(1); const rated = service.review({ userId: 'kid', sessionId: opened.session.sessionId, cardId: 'a', rating: 'again' }); expect(rated.card.state).toBe('learning'); expect(read().sessions['session-1'].reviews).toBe(1); });
  ...
  it('reports assignment minutes and reviews within the current study-day window', async () => {
    ...
    const service = new FlashcardStudyService({ progressStore: { update: (_u, fn) => { state = fn(state); return state; }, read: () => structuredClone(state) }, decks: { getFlashcardDeck: async () => deck }, now: () => Date.parse('2026-08-24T12:00:00.000Z'), boundaryHour: 4 });
    await expect(service.summary({ userId: 'kid', deckId: deck.id })).resolves.toMatchObject({ today: { reviewed: 1, activeSeconds: 30 } });
  });
  it('selects persisted due cards despite deck-prefixed storage keys', async () => { const { service } = harness(); const opened = await service.open({ userId: 'kid', deckId: deck.id, policy: { newCardLimit: 1 } }); service.review({ userId: 'kid', sessionId: opened.session.sessionId, cardId: 'a', rating: 'again' }); const resumed = await service.open({ userId: 'kid', deckId: deck.id, policy: { newCardLimit: 1 } }); expect(resumed.cards.map((card) => card.cardId)).toEqual(['b']); });
  ...
  it('records a revision migration while preserving surviving card history and retiring removed ids', async () => {
    ...
    const service = new FlashcardStudyService({ progressStore: { update: (_u, fn) => { state = fn(state); return state; }, read: () => structuredClone(state) }, decks: { getFlashcardDeck: async () => published }, now: () => Date.parse('2026-08-24T12:00:00.000Z'), id: () => 'revision-session' });
  ...
  it('opens an assigned assessment through a server-tagged quiz session', async () => {
    ...
    const assessed = new FlashcardStudyService({ progressStore: { update: (_u, fn) => fn({ cards: {}, sessions: {} }), read: () => ({ cards: {}, sessions: {} }) }, decks: { getFlashcardDeck: async () => deck }, assignments: { get: async () => ({ programs: [{ programId: 'flashcards', deckId: deck.id, policy: { quizRequired: true } }] }) }, grader, id: () => 'test-run' });
  ...
  it('uses the deck assessment even when assignment policy has no bank id', async () => {
    ...
    const assessed = new FlashcardStudyService({
