// backend/src/3_applications/school/CardLadderTuningService.test.mjs
import { describe, expect, it, vi } from 'vitest';
import { DEFAULT_SETTINGS, emptyDay, emptyStatusV3, emptyTuning } from '#domains/school/cardLadder/index.mjs';
import { CardLadderTuningService } from './CardLadderTuningService.mjs';
...
function make({ store = memoryStore(), tuner = undefined, notify = vi.fn(async () => ({ status: 'sent' })), programs = null, bounds = null, teacherGate = null, now = () => NOW } = {}) {
  ...
  const rows = programs ?? [{ programId: 'flashcards', deckId: DECK, policy: { mode: 'card-ladder' } }];
  const service = new CardLadderTuningService({
    store,
    assignments: {
...
describe('CardLadderTuningService', () => {
  ...
  it('applies a braked proposal, persists tuning.yml, logs each change and each dropped proposal', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('school.card-ladder.tuning', {
      learnerId: 'test-learner', package: PKG, day: YESTERDAY, setting: 'round.size', from: 5, to: 6, reason: 'all passed',
    });
    expect(logger.info).toHaveBeenCalledWith('school.card-ladder.tuning', expect.objectContaining({
      setting: 'review.gapScale', from: 1.1, to: 1.2, dropped: 'dwell',
    }));
    expect(logger.info).toHaveBeenCalledWith('school.card-ladder.tuning', expect.objectContaining({ setting: 'session.capMinutes', dropped: 'not-tunable' }));
  ...
  it('a push suppressed by quiet hours stays pending and goes out on a later tick — exactly one push', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('school.card-ladder.tuning-push', expect.objectContaining({ status: 'suppressed', day: YESTERDAY }));
    await service.deliverPushes();
    expect(store.s.tuning.history.at(-1).notified).toBe(true);
    expect(logger.info).toHaveBeenCalledWith('school.card-ladder.tuning-push', expect.objectContaining({ status: 'sent' }));
    await service.deliverPushes();
  ...
  it('a push that fails is warned and retried; the tuning write is never lost', async () => {
    ...
    expect(store.s.tuning.history.at(-1).notified).toBe(false);
    expect(logger.warn).toHaveBeenCalledWith('school.card-ladder.tuning-push', expect.objectContaining({ status: 'failed', error: 'push down' }));
    await service.deliverPushes();
    expect(store.s.tuning.history.at(-1).notified).toBe(true);
  });
  ...
  it('a push still pending after 48h is dropped with a warn', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith('school.card-ladder.tuning-push', expect.objectContaining({ status: 'dropped' }));
  });
  ...
  it('model failure → no changes, logged, the day is marked so it is not retried every tick', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith('school.card-ladder.tuning-failed', expect.objectContaining({ learnerId: 'test-learner', day: YESTERDAY, error: 'invalid tuner output: nope' }));
    expect(notify).not.toHaveBeenCalled();
  });
  ...
  describe('pending', () => {
    ...
    it('one package per learner even with two decks in it; an unloadable deck is skipped and logged', async () => {
      const store = memoryStore({ days: { [YESTERDAY]: studyDay(YESTERDAY) } });
      const programs = [
        { programId: 'flashcards', deckId: DECK, policy: { mode: 'card-ladder' } },
        { programId: 'flashcards', corpusId: DECK, policy: { mode: 'card-ladder' } },
        { programId: 'flashcards', deckId: 'gone', policy: { mode: 'card-ladder' } },
      ];
      const { service, logger } = make({ store, programs });
      expect(await service.pending({ now: NOW })).toHaveLength(1);
      expect(logger.warn).toHaveBeenCalledWith('school.card-ladder.tuning-deck-skipped', expect.objectContaining({ deckId: 'gone' }));
    });
  });
});

describe('CardLadderTuningService — races and bounds (fix round 1)', () => {
...
describe('CardLadderTuningService — grown-up view and undo', () => {
  ...
  it('the view is teacher-gated and shows current vs default, the last run and the history newest first', async () => {
    ...
    expect(teacherGate.assert).toHaveBeenLastCalledWith(expect.objectContaining({ userId: 'grown-up', action: 'card-ladder.tuning' }));
