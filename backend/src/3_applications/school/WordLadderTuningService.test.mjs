...
function make({ store = memoryStore(), tuner = undefined, notify = vi.fn(async () => {}), programs = null } = {}) {
...
describe('WordLadderTuningService', () => {
  ...
  describe('skip rules', () => {
    ...
    it('skips a day with no sitting that reached goal or cap (idle / unmount / leave never trigger)', async () => {
      for (const reason of ['idle', 'unmount', 'leave', null]) {
        const store = memoryStore({ days: { [YESTERDAY]: studyDay(YESTERDAY, { reason }) } });
        const { service } = make({ store });
  ...
  it('model failure → no changes, logged, the day is marked so it is not retried every tick', async () => {
    ...
    const out = await service.runFor(RUN);
    expect(out).toMatchObject({ status: null, applied: [], error: 'invalid tuner output: nope' });
    expect(store.s.tuning.values).toEqual({ 'round.size': 6 });
    expect(store.s.tuning.lastTunedDay).toBe(YESTERDAY);
    expect(store.s.tuning.history.at(-1)).toMatchObject({ day: YESTERDAY, status: null, error: 'invalid tuner output: nope', applied: [] });
  ...
  describe('no model (tuner disabled)', () => {
    it('writes a deterministic note and changes nothing', async () => {
      ...
      expect(store.s.tuning.history.at(-1).notes).toMatch(/no model/i);
      expect(notify).not.toHaveBeenCalled();
    });
  ...
  describe('pending', () => {
    ...
    it('omits a tuned day, an unfinished day and a learner with no study days', async () => {
      ...
      const idle = memoryStore({ days: { [YESTERDAY]: studyDay(YESTERDAY, { reason: 'idle' }) } });
