  ...
  async execute({
    ...
    const followUps = (await Promise.all(this.#followUps.map(async (source) => {
      try {
        ...
        this.#logger.warn?.('school.progress.follow-up-source-failed', { scopeType, scopeId, error: error.message });
        return [];
      }
    }))).flat();
    ...
    const expectations = mergeLearningExpectations(await Promise.all(
      this.#expectationSources.map(async (source) => {
        try {
          ...
          this.#logger.warn?.('school.progress.expectation-source-failed', { scopeType, scopeId, error: error.message });
          return [];
        }
      }),
    ));
