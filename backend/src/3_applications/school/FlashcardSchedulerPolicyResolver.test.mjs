...
const policySource = {
  ...
  learnerScheduler: () => ({ overrides: { maximumIntervalDays: 90 } }),
};

describe('FlashcardSchedulerPolicyResolver', () => {
  it('layers only trusted sources and snapshots safe overrides', async () => {
    const resolver = new FlashcardSchedulerPolicyResolver({ policySource, assignments: { get: async () => ({ programs: [{ programId: 'flashcards', deckId: deck.id, policy: { newCardLimit: 7 } }] }) } });
  ...
  it('rejects conflicting concept-specific scheduler settings', async () => {
    const resolver = new FlashcardSchedulerPolicyResolver({ policySource: { ...policySource, householdScheduler: () => ({ concepts: { one: { overrides: { requestRetention: 0.8 } }, two: { overrides: { requestRetention: 0.9 } } } }) }, assignments: { get: async () => null } });
