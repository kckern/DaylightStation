...
/** Re-render an artifact's YAML recipe without allocating a new card or artifact. */
export class ReprintIssuedArtifact {
  #artifacts; #renderIssuedArtifact; #sessions; #printer; #teacherGate; #exceptions; #clock; #logger;
  constructor({ issuedArtifacts, sessions, printer, teacherGate, curriculumExceptions = null,
    renderIssuedArtifact = null, clock = () => new Date(), logger = console } = {}) {
  ...
  async execute({ artifactId, reprintedBy, pin, idempotencyKey, apply = false } = {}) {
    ...
    if (prior) return { schema: 'school.artifact-reprint-receipt/v1', applied: true, idempotent: true,
      artifactId, sessionId, cardId: allocation?.cardId ?? null,
      sha256: artifact.manifest.renderInputSha256 ?? artifact.manifest.sha256 ?? null };
    ...
    const rendered = this.#renderIssuedArtifact
      ? await this.#renderIssuedArtifact.execute({ artifactId, artifact })
      : { bytes: artifact.bytes, duplex: artifact.manifest.renderContext?.duplex ?? null };
    if (!Buffer.isBuffer(rendered.bytes)) {
      throw new DomainInvariantError('worksheet artifact cannot be regenerated', { code: 'ARTIFACT_SOURCE_UNAVAILABLE' });
    }
    const renderedSha256 = rendered.sha256 ?? sha256Bytes(rendered.bytes);
    const preview = { schema: 'school.artifact-reprint-receipt/v1', applied: false, idempotent: false,
      ...
      byteLength: rendered.bytes.length, sha256: renderedSha256 };
    if (!apply) return preview;
    const result = await this.#printer.printPdf(rendered.bytes, {
      jobName: `school-reprint-${artifactId}`, user: state.learnerId,
      duplex: rendered.duplex ?? artifact.manifest.renderContext?.duplex ?? undefined,
    });
