...
describe('GetLearnerRecord (admin advocacy #14 — one merged record per child)', () => {
  it('merges six channels newest-first, each row tagged with its source', async () => {
    const uc = new GetLearnerRecord({
      ...
      quizRequests: () => [
        { at: '2026-08-04T10:00:00Z', userId: 'felix', kind: 'flag', bankId: 'caps', note: 'looks wrong' },
        { at: '2026-08-04T11:00:00Z', userId: 'milo', unitId: 'x' }, // other kid, excluded
      ],
      printRequests: (id) => (id === 'felix' ? [{ at: '2026-08-06T10:00:00Z', label: 'Maze', status: 'denied', deniedBy: 'kckern' }] : []),
      logger: silent,
    });
    const { entries } = await uc.execute({ learnerId: 'felix' });
  ...
  it('every store optional; a throwing store degrades to absent, never a crash', async () => {
    ...
    await expect(uc.execute({ learnerId: 'felix' })).resolves.toEqual({ learnerId: 'felix', entries: [] });
  });
});
