// backend/src/3_applications/school/CardLadderSittingService.test.mjs
import { describe, expect, it, vi } from 'vitest';
import { emptyDay, emptyStatusV3, emptyWordV3 } from '#domains/school/cardLadder/index.mjs';
...
function make({ judgementCache = null, recordings = null, mode = 'live', attempts = null, attemptsReader = null, teacherGate = null, judge = null, store = memoryStore(), media = false, decks = null, bounds = null, peek = undefined } = {}) {
  ...
  const service = new CardLadderSittingService({
...
describe('CardLadderSittingService', () => {
  ...
  it('intro: the start card, read-only, before any sitting opens', async () => {
    ...
    expect(store.s.writes).toBe(0);
    await expect(service.intro({ userId: 'someone-else', deckId: DECK })).rejects.toThrow(/no card-ladder assignment/);
  });
  ...
  it('open refuses a sibling\'s per-learner quiz document, logs it, never demotes, and does not re-log it (plan 3, spec §8)', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith('school.card-ladder.fold-refused', {
      learnerId: 'test-learner', package: 'korean-vocab', mode: 'live', attemptId: 'att_sibling', bankId,
    });
    logger.warn.mockClear();
    await service.open({ userId: 'test-learner', deckId: DECK });
    expect(logger.warn).not.toHaveBeenCalledWith('school.card-ladder.fold-refused', expect.anything());
  });
  ...
  it('an unreadable attempts log does not advance lastFoldedDay', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith('school.card-ladder.attempts-unreadable', expect.objectContaining({ learnerId: 'test-learner' }));
  });
  ...
  it('fold: the teacher gate refuses a non-teacher', async () => {
    ...
    expect(teacherGate.assert).toHaveBeenCalledWith(expect.objectContaining({ action: 'card-ladder.fold', context: { learnerId: 'test-learner' } }));
  });
  ...
  it('idle close: after 5 min without input, other open sittings close as idle at the last input', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('school.card-ladder.closed', expect.objectContaining({ sittingId: a.sittingId, reason: 'idle' }));
...
describe('CardLadderSittingService — drills, speaking, practice, My words', () => {
...
describe('CardLadderSittingService — graded and transition events (plan 4, spec §8)', () => {
  ...
  it('an ordinary response logs answered and its transition, never graded', async () => {
    ...
    expect(calls(logger, 'school.card-ladder.graded')).toEqual([]);
    expect(calls(logger, 'school.card-ladder.answered')).toEqual([expect.objectContaining({ learnerId: 'test-learner', sittingId, mode: 'live', itemId: item.id, type: 'flashcard', wordId: 'gawi' })]);
    expect(calls(logger, 'school.card-ladder.transition')).toEqual([{
  ...
  it('a graded recheck logs graded once and its transition, with mode', async () => {
    ...
    expect(calls(logger, 'school.card-ladder.transition')).toEqual([expect.objectContaining({
      mode: 'test', wordId: 'gawi', from: { state: 'mastered', stage: 0 }, to: { state: 'familiar', stage: null }, source: 'recheck',
    })]);
    expect(calls(logger, 'school.card-ladder.answered')).toHaveLength(1);
  });
  ...
  it('a replayed answer logs no second graded or transition', async () => {
    ...
    await service.respond({ userId: 'test-learner', sittingId, itemId: item.id, response: { dontKnow: true } });
    await service.respond({ userId: 'test-learner', sittingId, itemId: item.id, response: { dontKnow: true } });
    expect(calls(logger, 'school.card-ladder.graded')).toHaveLength(1);
    expect(calls(logger, 'school.card-ladder.transition')).toHaveLength(1);
  });
...
describe('CardLadderSittingService — grown-up word controls (plan 4 task 4, spec §6)', () => {
  ...
  it('every admin call is teacher-gated, and a refusal changes nothing', async () => {
    ...
    for (const [args] of teacherGate.assert.mock.calls) {
      expect(args).toEqual({ userId: 'parent', pin: '1234', action: 'card-ladder.admin', context: { learnerId: 'test-learner' } });
    }
  ...
  it('adminReset returns a word to new, clearing notYetCarry, and logs admin + transition', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('school.card-ladder.admin', expect.objectContaining({
      actorId: 'parent', learnerId: 'test-learner', package: 'korean-vocab', wordId: 'gawi', action: 'reset',
    }));
    expect(logger.info).toHaveBeenCalledWith('school.card-ladder.transition', expect.objectContaining({
  ...
  it('adminDropDeck removes a deck from decksSeen and keeps introduced words', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('school.card-ladder.admin', expect.objectContaining({ action: 'drop-deck', deckId: DECK_OTHER, wordId: null }));
  });
  ...
  it('adminRegrade overwrites the judge cache for that answer, records it on the item, and leaves word state', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('school.card-ladder.admin', expect.objectContaining({ action: 'regrade', wordId: 'gawi', pass: true }));
...
describe('CardLadderSittingService — tuned settings (plan 5 task 3, spec §7)', () => {
  ...
  it('tuned values are clamped to the household card_ladder.bounds too', async () => {
  ...
  it('a store with no tuning (or one that throws) opens on the defaults', async () => {
    ...
    await service.open({ userId: 'test-learner', deckId: DECK });
    expect(store.s.days[TODAY].atOpen.settings.round.size).toBe(5);
    expect(logger.warn).toHaveBeenCalledWith('school.card-ladder.tuning-unreadable', expect.objectContaining({ learnerId: 'test-learner', package: 'korean-vocab' }));
  });
...
describe('CardLadderSittingService — sequencing observability (spec §8)', () => {
  const calls = (logger, event) => logger.info.mock.calls.filter(([name]) => name === event).map(([, data]) => data);

  it('open logs the day and round plan, and why the first item was served', async () => {
    const { service, logger } = make();
    const { sittingId, item } = await service.open({ userId: 'test-learner', deckId: DECK });
    expect(calls(logger, 'school.card-ladder.day.planned')).toEqual([expect.objectContaining({ sittingId, dueRechecks: [], newAllowance: 4 })]);
    expect(calls(logger, 'school.card-ladder.round.planned')).toEqual([expect.objectContaining({
      sittingId, round: 'r1', index: 1, kind: 'new', size: 2, newIds: ['gawi', 'pul'], carryIds: [], hasMatch: true, itemId: null,
    })]);
    expect(calls(logger, 'school.card-ladder.item.served')).toEqual([expect.objectContaining({
  ...
  it('each answer logs the next item served and every plan change it caused', async () => {
    ...
    const served = calls(logger, 'school.card-ladder.item.served');
    ...
    const phases = calls(logger, 'school.card-ladder.round.phase').map((p) => `${p.from}>${p.to}`);
    expect(phases).toEqual(['intro>stream', 'stream>quiz']);
    expect(calls(logger, 'school.card-ladder.round.phase').at(-1)).toMatchObject({ queue: expect.arrayContaining(['gawi:3.1']), itemId: expect.any(String) });
  });
  ...
  it('a recognition pass logs the word climbing its prerequisites, and transitions carry them', async () => {
    ...
    const climbs = calls(logger, 'school.card-ladder.word.prereqs');
    ...
    const mastered = calls(logger, 'school.card-ladder.transition').find((t) => t.to.state === 'mastered');
  ...
  it('a recheck is served with the prerequisites it still lacks', async () => {
    const { service, logger } = make({ store: dueStore(1) });
    await service.open({ userId: 'test-learner', deckId: DECK });
    expect(calls(logger, 'school.card-ladder.item.served')[0]).toMatchObject({ source: 'recheck', reason: 'recheck-recognition:recognized<2,unmatched,stage<1' });
  });
  ...
  it('a sitting left open and then idle-closed is logged as abandoned with where it stopped', async () => {
    ...
    advance(6 * 60_000);
    await service.open({ userId: 'test-learner', deckId: DECK });
    const abandoned = logger.warn.mock.calls.filter(([name]) => name === 'school.card-ladder.sitting.abandoned').map(([, d]) => d);
