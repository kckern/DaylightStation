...
export class ConversationService {
  constructor({ conversationStore, random = Math.random }) {
  ...
  generateConversationId(nowMs) {
    ...
    if (typeof this.random !== 'function') throw new ValidationError('random required for generateConversationId');
    return `conv-${nowMs}-${this.random().toString(36).slice(2, 8)}`;
  }
}
