...
import { Notification } from '#domains/messaging/entities/Notification.mjs';
import { ValidationError, EntityNotFoundError } from '#domains/core/errors/index.mjs';

export class NotificationService {
  constructor({ notificationStore, channels = {}, random = Math.random }) {
  ...
  generateId(nowMs) {
    ...
    if (typeof this.random !== 'function') throw new ValidationError('random required for generateId');
    return `notif-${nowMs}-${this.random().toString(36).slice(2, 8)}`;
  }
}
