// Application workflow for validating and backfilling media-memory identities.
...
export class MediaMemoryValidatorService {
  ...
  constructor({ plexClient, watchStateStore, random = Math.random }) {
  ...
  selectEntriesToCheck(entries, nowMs) {
    ...
    if (typeof this.#random !== 'function') throw new TypeError('random is required for sampling');
    const shuffled = [...older].sort(() => this.#random() - 0.5);
