/**
 * Task 6 — page furniture: the page-x-of-y footer (every page), the
 * continuation strip (pages 2+), and the gutter margin (fixed side, or
 * alternating per page parity under `duplex`).
 *
...
describe('furniture — contentBox', () => {
  it('reserves footer band + continuation strip out of pageHeightPt, no gutter by default', () => {
    const box = contentBox(theme, {});
    expect(box.pageHeightPt).toBeCloseTo(
      theme.page.heightPt - theme.furniture.footerBandPt - theme.furniture.continuationStripPt, 6,
    );
...
describe('furniture — drawFurniture', () => {
  it('draws "page n of pageCount" on every page of a 3-page document', () => {
    const results = [1, 2, 3].map((page) => {
      ...
      drawFurniture(chain, { theme, page, pageCount: 3, title: 'Fixture' });
      return textCalls(calls).find((c) => c.str === `Page ${page} of 3`);
    });
    expect(results.every(Boolean)).toBe(true);
  });

  it('omits the continuation strip on page 1, draws it on pages 2 and 3', () => {
    const page1 = createRecorder();
    drawFurniture(page1.chain, { theme, page: 1, pageCount: 3, title: 'Fixture', nameLine: 'Alice' });
    expect(textCalls(page1.calls).some((c) => c.str === 'Fixture')).toBe(false);
    expect(textCalls(page1.calls).some((c) => c.str.includes('Name:'))).toBe(false);

    for (const page of [2, 3]) {
      const rec = createRecorder();
      drawFurniture(rec.chain, { theme, page, pageCount: 3, title: 'Fixture', nameLine: 'Alice' });
      const calls = textCalls(rec.calls);
      expect(calls.some((c) => c.str === 'Fixture'), `page ${page} title`).toBe(true);
      expect(calls.some((c) => c.str === 'Name: Alice'), `page ${page} name`).toBe(true);
      expect(calls.some((c) => c.str === `Page ${page} of 3`), `page ${page} footer`).toBe(true);
    }
  });

  it('blanks the strip\'s name field when nameLine is not supplied', () => {
    const rec = createRecorder();
    drawFurniture(rec.chain, { theme, page: 2, pageCount: 2, title: 'Fixture' });
    const nameCall = textCalls(rec.calls).find((c) => c.str.startsWith('Name:'));
  ...
  it('fixed (non-duplex) gutter keeps furniture on the same side across pages', () => {
    ...
    drawFurniture(p1.chain, { theme, page: 1, pageCount: 2, title: 'Fixture', gutter: true, duplex: false });
    const p2 = createRecorder();
    drawFurniture(p2.chain, { theme, page: 2, pageCount: 2, title: 'Fixture', gutter: true, duplex: false });
  ...
  it('duplex gutter flips the footer/strip x-offset by page parity', () => {
    const recto = createRecorder(); // page 1 (odd)
    drawFurniture(recto.chain, { theme, page: 1, pageCount: 2, title: 'Fixture', gutter: true, duplex: true });
    const verso = createRecorder(); // page 2 (even)
    drawFurniture(verso.chain, { theme, page: 2, pageCount: 2, title: 'Fixture', nameLine: 'Bo', gutter: true, duplex: true });
    ...
    const versoStripTitle = textCalls(verso.calls).find((c) => c.str === 'Fixture');
    ...
    // The strip moves with the same gutter-adjusted left edge as the footer.
    expect(versoStripTitle.xPt).toBe(versoFooter.xPt);
  });
  ...
  it('snapshot: a furnished 3-page document\'s full furniture draw trace', () => {
    const trace = [1, 2, 3].map((page) => {
      ...
      drawFurniture(rec.chain, {
        theme, page, pageCount: 3, title: 'Furnished Fixture', nameLine: page === 2 ? 'Alice' : null, duplex: true, gutter: true,
      });
...
describe('furniture — DocumentPdfRenderer integration (opt-in `options.furniture`)', () => {
  ...
  // lands mid-range (24-34 all produce 3 pages) under workbookTheme's default
  // scale/density with furniture's footer+strip reservation active — not a
  // borderline count that a stray leading-height tweak could tip over.
