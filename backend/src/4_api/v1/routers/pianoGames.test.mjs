...
function appFor(container, compatibilityRouters = {}) {
  const app = express();
  app.use(express.json());
  app.use('/api/v1/piano-games', createPianoGamesRouter({ container, compatibilityRouters }));
  return app;
}
...
test('mounts mature game compatibility routers before the generic route', async () => {
