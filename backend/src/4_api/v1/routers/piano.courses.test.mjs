...
describe('POST /api/v1/piano/users/:userId/video-log', () => {
  const progressFile = '/tmp/piano-test-user/apps/piano/video-progress.yml';

  afterEach(() => {
    if (fs.existsSync(progressFile)) fs.rmSync(progressFile, { force: true });
  });
  ...
  it('preserves existing completedAt on subsequent posts', async () => {
    ...
    expect(res2.body.completedAt).toBeTruthy();
  });
