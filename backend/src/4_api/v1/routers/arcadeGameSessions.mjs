 ...
 */
export function createArcadeGameSessionsRouter({ recordObservation = null, sessions = null, trackers = [], watchdog = null, sessionMonitor = null, grantLedger = null, grantArcadeGameTime = null, checkEligibility = null, summariseArcadeGameUsage = null, placements = null, logger = console }) {
  ...
  router.post('/grants', async (req, res) => {
    if (!grantArcadeGameTime) return res.status(503).json({ error: 'Play-time grants are not configured' });
    ...
    try {
      return res.json(await grantArcadeGameTime.execute({ userId, minutes: Number(minutes), by, reason, on }));
    } catch (error) {
      return res.status(400).json({ error: error.message });
    }
  });
  ...
  router.get('/usage', async (req, res) => {
    if (!summariseArcadeGameUsage) return res.status(503).json({ error: 'Play-session recording is not configured' });
    const since = req.query.since || new Date(Date.now() - 7 * 24 * 3600 * 1000).toISOString();
    ...
    try {
      return res.json(await summariseArcadeGameUsage.execute({ since, until }));
    } catch (error) {
...
export default createArcadeGameSessionsRouter;
