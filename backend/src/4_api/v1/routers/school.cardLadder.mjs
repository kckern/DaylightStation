...
export function mountCardLadderRoutes({
  ...
  router.get('/card-ladder/stage', wrap(async (_req, res) => noStore(res).json({ screen: stageScreen })));
  router.post('/card-ladder/fold', wrap(async (req, res) => {
  ...
  router.get('/card-ladder/admin/words', wrap(async (req, res) => {
  ...
  const adminPost = (path, method, fields) => router.post(`/card-ladder/admin/${path}`, wrap(async (req, res) => {
    const body = req.body || {};
  ...
  router.get('/card-ladder/admin/tuning', wrap(async (req, res) => {
    const { learnerId, deckId } = req.query;
    const pin = req.query.pin ?? capabilityProof(req) ?? null;
    const actorId = sessionActorId(req) ?? req.query.actorId ?? null;
    noStore(res).json(await tuning().adminTuning({ learnerId, deckId, actorId, pin }));
  }));
  router.post('/card-ladder/admin/tuning/undo', wrap(async (req, res) => {
  ...
  mount('/card-ladder/test', () => cardLadderTest, { test: true });
  mount('/card-ladder', () => cardLadderStudy, { test: false });
}
