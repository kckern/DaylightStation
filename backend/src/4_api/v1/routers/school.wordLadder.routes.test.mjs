...
describe('/api/v1/school/word-ladder (through the School router)', () => {
  it('404s when the word ladder (or its test mode) is not wired', async () => {
    await request(app(null)).post('/api/v1/school/word-ladder/open').send({ userId: 'kid', deckId: 'd' }).expect(404);
    const live = { open: vi.fn() };
    await request(app(live)).post('/api/v1/school/word-ladder/test/open').send({ userId: 'kid', deckId: 'd' }).expect(404);
  ...
  it('opens a sitting and never caches it', async () => {
    ...
    const res = await request(app(service)).post('/api/v1/school/word-ladder/open')
  ...
  it('maps a missing assignment to 403 and a bad response to 400', async () => {
    ...
    await request(app(invalid)).post('/api/v1/school/word-ladder/sittings/korean-vocab.live.1/items/i9')
  ...
  it('serves the test mode, the stage screen and the teacher fold', async () => {
    ...
    const server = app(live, { wordLadderTest: test, wordLadderStageScreen: 'portal' });
    ...
    const res = await request(server).post('/api/v1/school/word-ladder/fold').send({ learnerId: 'kid', actorId: 'parent' }).expect(200);
