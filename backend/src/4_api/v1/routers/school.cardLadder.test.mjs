...
describe('card-ladder routes', () => {
  it('live open ignores a scenario; test open passes it', async () => {
    const { a, live, test } = app();
    await request(a).post('/card-ladder/open').send({ userId: 'u', deckId: 'd', scenario: 'fresh' }).expect(200);
    expect(live.open).toHaveBeenCalledWith({ userId: 'u', deckId: 'd' });
    await request(a).post('/card-ladder/test/open').send({ userId: 'u', deckId: 'd', scenario: 'fresh' }).expect(200);
  ...
  it('intro is a read-only GET on both mounts; the poster ref becomes a self-service URL', async () => {
    ...
    // Never a write verb.
    await request(a).post('/card-ladder/intro').send({ userId: 'u', deckId: 'd' }).expect(404);
  });
  it('responses are private no-store and the stage screen is served', async () => {
    const { a } = app();
    const res = await request(a).post('/card-ladder/sittings/p.live.1/items/x').send({ userId: 'u', response: { seen: true } }).expect(200);
    expect(res.headers['cache-control']).toBe('private, no-store');
    expect((await request(a).get('/card-ladder/stage').expect(200)).body).toEqual({ screen: 'portal' });
  });
  it('routes get, respond and close to the right service', async () => {
    const { a, live, test } = app();
    await request(a).get('/card-ladder/sittings/p.live.1?userId=u').expect(200);
    expect(live.get).toHaveBeenCalledWith({ userId: 'u', sittingId: 'p.live.1' });
    await request(a).get('/card-ladder/test/sittings/test.p.t.1?userId=u').expect(200);
    expect(test.get).toHaveBeenCalledWith({ userId: 'u', sittingId: 'test.p.t.1' });
    await request(a).post('/card-ladder/sittings/p.live.1/items/x').send({ userId: 'u', response: { typed: 'a' } }).expect(200);
    expect(live.respond).toHaveBeenCalledWith({ userId: 'u', sittingId: 'p.live.1', itemId: 'x', response: { typed: 'a' } });
    await request(a).post('/card-ladder/test/sittings/test.p.t.1/close').send({ userId: 'u' }).expect(200);
  ...
  it('fold is live only; an unwired test service is not configured', async () => {
    const { a, live } = app({ cardLadderTest: null });
    await request(a).post('/card-ladder/fold').send({ learnerId: 'k', actorId: 'p' }).expect(200);
    expect(live.fold).toHaveBeenCalledWith({ learnerId: 'k', actorId: 'p', pin: null });
    await request(a).post('/card-ladder/test/open').send({ userId: 'u', deckId: 'd' }).expect(503);
  });
  it('open passes capabilities on both mounts', async () => {
    const { a, live, test } = app();
    await request(a).post('/card-ladder/open').send({ userId: 'u', deckId: 'd', capabilities: { microphone: true } }).expect(200);
    expect(live.open).toHaveBeenCalledWith({ userId: 'u', deckId: 'd', capabilities: { microphone: true } });
    await request(a).post('/card-ladder/test/open').send({ userId: 'u', deckId: 'd', scenario: 'tricky', capabilities: { microphone: false } }).expect(200);
  ...
  it('a raw audio take is posted to the mount\'s own service', async () => {
    const { a, live, test } = app();
    const res = await request(a).post('/card-ladder/sittings/p.live.1/recordings/d1:2?userId=u&ext=ogg')
    ...
    await request(a).post('/card-ladder/test/sittings/test.p.t.1/recordings/d1:2?userId=u')
  ...
  it('practice and My words route to the mount\'s service', async () => {
    ...
    await request(a).post('/card-ladder/test/sittings/test.p.t.1/practice').send({ userId: 'u', mode: 'match' }).expect(200);
    ...
    const res = await request(a).get('/card-ladder/words?userId=u&deckId=d').expect(200);
    expect(res.headers['cache-control']).toBe('private, no-store');
    expect(live.words).toHaveBeenCalledWith({ userId: 'u', deckId: 'd', sittingId: null });
    await request(a).get('/card-ladder/test/words?userId=u&deckId=d&sittingId=test.p.t.1').expect(200);
  ...
  it('grown-up word controls are live only and pass actorId + pin through', async () => {
    ...
    const res = await request(a).get('/card-ladder/admin/words?learnerId=k&deckId=d&actorId=p&pin=1234').expect(200);
    expect(res.headers['cache-control']).toBe('private, no-store');
    expect(admin.adminWords).toHaveBeenCalledWith(who);
    await request(a).post('/card-ladder/admin/reset').send({ ...who, wordId: 'w' }).expect(200);
    expect(admin.adminReset).toHaveBeenCalledWith({ ...who, wordId: 'w' });
    await request(a).post('/card-ladder/admin/mastered').send({ ...who, wordId: 'w', stage: 2 }).expect(200);
    expect(admin.adminMarkMastered).toHaveBeenCalledWith({ ...who, wordId: 'w', stage: 2 });
    await request(a).post('/card-ladder/admin/exclude').send({ ...who, wordId: 'w', excluded: false }).expect(200);
    expect(admin.adminExclude).toHaveBeenCalledWith({ ...who, wordId: 'w', excluded: false });
    await request(a).post('/card-ladder/admin/drop-deck').send({ ...who, dropDeckId: 'x' }).expect(200);
    expect(admin.adminDropDeck).toHaveBeenCalledWith({ ...who, dropDeckId: 'x' });
    await request(a).post('/card-ladder/admin/regrade').send({ ...who, day: '2026-09-20', itemId: 'rc:w', pass: true }).expect(200);
    expect(admin.adminRegrade).toHaveBeenCalledWith({ ...who, day: '2026-09-20', itemId: 'rc:w', pass: true });
    await request(a).post('/card-ladder/test/admin/reset').send({ ...who, wordId: 'w' }).expect(404);
    const { a: unwired } = app({ cardLadderStudy: null });
    await request(unwired).post('/card-ladder/admin/reset').send({ ...who, wordId: 'w' }).expect(503);
  });
  it('the admin words GET takes the console cookie capability when no query pin is given', async () => {
    ...
    await request(a).get('/card-ladder/admin/words?learnerId=k&deckId=d&actorId=p').set('Cookie', 'daylight_teacher_session=cap-1').expect(200);
    expect(adminWords).toHaveBeenLastCalledWith({ learnerId: 'k', deckId: 'd', actorId: 'p', pin: proof });
    await request(a).get('/card-ladder/admin/words?learnerId=k&deckId=d&actorId=p&pin=1234').set('Cookie', 'daylight_teacher_session=cap-1').expect(200);
    expect(adminWords).toHaveBeenLastCalledWith({ learnerId: 'k', deckId: 'd', actorId: 'p', pin: '1234' });
    await request(a).get('/card-ladder/admin/words?learnerId=k&deckId=d&actorId=p').expect(200);
  ...
  it('the admin words GET derives the acting teacher from the capability session when no actorId is given', async () => {
    ...
    await request(a).get('/card-ladder/admin/words?learnerId=k&deckId=d')
    ...
    await request(a).get('/card-ladder/admin/words?learnerId=k&deckId=d&actorId=someone-else')
      .set('Cookie', 'daylight_teacher_session=cap-1').expect(200);
    expect(adminWords).toHaveBeenLastCalledWith({ learnerId: 'k', deckId: 'd', actorId: 'p', pin: proof });
    // No cookie, no session: falls back to the query actorId (the CLI case).
    await request(a).get('/card-ladder/admin/words?learnerId=k&deckId=d&actorId=q').expect(200);
  ...
  it('tuning GET and undo POST are live only, reach the tuning service, and take the acting teacher from the capability session', async () => {
    ...
    const { a } = app({ cardLadderTuning: tuning, capabilityProof, teacherCapabilitySessions });
    const res = await request(a).get('/card-ladder/admin/tuning?learnerId=k&deckId=d&actorId=someone-else')
      .set('Cookie', 'daylight_teacher_session=cap-1').expect(200);
    expect(res.headers['cache-control']).toBe('private, no-store');
    expect(tuning.adminTuning).toHaveBeenLastCalledWith({ learnerId: 'k', deckId: 'd', actorId: 'p', pin: proof });
    await request(a).post('/card-ladder/admin/tuning/undo').set('Cookie', 'daylight_teacher_session=cap-1')
    ...
    await request(a).post('/card-ladder/admin/tuning/undo').send({ learnerId: 'k', deckId: 'd', setting: 'round.size', actorId: 'q', pin: '1234' }).expect(200);
    expect(tuning.adminUndo).toHaveBeenLastCalledWith({ learnerId: 'k', deckId: 'd', setting: 'round.size', actorId: 'q', pin: '1234' });
    await request(a).post('/card-ladder/test/admin/tuning/undo').send({ learnerId: 'k', setting: 'round.size' }).expect(404);
    const { a: unwired } = app();
    await request(unwired).get('/card-ladder/admin/tuning?learnerId=k&deckId=d').expect(503);
    await request(unwired).post('/card-ladder/admin/tuning/undo').send({ learnerId: 'k', setting: 'round.size' }).expect(503);
  });
});
