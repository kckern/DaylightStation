...
export function createRequirementsRouter({ operations, actorFromRequest }) {
  if (!operations || !actorFromRequest) throw new Error('createRequirementsRouter requires operations and actorFromRequest');
  ...
  router.get('/', asyncHandler(async (req, res) => {
    res.json(await operations.getCurrentRequirements(req.householdId, filters(req.query)));
  }));

  router.get('/:requirementId', asyncHandler(async (req, res) => {
    const response = await operations.getCurrentRequirements(req.householdId, { ...filters(req.query), requirementId: req.params.requirementId });
    const evaluated = response.items.find(item => item.evaluation.requirementId === req.params.requirementId);
    res.json(oneOr404(response, response.definitions.find(item => item.id === req.params.requirementId), { evaluation: evaluated?.evaluation ?? null }, 'Requirement not found', 'REQUIREMENT_NOT_FOUND'));
  }));
...
export default createRequirementsRouter;
