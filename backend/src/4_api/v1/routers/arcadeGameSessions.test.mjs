...
import { createArcadeGameSessionsRouter } from './arcadeGameSessions.mjs';
...
describe('POST /observations — push ingress for self-reporting surfaces', () => {
  it('feeds the same use case the polled source uses', async () => {
    const r = await call(createArcadeGameSessionsRouter({ recordObservation, logger: quiet }), 'POST', '/observations', {
  ...
  it('defaults a self-reported observation to exact confidence', async () => {
    await call(createArcadeGameSessionsRouter({ recordObservation, logger: quiet }), 'POST', '/observations', {
  ...
  it('rejects an observation missing its essentials', async () => {
    const r = await call(createArcadeGameSessionsRouter({ recordObservation, logger: quiet }), 'POST', '/observations', { deviceId: 'd' });
    expect(r.status).toBe(400);
  });

  it('says so plainly when metering is not configured', async () => {
    const r = await call(createArcadeGameSessionsRouter({ logger: quiet }), 'POST', '/observations', {
...
describe('GET /devices/:deviceId', () => {
  it('returns the open session', async () => {
    const r = await call(createArcadeGameSessionsRouter({ sessions, logger: quiet }), 'GET', '/devices/tv');
    expect(r.body.session).toMatchObject({ id: 'ps_1', playedMs: 42_000 });
  });

  it('returns null rather than 404 for an idle device', async () => {
    const r = await call(createArcadeGameSessionsRouter({ sessions, logger: quiet }), 'GET', '/devices/art-panel');
    expect(r.status).toBe(200);
...
describe('GET /open', () => {
  it('returns every open session for a house-wide initial snapshot', async () => {
    const r = await call(createArcadeGameSessionsRouter({ sessions, logger: quiet }), 'GET', '/open');
    expect(r.status).toBe(200);
...
describe('GET /devices/:deviceId/history — played time became money', () => {
  ...
  it('returns sessions with what was played and how precisely', async () => {
    const r = await call(createArcadeGameSessionsRouter({ sessions: withHistory, logger: quiet }),
  ...
  it('returns an empty list rather than an error for a device with no history', async () => {
    const r = await call(createArcadeGameSessionsRouter({ sessions: withHistory, logger: quiet }), 'GET', '/devices/other/history');
    expect(r.body.sessions).toEqual([]);
  });
});
...
describe('GET /health', () => {
  it('exposes per-device observation health', async () => {
    ...
    const r = await call(createArcadeGameSessionsRouter({ trackers, logger: quiet }), 'GET', '/health');
  ...
  it('reports devices the meter has lost sight of', async () => {
    ...
    const r = await call(createArcadeGameSessionsRouter({ watchdog, logger: quiet }), 'GET', '/health');
  ...
  it('exposes the house-wide open-session monitor health', async () => {
    ...
    const r = await call(createArcadeGameSessionsRouter({ sessionMonitor, logger: quiet }), 'GET', '/health');
...
describe('parent controls — grant, extend, revoke', () => {
  ...
  const router = () => createArcadeGameSessionsRouter({ grantLedger, grantArcadeGameTime, logger: quiet });
  beforeEach(() => { rows = []; });
  ...
  it('says so plainly when grants are not configured', async () => {
    const r = await call(createArcadeGameSessionsRouter({ logger: quiet }), 'POST', '/grants', { userId: 'c', minutes: 5 });
    expect(r.status).toBe(503);
  });
});
...
describe('GET /usage — the monitoring surface', () => {
  ...
  const withLedger = (rows) => createArcadeGameSessionsRouter({
    summariseArcadeGameUsage: new SummariseArcadeGameUsage({ sessions: { listSince: async () => rows }, logger: quiet }),
    logger: quiet,
  });
  ...
  it('says so plainly when recording is not configured', async () => {
    const r = await call(createArcadeGameSessionsRouter({ logger: quiet }), 'GET', '/usage');
    expect(r.status).toBe(503);
  });
});
