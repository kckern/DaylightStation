/**
 * ShoppingHarvester - Shopping Receipt Harvester
 *
...
import {
  ...
  getDefaultUsername,
  householdLoadFile
} from '#backend/_legacy/lib/io.mjs';
...
export async function extractReceiptData(email, retailerName, logger = defaultLogger) {
  // Dynamic import for AI gateway
  const { getAIGateway, systemMessage, userMessage } = await import('#backend/_legacy/lib/ai/index.mjs');
...
export default async function harvestShopping(logger, guidId, req) {
  ...
  try {
    const householdId = configService.getUserHouseholdId(username);
    const householdConfig = householdLoadFile(householdId, 'config');
    const shoppingConfig = householdConfig?.shopping || DEFAULT_CONFIG;

    if (!shoppingConfig.enabled) {
      throw new Error('Shopping harvester not enabled');
    }

    config = {
      timezone: shoppingConfig.timezone || DEFAULT_CONFIG.timezone,
      retailers: shoppingConfig.retailers || DEFAULT_CONFIG.retailers
    };
  } catch (error) {
