 ...
 * engine correctly." — which is exactly the environment this worker runs
 * in, since the whole point of putting Stockfish here is to keep its
 * movetime off the main thread. `isMainThread` is a plain, writable
 * property on the (thread-local, cached) `worker_threads` module, not a
 * getter, so flipping it for the duration of the synchronous `require()`
 * inside `initEngine()` steers it down the normal
 * `module.exports = <factory>` path instead. The flip only needs to bracket
 * that synchronous require — `initEngine()` returns a Promise for the
 * (genuinely async) WASM instantiation, so the value is restored before
 * anything awaits it.
 */
