...
import { IRequirementsPolicySource } from '#apps/requirements/ports/IRequirementsPolicySource.mjs';
...
function expression(raw, path = 'expression') {
  ...
  if (raw.reference) {
    const value = typeof raw.reference === 'string' ? raw.reference : raw.reference.requirement ?? raw.reference.requirement_id;
    return { kind: 'reference', requirementId: value, nodeId: path };
  }
...
export class YamlRequirementsPolicySource extends IRequirementsPolicySource {
  #load;
  constructor({ load }) {
    super();
    if (typeof load !== 'function') throw new Error('YamlRequirementsPolicySource requires load');
    this.#load = load;
  }
  ...
  async loadCandidate(householdId) {
    ...
    if (!raw) {
      const error = new Error('Requirements policy source is missing');
      error.name = 'ValidationError';
      error.code = 'POLICY_SOURCE_MISSING';
      throw error;
    }
    if (raw.schema !== 'daylight.requirements-policy/v1') {
      const error = new Error('Unsupported requirements policy schema');
      error.name = 'ValidationError';
    ...
    const candidate = {
      ...
      requirements: Object.fromEntries(Object.entries(raw.requirements ?? {}).map(([id, value]) => [id, {
        id,
...
export default YamlRequirementsPolicySource;
