 ...
 *
 * Location: backend/src/1_adapters/persistence/files/DataService.mjs
 */
...
const readYamlFile = (absolutePath) => {
  try {
    ...
    return loadYamlFromPath(absolutePath) || null;
  } catch (err) {
    return null;
  }
};
...
const writeYamlFile = (absolutePath, data) => {
  try {
    saveYamlToPath(absolutePath, data, { lineWidth: -1, quotingType: '"' });
    return true;
  } catch (err) {
    if (err.code === 'EACCES') {
...
const safeStatSync = (p) => getStats(p);
