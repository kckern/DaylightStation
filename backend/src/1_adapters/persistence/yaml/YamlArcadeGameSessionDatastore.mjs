 ...
 *
 * Layout under the household `gaming/arcade-game-sessions/` tree:
...
import { IArcadeGameSessionRepository } from '#apps/gaming/ports/IArcadeGameSessionRepository.mjs';
import { ArcadeGameSession } from '#domains/gaming/entities/ArcadeGameSession.mjs';

export class YamlArcadeGameSessionDatastore extends IArcadeGameSessionRepository {
  #configService; #logger;

  constructor({ configService, logger = console } = {}) {
    super();
    if (!configService) {
      throw new InfrastructureError('YamlArcadeGameSessionDatastore requires configService', {
  ...
  #root(householdId) { return this.#configService.getHouseholdPath('gaming/arcade-game-sessions', householdId); }
  ...
  async findCurrentForDevice(deviceId, { householdId = null } = {}) {
    ...
    try {
      return ArcadeGameSession.fromSnapshot(snapshot);
    } catch (error) {
      // A corrupt file must not wedge every future session on this device.
      this.#logger.warn?.('arcade.session.current_unreadable', { deviceId, error: error.message });
      return null;
    }
  }
  ...
  async findById(sessionId, { householdId = null, on = null } = {}) {
    ...
    return row ? ArcadeGameSession.fromSnapshot(row) : null;
  }
  ...
  async listForDeviceSince(deviceId, sinceIso, { householdId = null } = {}) {
    ...
    for (let t = since; t <= Date.now() + day; t += day) {
      ...
      for (const row of rows) {
        ...
        seen.add(row.id);
        try { out.push(ArcadeGameSession.fromSnapshot(row)); } catch { /* skip unreadable row */ }
      }
    }
...
export default YamlArcadeGameSessionDatastore;
