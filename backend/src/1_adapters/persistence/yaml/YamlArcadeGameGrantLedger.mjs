...
import { IArcadeGameGrantLedger } from '#apps/gaming/ports/IArcadeGameGrantLedger.mjs';

export class YamlArcadeGameGrantLedger extends IArcadeGameGrantLedger {
  #configService; #logger;

  constructor({ configService, logger = console } = {}) {
    super();
    if (!configService) {
      throw new InfrastructureError('YamlArcadeGameGrantLedger requires configService', {
  ...
  async forUserOn(userId, isoDate) {
    ...
    try {
      ...
      this.#logger.warn?.('arcade.grant.ledger_unreadable', { userId, isoDate, error: error.message });
...
export default YamlArcadeGameGrantLedger;
