 ...
 */
export class FleetArcadeGameSessionAnnouncer extends IArcadeGameSessionAnnouncer {
  #bus; #logger;

  constructor({ eventBus, logger = console }) {
    super();
    if (!eventBus?.broadcast) throw new Error('FleetArcadeGameSessionAnnouncer requires an eventBus with broadcast()');
    this.#bus = eventBus;
    this.#logger = logger;
  }
  ...
  #publish(session, state) {
    ...
    try {
      ...
      this.#logger.warn?.('arcade.fleet.publish_failed', { deviceId: session.deviceId, error: error.message });
    }
  }
}

export default FleetArcadeGameSessionAnnouncer;
