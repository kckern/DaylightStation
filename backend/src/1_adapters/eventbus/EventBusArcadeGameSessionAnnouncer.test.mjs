...
import { EventBusArcadeGameSessionAnnouncer } from './EventBusArcadeGameSessionAnnouncer.mjs';
...
describe('EventBusArcadeGameSessionAnnouncer', () => {
  it('publishes the system and its placement on start', async () => {
    ...
    expect(sent[0].topic).toBe('arcade-session:tv');
    expect(sent[0].payload).toMatchObject({
      event: 'arcade.session.started',
      system: 'gb',
    ...
    expect(sent[1]).toMatchObject({ topic: 'arcade-game-sessions', payload: { event: 'arcade.session.started' } });
  });
  ...
  it('repeats the placement on every progress tick', async () => {
    ...
    expect(sent[0].payload.event).toBe('arcade.session.progress');
  ...
  it('keeps playedMs cumulative and never a delta', async () => {
    ...
    expect(sent.filter((m) => m.topic === 'arcade-session:tv').map((m) => m.payload.playedMs))
  ...
  it('replays an open session to a client subscribing mid-game', async () => {
    ...
    await subscribe('client-1', ['arcade-session:tv']);
    expect(direct).toHaveLength(1);
    expect(direct[0]).toMatchObject({
      clientId: 'client-1',
      payload: {
        topic: 'arcade-session:tv', event: 'arcade.session.progress',
  ...
  it('replays all open sessions to the house-wide topic', async () => {
    ...
    await subscribe('client-2', ['arcade-game-sessions']);
    expect(direct.map((m) => m.payload.sessionId)).toEqual(['s1', 's2']);
    expect(direct.every((m) => m.payload.topic === 'arcade-game-sessions')).toBe(true);
  });
});
