 ...
 */
export class RoleBasedArcadeGameGrants extends IArcadeGameTimeGrant {
  #profileFor; #adminRoles; #logger;

  constructor({ profileFor, adminRoles = DEFAULT_ADMIN_ROLES, logger = console }) {
    super();
    if (typeof profileFor !== 'function') throw new Error('RoleBasedArcadeGameGrants requires profileFor()');
  ...
  async forSession(session) {
    ...
    try {
      ...
      this.#logger.warn?.('arcade.grant.profile_unavailable', { userId, error: error.message });
      return null;
    }
...
export default RoleBasedArcadeGameGrants;
