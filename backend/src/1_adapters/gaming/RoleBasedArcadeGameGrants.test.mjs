...
import { RoleBasedArcadeGameGrants } from './RoleBasedArcadeGameGrants.mjs';

const quiet = { warn() {} };
const build = (profiles) => new RoleBasedArcadeGameGrants({
...
describe('RoleBasedArcadeGameGrants', () => {
  ...
  it('never falls back to unlimited when the profile cannot be read', async () => {
    const g = new RoleBasedArcadeGameGrants({
  ...
  it('requires a profile lookup rather than guessing', () => {
    expect(() => new RoleBasedArcadeGameGrants({})).toThrow(/profileFor/);
  });
});
