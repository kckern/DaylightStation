 ...
 * from outside. This is the only file in the arcade-game-session slice that knows
 ...
 */
import { IArcadeGameObservationSource } from '#apps/gaming/ports/IArcadeGameObservationSource.mjs';
...
export class RetroArchArcadeGameObservationSource extends IArcadeGameObservationSource {
  ...
  constructor({
    ...
    super();
    if (!kioskClient?.command) throw new Error('RetroArchArcadeGameObservationSource requires a kiosk client');
    if (!packageName) throw new Error('RetroArchArcadeGameObservationSource requires packageName');
  ...
  async observe(deviceId, { expectedContent = null } = {}) {
    ...
    if (foreground === null) {
      ...
      if (!focused.available) {
        ...
        return { ...base, state: ArcadeGameSessionState.UNKNOWN, loaded: null, content: null, channel: 'none' };
      }
    ...
    if (foreground !== this.#package) {
      ...
      if (misses < this.#missesBeforeGone) {
        return { ...base, state: ArcadeGameSessionState.UNKNOWN, loaded: null, content: null, channel: presenceChannel };
      }
      // Sustained absence: nothing is loaded on this device.
      return { ...base, state: ArcadeGameSessionState.PAUSED, loaded: false, content: null, channel: presenceChannel };
    }
    ...
    if (cpu === null) {
      ...
      if (presenceChannel === 'adb') {
        return {
          ...base, ...identity, state: ArcadeGameSessionState.UNKNOWN, loaded: true,
          channel: 'adb', degraded: true,
        };
      }
      return {
        ...base, ...identity, state: ArcadeGameSessionState.PLAYING, loaded: true,
    ...
    if (cpu.pid === null) {
      ...
      this.#cpu.delete(deviceId);
      return { ...base, state: ArcadeGameSessionState.PAUSED, loaded: false, content: null, channel: 'adb' };
    }
    ...
    if (!previous || previous.pid !== cpu.pid) {
      return { ...base, ...identity, state: ArcadeGameSessionState.UNKNOWN, loaded: true, channel: 'adb' };
    }

    const delta = cpu.ticks - previous.ticks;
    const state = delta > IDLE_TICK_THRESHOLD ? ArcadeGameSessionState.PLAYING : ArcadeGameSessionState.PAUSED;
  ...
  async #currentIdentity(expectedContent) {
    ...
    if (expectedContent?.contentId && expectedContent.contentId !== content?.contentId
      ...
      this.#logger.warn?.('arcade.content.intent_mismatch', {
        loadId: current.file,
  ...
  async #foregroundApp() {
    try {
      ...
      this.#logger.debug?.('arcade.observe.kiosk_failed', { error: error.message });
      return null;
    }
  }
  ...
  async #focusedApp() {
    ...
    try {
      ...
      this.#logger.debug?.('arcade.observe.adb_focus_failed', { error: error.message });
  ...
  async #cpuTicks() {
    ...
    try {
      ...
      this.#logger.debug?.('arcade.observe.adb_failed', { error: error.message });
      return null;
    }
  }
}

export default RetroArchArcadeGameObservationSource;
