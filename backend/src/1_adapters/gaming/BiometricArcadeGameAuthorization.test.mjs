...
import { BiometricArcadeGameAuthorization } from './BiometricArcadeGameAuthorization.mjs';

const quiet = { info() {}, warn() {} };
const build = (requestUnlock) => new BiometricArcadeGameAuthorization({
  biometricGateway: { requestUnlock }, logger: quiet,
});

describe('BiometricArcadeGameAuthorization', () => {
  ...
  it('requires a gateway rather than silently doing nothing', () => {
    expect(() => new BiometricArcadeGameAuthorization({})).toThrow(/biometric gateway/);
  });
});
