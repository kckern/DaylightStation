import { IArcadeGameTimeGrant } from '#apps/gaming/ports/IArcadeGameTimeGrant.mjs';
 ...
 */
export class LedgerArcadeGameTimeGrants extends IArcadeGameTimeGrant {
  #ledger; #isAdmin; #today; #logger;

  constructor({ ledger, isAdmin, today = () => new Date().toISOString().slice(0, 10), logger = console }) {
    super();
    if (!ledger?.forUserOn) throw new Error('LedgerArcadeGameTimeGrants requires a grant ledger');
    if (typeof isAdmin !== 'function') throw new Error('LedgerArcadeGameTimeGrants requires isAdmin()');
    this.#ledger = ledger;
  ...
  async forSession(session) {
    ...
    try {
      ...
      this.#logger.warn?.('arcade.grant.admin_check_failed', { userId, error: error.message });
    }
    ...
    try {
      ...
      this.#logger.warn?.('arcade.grant.ledger_failed', { userId, day, error: error.message });
      return null;
    }
  }
}

export default LedgerArcadeGameTimeGrants;
