...
import { LedgerArcadeGameTimeGrants } from './LedgerArcadeGameTimeGrants.mjs';

const quiet = { warn() {} };
const build = (grantedMs, isAdmin = async () => false) => new LedgerArcadeGameTimeGrants({
  ledger: { forUserOn: async () => ({ grantedMs, entries: [] }) },
  isAdmin, today: () => '2026-09-11', logger: quiet,
});

describe('LedgerArcadeGameTimeGrants', () => {
  ...
  it('gives no grant when the ledger cannot be read', async () => {
    const g = new LedgerArcadeGameTimeGrants({
