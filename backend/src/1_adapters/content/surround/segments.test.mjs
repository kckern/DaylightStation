...
import { toSpans, withOffsets } from './segments.mjs';
...
describe('toSpans', () => {
  ...
  it('takes explicit spans verbatim, so a gap between segments survives', () => {
    expect(toSpans({ spans: [[12.4, 121.0], [128.6, 275.2]], count: 2 })).toEqual([
      { start: 12.4, end: 121.0 }, { start: 128.6, end: 275.2 }
    ]);
  });

  it('pads to count so a segment with no timing keeps its position', () => {
...
describe('withOffsets', () => {
  it('lays segments end to end on a sounding-time rail, skipping dead time', () => {
  ...
  it('gives an untimed segment zero duration and does not advance the rail', () => {
  ...
  // isn't the degenerate segment's own duration (that's the easy half); it's
  // whether it quietly eats into or grants width on the rail, which would
  // misplace every segment after it.
  it('gives an end-before-start span zero duration and does not move the following segment', () => {
    const out = withOffsets([{ start: 0, end: 5 }, { start: 20, end: 10 }, { start: 50, end: 60 }]);
    expect(out[1].duration).toBe(0);
    expect(out[2].offset).toBe(5);
  });

  it('gives an end-equals-start span zero duration and does not move the following segment', () => {
    const out = withOffsets([{ start: 0, end: 5 }, { start: 10, end: 10 }, { start: 50, end: 60 }]);
    expect(out[1].duration).toBe(0);
    expect(out[2].offset).toBe(5);
  });

  it('gives a start with no end zero duration and does not move the following segment', () => {
