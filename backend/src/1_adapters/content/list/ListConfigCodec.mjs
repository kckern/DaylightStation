// Persistence compatibility codec for legacy and canonical list shapes.

// Internal, non-serialized provenance. Enumerable symbols survive object
// spread in application edits while YAML/JSON encoders ignore symbol keys.
const AUTHORED_TITLE = Symbol.for('daylight.list-item-authored-title');

function preserveAuthoredTitle(result, source) {
  if (source?.title != null) {
    Object.defineProperty(result, AUTHORED_TITLE, { value: true, enumerable: true });
  }
  return result;
}
...
export function normalizeListItem(item) {
  ...
  // A distinct admin label can coexist with an authored title. Retain it so
  // editing the label never silently renames the underlying authored item.
...
export function denormalizeItem(item) {
  ...
  // Preserve authored title/action-key items in that shape. Legacy
  // label/input items still round-trip as label/input, including normalized
  // legacy items whose derived title equals their original label.
  if (!item[AUTHORED_TITLE]) {
    if (result.title && !result.label) result.label = result.title;
    delete result.title;
  } else if (result.label === result.title) {
    // A redundant label was derived from a legacy-compatible read; authored
    // title remains the canonical stored field.
    delete result.label;
  }
...
export const ListConfigCodec = Object.freeze({ normalizeListConfig, serializeListConfig });
