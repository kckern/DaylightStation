import { IArcadeGameOverlay } from '#apps/gaming/ports/IArcadeGameOverlay.mjs';
 ...
 */
export class FullyKioskArcadeGameOverlay extends IArcadeGameOverlay {
  ...
  constructor({ clientsByDevice, httpClient = null, gravity = 80, logger = console }) {
    super();
    if (!clientsByDevice) throw new Error('FullyKioskArcadeGameOverlay requires clientsByDevice');
  ...
  async arm(deviceId, url) {
    ...
    if (!client) {
      ...
      this.#logger.debug?.('arcade.overlay.not_declared', { deviceId });
      return false;
    }
    if (!(await this.#resolves(url))) {
      this.#logger.error?.('arcade.overlay.url_unreachable', {
  ...
  async #resolves(url) {
    ...
    try {
      ...
      this.#logger.debug?.('arcade.overlay.verify_failed', { url, error: error.message });
      return false;
    }
  }
}

export default FullyKioskArcadeGameOverlay;
