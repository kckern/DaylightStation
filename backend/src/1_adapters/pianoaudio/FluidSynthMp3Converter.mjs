...
import { deleteFile, ensureDir, fileExists, getStats, renameFile } from '#system/utils/FileIO.mjs';
...
export class FluidSynthMp3Converter extends IMidiConverter {
  ...
  constructor({ soundfontPath, sourceDir, destDir, scratchDir = '/tmp/pianoaudio', logger = console, execFile = execFileAsync }) {
    super();
    if (!soundfontPath) throw new Error('FluidSynthMp3Converter requires soundfontPath');
    if (!sourceDir || !destDir) throw new Error('FluidSynthMp3Converter requires sourceDir and destDir');
  ...
  async convertRecording({ recordingId } = {}) {
    ...
    const mp3Path = this.#resolve(this.#destDir, mirrorRelForMidiRel(recordingId, 'mp3'));
    if (!midiPath || !mp3Path) throw new Error('Invalid MIDI recording id');
