...
export class YamlCardLadderStore {
  ...
  #load(base, parse, empty, context) {
    ...
    try {
      ...
      this.#logger.error?.('school.card-ladder.store-corrupt', { ...context, error: error.message });
  ...
  writeTuning(userId, pkg, tuning) {
    ...
    if (!current.file) throw new InfrastructureError(`cannot resolve card-ladder files for ${userId}`, { code: 'UNKNOWN_USER' });
    if (current.state === 'corrupt') {
      throw new DomainInvariantError(`card-ladder tuning for '${userId}' is corrupt — refusing to overwrite it`, { code: 'CARD_LADDER_TUNING_CORRUPT' });
    }
    const next = {
  ...
  transact(userId, pkg, day, fn) {
    ...
    if (!status.file || !dayFile.file) throw new InfrastructureError(`cannot resolve card-ladder files for ${userId}`, { code: 'UNKNOWN_USER' });
    if (status.state === 'corrupt' || dayFile.state === 'corrupt') {
      throw new DomainInvariantError(`card-ladder status for '${userId}' is corrupt — refusing to overwrite it`, { code: 'CARD_LADDER_STATUS_CORRUPT' });
    }
    const next = fn({ status: structuredClone(status.value), dayFile: structuredClone(dayFile.value) });
    if (next?.status?.schema !== STATUS_SCHEMA_V3 || next?.dayFile?.schema !== DAY_SCHEMA) throw new TypeError('card-ladder transaction returned an invalid shape');
...
export default YamlCardLadderStore;
