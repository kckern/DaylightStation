...
describe('HmacSchoolReelGrantIssuer', () => {
  it('binds a short-lived launch to its learner, unit, reel, and revision', () => {
    ...
    const token = issuer.issue({ learnerId: 'milo', unitId: 'language-reel-10', reelId: '10', revision: 'rev-a' });
    expect(issuer.verify(token, { learnerId: 'milo', unitId: 'language-reel-10', reelId: '10' }).ok).toBe(true);
    expect(issuer.verify(token, { learnerId: 'milo', unitId: 'language-reel-11', reelId: '10' }).reason).toBe('unitId');
    now += 2 * 60 * 60 * 1000;
