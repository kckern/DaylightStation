...
describe('HmacSchoolCubeGrantIssuer', () => {
  it('binds the token to its learner, course, and revision', () => {
    ...
    const token = issuer.issue({ learnerId: 'learner3', unitId: 'cube-1', courseId: 'beginner-v1', revision: 1 });
    expect(issuer.verify(token, { learnerId: 'learner3', courseId: 'beginner-v1' }).ok).toBe(true);
    expect(issuer.verify(token, { learnerId: 'learner4' }).reason).toBe('learnerId');
