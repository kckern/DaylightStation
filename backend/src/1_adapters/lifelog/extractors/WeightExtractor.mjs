...
/**
 * Weight metrics extractor
 * @implements {ILifelogExtractor}
 */
export class WeightExtractor extends ILifelogExtractor {
  get source() {
    return 'weight';
  }
  ...
  summarize(entry) {
    ...
    const lines = ['WEIGHT METRICS:', `  Current: ${entry.lbs}lbs`];
    ...
    if (entry.trend7day !== undefined)
      lines.push(`  7-day trend: ${formatTrend(entry.trend7day)}lbs`);
    if (entry.trend14day !== undefined)
      lines.push(`  14-day trend: ${formatTrend(entry.trend14day)}lbs`);
    if (entry.calorieBalance !== undefined)
      lines.push(`  Calorie balance: ${entry.calorieBalance} calories`);
