/**
 * Todoist Lifelog Extractor
 *
 * Extracts task activity from todoist.yml (date-keyed structure)
 * Lifelog contains: tasks CREATED and COMPLETED on each date
 *
 * @module applications/lifelog/extractors
 */

import { ILifelogExtractor, ExtractorCategory } from '#apps/lifelog/ports/ILifelogExtractor.mjs';

/**
 * Todoist task activity extractor
 * @implements {ILifelogExtractor}
 */
export class TodoistExtractor extends ILifelogExtractor {
  get source() {
    return 'todoist';
  }
   ...
   * @param {Object} data - Full todoist.yml data (date-keyed: { '2025-12-30': [...], ... })
   * @param {string} date - Target date 'YYYY-MM-DD'
   * @returns {Object|null} Extracted data or null
   */
  extractForDate(data, date) {
    // Handle both old format (array of open tasks) and new format (date-keyed activity)
    if (Array.isArray(data)) {
      // Old format - open tasks only, no activity dates
      return null;
    }
...
export const todoistExtractor = new TodoistExtractor();

export default TodoistExtractor;
