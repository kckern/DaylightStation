...
/**
 * Gmail activity extractor
 * @implements {ILifelogExtractor}
 */
export class GmailExtractor extends ILifelogExtractor {
  get source() {
    return 'gmail';
  }
  ...
  extractForDate(data, date) {
    ...
    return {
      ...
      received: dayMessages.filter(
        (m) => m.category === 'received' || (!m.isSent && m.category !== 'sent')
      ),
      total: dayMessages.length,
    };
  }
  ...
  summarize(entry) {
    ...
    if (entry.received.length) {
      lines.push(
        `  Received ${entry.received.length} important email${entry.received.length > 1 ? 's' : ''} (still in inbox):`
      );
      entry.received.slice(0, 5).forEach((m) => {
        const sender = m.from?.split('<')[0]?.trim() || m.from || 'Unknown';
        const snippet = m.snippet
          ? ` - ${m.snippet.substring(0, 80)}${m.snippet.length > 80 ? '...' : ''}`
          : '';
