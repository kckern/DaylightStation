...
describe('decideRecovery', () => {
  ...
  it.each([
    ...
    ['replaces a decoder-rejected rendition with an eligible candidate', [observation({ failure: { kind: 'decoder' } })], ledger, [{ ...original, renditionId: 'safe-h264' }], 1000, { action: 'replace', reason: 'decoder-rejected', renditionId: 'safe-h264' }],
  ...
  it('applies the sixty-second healthy reset before evaluating the incident cap but retains the rolling cap', () => {
    ...
    expect(decideRecovery({ observations: [observation({ observedAt: 60000, failure: { kind: 'decoder' } })], ledger: prior, candidates: [{ ...original, renditionId: 'safe-h264' }], now: 61000 }))
