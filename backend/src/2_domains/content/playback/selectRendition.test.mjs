...
describe('selectRendition', () => {
  it.each([
    ...
    ['treats an unavailable readiness result as explicit negative evidence', original, { readinessByRendition: { 'original-h264': 'unavailable' } }, admittedCapacity, { kind: 'prepare', reason: 'no-compatible-rendition' }],
  ...
  it('does not require probe capacity for an already validated assessed rendition', () => {
    expect(selectRendition({ candidates: [original], client, tracks,
      evidence: { assessmentTriggered: true, readinessByRendition: { 'original-h264': 'validated' } }, capacity: noProbeCapacity }))
      .toEqual({ kind: 'selected', renditionId: 'original-h264' });
  });
});
