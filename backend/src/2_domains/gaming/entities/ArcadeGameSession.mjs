...
import { ArcadeGameSessionState, assertArcadeGameSessionState, accrues } from '../value-objects/ArcadeGameSessionState.mjs';
...
export const ArcadeGameSessionStatus = Object.freeze({
  PENDING: 'pending',
  ACTIVE: 'active',
  ENDED: 'ended',
});

export const ArcadeGameSessionEndReason = Object.freeze({
  QUIT: 'quit',
  SWITCHED: 'switched',
  EXPIRED: 'expired',
  LOST: 'lost',
...
const END_REASONS = new Set(Object.values(ArcadeGameSessionEndReason));
...
export class ArcadeGameSession {
  ...
  constructor({
    ...
    status = ArcadeGameSessionStatus.PENDING,
  ...
  isActive() { return this.#status === ArcadeGameSessionStatus.ACTIVE; }
  isEnded() { return this.#status === ArcadeGameSessionStatus.ENDED; }
  ...
  observe({ state, observedAt, confidenceMs = 0, controllers = null }) {
    if (this.#status === ArcadeGameSessionStatus.ENDED) {
      throw new DomainInvariantError('Cannot observe an ended play session', {
        code: 'ARCADE_SESSION_ENDED', sessionId: this.#id,
      });
    }
    assertArcadeGameSessionState(state);
    ...
    let started = false;
    if (this.#status === ArcadeGameSessionStatus.PENDING && accrues(state)) {
      this.#status = ArcadeGameSessionStatus.ACTIVE;
  ...
  reconcilePlayedMs(playedMs, { confidenceMs = 0 } = {}) {
    ...
    this.#status = ArcadeGameSessionStatus.ENDED;
  ...
  static fromSnapshot(snapshot) {
    return new ArcadeGameSession(snapshot);
  }
}

export default ArcadeGameSession;
