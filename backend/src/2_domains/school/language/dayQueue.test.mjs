...
describe('graduation', () => {
  it('promotes yesterday\'s work one rung', () => {
    ...
    const queue = build({ log, day: 2, dailyLimit: 1, admission: [] });
  ...
  it('does NOT promote the same day — one rung per day is the whole method', () => {
    ...
    const log = [ev(1, 'repetition', 1)];
    const queue = build({ log, day: 1, dailyLimit: 1, admission: [] });
    // Today's own sentence may be PRACTISED at dictation (the warm-up fill);
    // what must not happen is dictation CREDIT on the day it was shadowed.
    expect(queue.some((e) => e.rung === 'dictation' && !e.practice)).toBe(false);
  });
  ...
  it('drops a sentence that climbed past this rung on an earlier day', () => {
    const log = [ev(1, 'repetition', 1), ev(1, 'dictation', 2)];
    const queue = build({ log, day: 3, dailyLimit: 1, admission: [] });
  ...
  it('retires a sentence that has climbed the whole ladder', () => {
    ...
    expect(build({ log, day: 5, dailyLimit: 1, admission: [] })).toEqual([]);
  });
  ...
  it('uses the EARLIEST clearing so a retry cannot stall the ladder', () => {
    ...
    const queue = build({ log, day: 7, dailyLimit: 1, admission: [] });
    expect(queue).toEqual([{ seq: 1, rung: 'dictation', done: false }]);
  });
});
...
describe('capability degradation', () => {
  it('graduates across a rung the device cannot perform', () => {
    ...
    const queue = build({ log, day: 3, dailyLimit: 1, admission: [], capabilities: noMic });
  ...
  it('never queues a rung the device cannot perform', () => {
    ...
    const queue = build({ log, day: 2, dailyLimit: 1, admission: [], capabilities: latinOnly });
...
describe('undated legacy evidence', () => {
  ...
  it('counts an undated event as cleared before any real day', () => {
    const queue = build({ log: [legacy(1)], day: 1, dailyLimit: 1, admission: [] });
    expect(queue).toEqual([{ seq: 1, rung: 'interpretation', done: false }]);
  });
  ...
  it('retires a legacy sentence once its final rung is cleared', () => {
    const queue = build({
      log: [legacy(1), ev(1, 'interpretation', 1)], day: 2, dailyLimit: 1, admission: [],
    });
    expect(queue).toEqual([]);
  });
  ...
  it('places a whole legacy import at the right rung without duplicates', () => {
    ...
    // Oldest first, capped at the day's limit; 3, 4 and 5 stay owed for later.
    expect(queue.filter((e) => e.rung === 'interpretation').map((e) => e.seq)).toEqual([1, 2]);
...
describe('sentences with no audio', () => {
  ...
  it('never queues an unplayable sentence as a graduate', () => {
    ...
    const queue = build({ log, day: 2, dailyLimit: 1, admission: [], playable });
...
describe('warm-up fill', () => {
  ...
  it('holds the same number on every rung when the pipeline does not match the limit', () => {
    // Found live 2026-09-12: a first day run at five a day, then an enrollment
    // of three a day, served 3 repetitions, 8 dictations, 1 recording and no
    // interpretation. Every rung holds exactly the day's limit.
    const log = [1, 2, 3, 4, 5].map((seq) => ({ day: 1, seq, rung: 'repetition' }));
    const dayTwo = build(log, 2);
    const perRung = Object.fromEntries(CHAIN.map((rung) => [rung, dayTwo.filter((e) => e.rung === rung).length]));
    expect(perRung).toEqual({ repetition: 3, dictation: 3, recording: 3, interpretation: 3 });
    // Oldest graduates first; the two beyond the limit are owed, not lost.
    expect(dayTwo.filter((e) => e.rung === 'dictation' && !e.practice).map((e) => e.seq)).toEqual([1, 2, 3]);
    completeAll(log, 2, dayTwo);
    const dayThree = build(log, 3);
    expect(dayThree.filter((e) => e.rung === 'dictation' && !e.practice).map((e) => e.seq)).toEqual([4, 5, 6]);
    for (const rung of CHAIN) expect(dayThree.filter((e) => e.rung === rung)).toHaveLength(LIMIT);
  });

  it('keeps a rung the same size as its steps are finished', () => {
    // Finishing a capped graduate must not pull the next owed one in behind it.
    const log = [1, 2, 3, 4, 5].map((seq) => ({ day: 1, seq, rung: 'repetition' }));
    log.push({ day: 2, seq: 1, rung: 'dictation' });
    const dictation = build(log, 2).filter((e) => e.rung === 'dictation');
