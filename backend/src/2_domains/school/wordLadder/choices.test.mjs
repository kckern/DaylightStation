...
describe('choices', () => {
  ...
  it('the cue is one English bundle: the gloss, plus whether a picture and gloss audio exist', () => {
    expect(cueFor(gawi, { image: false, glossAudio: false }, 's')).toEqual({ type: 'english', text: 'Scissors', image: false, audio: false });
    expect(cueFor(gawi, { image: true, glossAudio: true }, 's')).toEqual({ type: 'english', text: 'Scissors', image: true, audio: true });
    expect(cueFor(gawi, { image: true }, 's')).toEqual({ type: 'english', text: 'Scissors', image: true, audio: false });
    expect(cueFor(gawi, { glossAudio: true }, 's')).toEqual({ type: 'english', text: 'Scissors', image: false, audio: true });
  });
  it('the bundle is deterministic: the seed changes nothing', () => {
    ...
    expect(cueFor(gawi)).toEqual({ type: 'english', text: 'Scissors', image: false, audio: false });
  });
