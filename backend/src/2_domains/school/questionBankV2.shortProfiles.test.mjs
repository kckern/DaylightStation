...
describe('short worksheet profiles', () => {
  it('issues the requested three-question lower and five-question upper variants', () => {
    expect(issueWorksheet({ bank, learnerId: 'learner3', enrollmentId: 'e1', lessonId: 'd1', profile: 'lower-3', seed: 'a' }).items).toHaveLength(3);
    expect(issueWorksheet({ bank, learnerId: 'learner4', enrollmentId: 'e2', lessonId: 'd1', profile: 'upper-5', seed: 'b' }).items).toHaveLength(5);
  });
});
