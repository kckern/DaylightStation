...
export class RequirementDefinition {
  constructor({ id, schemaVersion, subjectKinds, periodKinds, expression, progress = null, reasonLabels = {} }) {
    this.id = requireNamespacedId(id, 'requirement.id');
    this.schemaVersion = requirePositiveInteger(schemaVersion, 'requirement.schemaVersion');
    ...
    if (!this.subjectKinds.length || this.subjectKinds.some(kind => !SUBJECT_KINDS.includes(kind))) {
      fail('Requirement subject kinds are invalid', 'INVALID_SUBJECT_KINDS', 'subjectKinds');
    }
    if (!this.periodKinds.length || this.periodKinds.some(kind => !PERIOD_KINDS.includes(kind))) {
      fail('Requirement period kinds are invalid', 'INVALID_PERIOD_KINDS', 'periodKinds');
    }
    if (!expression || typeof expression !== 'object') fail('Requirement expression is required', 'EXPRESSION_REQUIRED', 'expression');
