...
import { RequirementDefinition } from '../definitions/RequirementDefinition.mjs';
...
function validateExpression(node, graph, requirement, path, bindings = new Map([['subject', requirement.subjectKinds], ['period', requirement.periodKinds]])) {
  if (!node || !NODE_KINDS.has(node.kind)) fail('Unsupported requirement expression', 'UNSUPPORTED_EXPRESSION', path);
  ...
  if (node.kind === 'reference') {
    const referenced = graph.requirements.get(node.requirementId);
    if (!referenced) fail('Unknown requirement reference', 'UNKNOWN_REQUIREMENT', path);
    if (requirement.subjectKinds.some(kind => !referenced.subjectKinds.includes(kind))
      || requirement.periodKinds.some(kind => !referenced.periodKinds.includes(kind))) {
      fail('Requirement reference has an incompatible subject/period contract', 'INCOMPATIBLE_REQUIREMENT_REFERENCE', path);
    }
  }
  if (node.kind === 'count') {
  ...
  if (node.kind === 'count') {
    ...
    validateExpression(node.where, graph, requirement, `${path}/where`, new Map([...bindings, [node.as, [memberKind]]]));
  } else {
    walkChildren(node, (child, index) => validateExpression(child, graph, requirement, `${path}/${index ?? child.kind}`, bindings));
  }
}
...
function detectCycles(requirements) {
  ...
  for (const [id, requirement] of requirements) {
    const refs = [];
    walk(requirement.expression, node => { if (node.kind === 'reference') refs.push(node.requirementId); });
    edges.set(id, refs);
  }
  const visiting = new Set();
  const visited = new Set();
  const visit = id => {
    if (visiting.has(id)) fail('Requirement reference cycle detected', 'REQUIREMENT_CYCLE', 'requirements', { requirementId: id });
  ...
  for (const id of requirements.keys()) visit(id);
  return edges;
}

export class PolicyGraph {
  constructor({ schemaVersion, policyRevision, digest, publishers, subjectSets, claimTypes, requirements, entitlements, activatedAt, dependencyGraph }) {
  ...
  static create(candidate, context = {}) {
    ...
    const requirements = new ReadonlyMap(asEntries(candidate.requirements).map(([id, value]) => [id, value instanceof RequirementDefinition ? value : new RequirementDefinition({ ...value, id })]));
    for (const requirement of requirements.values()) if (requirement.schemaVersion !== 1) fail('Unsupported requirement schema version', 'UNSUPPORTED_REQUIREMENT_SCHEMA', 'schemaVersion', { requirementId: requirement.id });
    const shell = { claimTypes, subjectSets, requirements, catalog };
    for (const requirement of requirements.values()) {
      validateExpression(requirement.expression, shell, requirement, requirement.id);
      if (requirement.progress) {
        const nodes = [];
        walk(requirement.expression, node => nodes.push(node));
        const basis = nodes.find(node => node.nodeId === requirement.progress.basisNodeId);
        ...
        if (!clearComparison && !clearCount) fail('Progress basis is missing or ambiguous', 'INVALID_PROGRESS_BASIS', 'progress', { requirementId: requirement.id });
      }
    }
    const dependencyGraph = detectCycles(requirements);
    ...
    for (const entitlement of entitlements.values()) if (!requirements.has(entitlement.requirementId)) fail('Entitlement references unknown requirement', 'UNKNOWN_REQUIREMENT', 'entitlements', { capabilityId: entitlement.capabilityId });

    return new PolicyGraph({
      schemaVersion, policyRevision, digest: candidate.digest, publishers: deepFreeze(publishers),
      subjectSets, claimTypes, requirements, entitlements, activatedAt: candidate.activatedAt,
