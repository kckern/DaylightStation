...
| `day.rows.committed` | info | `modules/Health/healthResources.js` (from `AddCombobox` pick) | a quick-added row put straight onto its day from the response. `{date, rows, shown}`; `shown: false` means the day was not loaded here (or already held the row) and the row waits for the refetch. |
