...
| **Life events** | During a check-in or retro, the coach spots one in the recent calendar [EXISTS] | "Oh right, that did change things" | "Your calendar had 'Moving day' on the 20th. Want me to note that as a life event?" (at most two) | Says yes or no | `suggest_life_events` finds candidates (read-only); `add_life_event` records only what she confirms, with the calendar item's date and summary kept as its signal (so a renamed event is not offered again). Past events only |
