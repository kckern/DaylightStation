# Content Adapters: Singing and Narrated

Content adapters bridge between abstract content IDs (like `singing:hymn/123`) and local files on disk (YAML metadata + audio files). They are the core abstraction that lets DaylightStation serve hymns, scripture, talks, karaoke, poetry readings, or any other text-with-audio content without code changes.

## Architecture Overview

The singing and narrated adapters know nothing about hymns, scripture, or talks. They understand *collections* — directories of numbered YAML files with corresponding media. What makes `hymn/` a hymn collection vs a karaoke collection is entirely determined by the content you put in the directory and the optional `manifest.yml` you write.

Both adapters share the same contract:

| Method | Purpose |
|--------|---------|
| `canResolve(id)` | Claim IDs by prefix (`singing:` or `narrated:`) |
...
- **Singing** = participatory (lyrics displayed for sing-along, centered text, verse-by-verse)
- **Narrated** = follow-along (text scrolls as audio plays, left-aligned, paragraph-based)

A user could put poetry in the singing adapter if they wanted centered verse display, or put hymn lyrics in the narrated adapter for a read-along experience. The adapter choice determines how the frontend renders the content.
...
| File | Purpose |
|------|---------|
| `backend/src/1_adapters/content/singing/SingingAdapter.mjs` | Singing adapter implementation |
| `backend/src/1_adapters/content/singing/manifest.mjs` | Singing adapter metadata and config schema |
| `backend/src/1_adapters/content/narrated/NarratedAdapter.mjs` | Narrated adapter implementation |
| `backend/src/1_adapters/content/narrated/manifest.mjs` | Narrated adapter metadata and config schema |
| `backend/src/1_adapters/content/narrated/resolvers/scripture.mjs` | Scripture path resolver |
...
| ID | Resolves to |
|----|-------------|
| `singing:hymn/1` | `dataPath/hymn/0001-*.yml` + `mediaPath/hymn/0001-*.mp3` |
| `singing:primary/2` | `dataPath/primary/0002-*.yml` + `mediaPath/primary/0002-*.mp3` |
| `narrated:scripture/nt` | Resolved by scripture resolver to specific chapter |
| `narrated:talks/ldsgc202410/smith` | `dataPath/talks/ldsgc202410/smith.yml` |
...
For the **singing adapter**, `dataPath` points to `data/content/songs/` — so collections are `hymn/`, `primary/`, etc. For the **narrated adapter**, `dataPath` points to `data/content/` — so collections are `scripture/`, `talks/`, `poetry/`, etc.

The narrated adapter's `dataPath` is one level higher because narrated collections tend to have deeper nesting (scripture has volumes, text versions, and verse IDs).
...
**Singing adapter** (no manifest needed):
```yaml
contentType: stanzas
style:
  fontFamily: serif
...
**Narrated adapter** (no manifest needed):
```yaml
...
| `resolver` | string | Name of a resolver module (narrated only — see Resolvers section) |
| `containerType` | string | Selection strategy hint: `sequential`, `watchlist` |
| `ambient` | boolean | Whether to play ambient background audio (narrated only) |
...
This tells the narrated adapter: use the `scripture` resolver for path resolution, render as verses with serif font, play ambient music, and use these text/audio version defaults per volume.

---

## Item YAML Format

Each content item is a YAML file containing metadata and text content. The structure varies slightly between singing and narrated.
...
The singing adapter reads `metadata.verses` as an array of arrays — each inner array is a stanza, each string is a line. `title` and `number` (or `hymn_num`) are used for display and search.

### Narrated items

The narrated adapter is more flexible. It tries these fields in order: `metadata.verses`, `metadata.content`, `metadata.paragraphs`, or treats the root as an array directly. This means narrated YAML can be structured differently per collection.
...
Both adapters read `duration` from YAML metadata. If absent, the singing adapter probes the audio file using `music-metadata` (`parseFile()`) to extract duration automatically. The narrated adapter falls back to `0` if no duration is specified.

---

## The Scripture Resolver

Resolvers are a narrated-adapter feature that transforms user-friendly paths into actual file lookups. The scripture resolver (`resolvers/scripture.mjs`) is the reference implementation.
...
The resolver returns both `textPath` and `audioPath` independently. This allows a collection to use one text version (e.g., `kjvf` — King James with footnotes) while playing a different audio recording (e.g., `nirv` — narrated in a different voice). The manifest `defaults` section configures this per volume.

---

## Bookmark Tracking and Playable Resolution

When a narrated content request is a container (like `scripture/nt`), the adapter doesn't just return the first item — it consults watch history to find where the user left off.
...
- Current: `narrated:scripture/nt/kjvf/25065`

Both are checked in `scriptures.yml` first, then falls back to `narrated.yml`.

### getItem() also uses selection

When you request `narrated:scripture/nt` via `getItem()`, instead of returning a container listing, it returns the actual next chapter as a fully-resolved item with content, audio URL, and metadata. This means any component that calls `getItem()` gets a playable result without needing to understand bookmarking.
...
### Example: Adding a karaoke collection (singing)
...
The item is now addressable as `singing:karaoke/1`.

### Example: Adding a poetry collection (narrated)

1. Create `data/content/poetry/` with YAML files
2. Create `media/audio/poetry/` with matching audio files
...
Items are addressable as `narrated:poetry/frost-road-not-taken`.
...
If you do need one, create a module in `backend/src/1_adapters/content/narrated/resolvers/` that exports an object with a `resolve(input, dataPath, options)` method, then reference it in your manifest:
