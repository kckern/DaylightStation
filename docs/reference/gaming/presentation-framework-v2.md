...
Each unordered material pair has exactly one visual interface owner. Authors declare the reviewed `inside -> outside` interface once; when the compiler encounters the same boundary from the reverse material cell, that side renders its ordinary material fill and leaves the transition to the owner. At three-material corners, passive contacts keep the local fill while active interface wedges are clipped to only the contacts they own. Declaring both directions is invalid catalog capability, because two otherwise correct autotiles would cut the same boundary twice.
...
Static scene promotion is not framework completion. The animation exit gate applies equally to actors and animated objects: every canonical actor/NPC/player/creature/item/object/prop/effect candidate has an explicit runtime-reviewed or deferred disposition; no deferred candidates remain for a release advertised as complete; decoded source coverage proves that every non-empty grid cell (aggregated across assets sharing a sheet) or complete freeform source is mapped rather than treating one preview frame as whole-sheet coverage; every reachable frame has decoded bounds and every clip has timing, scale, fixed-anchor, internal-grid-seam, and silhouette-stability QA; every locomoting actor declares and fulfills a complete `four-way` or `horizontal` facing scheme; authored facing provenance is explicit and cannot resolve through mirroring or a shared source clip; synthesized mirroring is catalog-owned; every in-place prop/effect proves a fixed world anchor and reviewed effect envelope; every interactable, destructible, or stateful object exposes stable states plus one-shot endpoint-checked transition clips; every one-shot actor, object, and effect state explicitly returns to another state or declares itself terminal; and control-simulation GIFs prove stationary idle, in-place animation, translated locomotion, airborne motion, and facing changes. QA reports separate actor, object, temporally animated actor/object, state-clip, and transition-clip counts so a catalog of named static poses cannot satisfy the motion gate. Layered actors additionally require exact frame geometry/state/timing registration across every body, wearable, tool, held-item, and mount source; reviewed transparent phases for intentionally blank cells; deterministic catalog-owned rig slots; representative mounted `qa_assemblies`; composite action/facing GIFs; and rigged control simulations. Animated terrain remains under synchronized topology animation QA, while sprite/object motion uses the state-machine gate.

The release exit criteria are therefore conjunctive, not interchangeable:

- tile, connector, height, and component topology passes exhaustive edge/corner/join renders;
- every adopted source is either fully semantically mapped or remains explicitly deferred, with zero partial mappings presented as complete;
- actors pass complete facing, authored-versus-mirrored direction provenance, ground-anchor, scale, locomotion, action-return/terminal, dynamic-rig assembly, composite action/facing, and control-simulation QA;
- every body/wearable/tool/mount overlay source maps all non-empty cells, explicitly marks and decodes registered blank phases, and proves exact state/facing/timing/anchor compatibility with its rig base;
- animated items, props, mechanisms, and effects pass temporal timing, stable-anchor, state endpoint, transition, and envelope QA, including animations not currently placed in a showcase scene;
- reports show zero runtime errors and zero deferred candidates before the framework is described as library-complete.
