...
`games.yml → arcade_sessions.stale_after_ms` when a surface has a deliberately
slower heartbeat.
...
Enforcement is explicitly configured at `games.yml → arcade_sessions.mode`.
...
The HTTP surface is available under `/api/v1/arcade-game-sessions`:
...
`GET /api/v1/arcade-game-sessions/placement` returns the whole table, `?system=` one of
