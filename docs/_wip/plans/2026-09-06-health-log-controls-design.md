...
Never ratify edits automatically. Stamp user provenance and manualFields only for nutrients actually corrected. Shared client/server arithmetic must agree to stored precision. Capture the ladder and its revision at gesture start; calorie/density saves require the same server revision. A configuration change returns a conflict and requires refreshed context before explicit rebase, never a silent change between preview and save.
