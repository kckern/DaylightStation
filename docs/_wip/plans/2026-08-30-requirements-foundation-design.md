# Requirements bounded context — foundation design

**Date:** 2026-08-30

**Status:** Foundation implemented 2026-08-30; producer/consumer migrations pending

**Scope:** Backend contracts, ownership, persistence, delivery, and verification

**Architecture authority:** [DDD reference](../../reference/core/layers-of-abstraction/ddd-reference.md), [domain guidelines](../../reference/core/layers-of-abstraction/domain-layer-guidelines.md), [application guidelines](../../reference/core/layers-of-abstraction/application-layer-guidelines.md), [adapter guidelines](../../reference/core/adapter-layer-guidelines.md), [API guidelines](../../reference/core/layers-of-abstraction/api-layer-guidelines.md), and [decision register](../../reference/core/layers-of-abstraction/decision-register.md)

---

## 1. Decision

Create `requirements` as a Level 1 shared bounded context. It receives authenticated,
typed assertions from existing producer contexts, evaluates household-authored policy,
and publishes current requirement state plus binary entitlement decisions for registered
consumers.
...
Requirements published language. Requirements never reaches into their repositories
or interprets their private records.

The event bus remains transport. It cannot establish truth. Only an authenticated
assertion command accepted by the Requirements application can change the projection.
Arbitrary event-bus messages cannot establish truth.

The implemented foundation lives under the corresponding `requirements` domain,
...
- Give unrelated producers and consumers one stable, system-wide vocabulary for facts,
  requirements, progress, and entitlements.
...
| **Requirement** | A named four-state policy expression over qualified assertions and other requirements. |
| **Evaluation** | The current state, optional progress, sanitized reasons, validity, and revision for a requirement instance. |
| **Entitlement** | A named binary capability whose decision is based on one requirement and an explicit failure posture. |
...
| **Transition** | A past-tense event emitted only when an already-materialized requirement state or entitlement decision genuinely changes. |
| **Policy graph** | The atomically activated aggregate containing subject sets, claim types, requirements, and entitlements. |
| **Source revision** | A publisher-monotonic revision for one assertion identity. It is distinct from the household revision. |
| **Household revision** | A monotonic sequence assigned to every accepted state-changing Requirements commit in one household. |

An assertion is evidence, not a decision. A requirement interprets evidence. An
entitlement turns one requirement state into one binary capability decision. A consumer
...
School / Fitness / chores / other producers
  private state machines
        ...
        v
authenticated Requirements assertion command
        |
        v
Requirements policy graph -> evaluations -> entitlement decisions
        |
...
`school.day.complete`; Requirements does not import School entities and rediscover that
...
- current requirement evaluations;
...
and FileIO exactly generic. Do not add requirement IDs, policy parsing, assertion
persistence, state evaluation, publisher authority, or entitlement knowledge to this
layer.

### `2_domains/requirements` — pure Level 1 shared domain
...
When implementation creates the folder, add `requirements` to **both** Level 1 hierarchy
tables in `domain-layer-guidelines.md` and `ddd-reference.md` in the same change, as
required by Decision D6. This design-only phase does not create the folder and therefore
does not edit those reference tables yet.

### `3_applications/requirements` — orchestration
...
All Requirements ports live here. The container receives concrete implementations from
...
Requirements ingress use case without moving the producer's translation policy into
composition.
...
ingress. Adapters translate; they do not decide whether a requirement is satisfied.
...
Own injected router/handler factories for requirement, entitlement, transition,
...
must not query Requirements state.
...
backend/src/
  ...
  3_applications/requirements/
    RequirementsContainer.mjs
    contracts/
    ports/
      IRequirementsPolicySource.mjs
      IRequirementsProjectionRepository.mjs
      IRequirementsTransitionRepository.mjs
      IRequirementsEventPublisher.mjs
      IRequirementsAdministrationAuthorizer.mjs
    usecases/

  1_adapters/requirements/
    config/YamlRequirementsPolicySource.mjs
    persistence/YamlRequirementsProjectionRepository.mjs
    persistence/YamlRequirementsTransitionRepository.mjs
    eventbus/RequirementsEventBusPublisher.mjs
    ingress/AuthenticatedRequirementsIngress.mjs

  4_api/v1/
    routers/requirements.mjs
    handlers/requirements/

  5_composition/modules/requirements.mjs
```

Names are directional guidance, not an implementation checklist. If the existing
repository conventions call for fewer files, the ownership boundaries still hold.
...
### 5.4 `RequirementDefinition` and expression tree

```text
RequirementDefinition
  id: namespaced RequirementId
  schemaVersion: positive integer
  subjectKinds: non-empty set<SubjectKind>
  periodKinds: non-empty set<PeriodKind>
...
`RequirementExpression` is a closed, recursively validated algebra. Each YAML node maps
to exactly one of these types:

| Node | Purpose |
|---|---|
| `claim` | Read one explicitly qualified boolean claim source. |
| `reference` | Reuse another requirement with compatible subject/period binding. |
...
PolicyGraph
  ...
  requirements: map<RequirementId, RequirementDefinition>
...
progress remains on the requirement evaluation.
...
StateObservation
  ...
  kind: requirement | entitlement
  key: requirement/capability + subject + period
  current: sanitized RequirementEvaluation | EntitlementDecision
  initial: boolean
...
not** emit `RequirementStateChanged` or `EntitlementDecisionChanged`. A new subscriber
...
- `RequirementStateChanged` is emitted only when the four-state value changes;
...
```text
(policyGraph, activeAssertions, requirementId, subject, period, now, timezone)
  -> RequirementEvaluation draft
```
...
| Requirement does not apply to the requested subject/period contract | `not_applicable` |
...
- a requirement may explicitly select one expression node as its progress basis;
...
An entitlement decision inherits the requirement's validity. When the boundary passes,
the application reevaluates even if no new assertion arrived.

### 6.6 Entitlement decision table

| Requirement state | Failure posture | Decision | Degraded |
|---|---|---|---|
...
the requirement does not govern that subject/period; it is not degraded evidence.

---

## 7. Household policy configuration

The household policy is the Requirements config. Under the repository's current
domain-first layout it lives at:

```text
data/household[-{hid}]/requirements/config.yml
```

This is the household `config/requirements.yml` concept named in the feature brief, but
it does not recreate the retired flat `household/config/` directory. Implementation adds
`requirements: 'requirements/config'` to `shared/contracts/householdConfig.mjs`; the
composition loader resolves it and injects a semantic policy loader/source.

### 7.1 Schema sketch

```yaml
schema: daylight.requirements-policy/v1
...
claim_types:
  ...
  chores.daily.attested:
    ...
    validity:
      actor_required: true
      accepted_actor_roles: [caregiver]
      must_fit_period: true
...
entitlements:
  piano.games:
    requirement: school.day-complete
    failure_posture: fail_closed

  fitness.cartoons:
    requirement: fitness.saturday-exercise
    failure_posture: fail_closed

  household.celebration:
    requirement: household.school-complete
...
5. subject sets are non-empty, homogeneous, duplicate-free, and resolve against the
   household roster;
6. claim, requirement, subject-set, and entitlement references exist;
...
10. the requirement-reference graph is acyclic;
11. every entitlement has exactly one requirement and a declared `fail_open` or
...
On a first boot with no valid graph to retain, Requirements reports
...
| `EvaluateRequirements` | Purely evaluate requested or affected requirement instances using supplied time/timezone, then materialize observations and genuine transitions. |
| `DecideEntitlements` | Apply the normative decision table to evaluations. |
| `GetCurrentRequirements` | Return subscriber-safe current evaluations/definitions with filters and revision. |
| `GetCurrentEntitlements` | Return current binary decisions with filters and revision. |
| `GetRequirementDiagnostics` | Enforce administrative access and return raw assertions/provenance. |
| `ReplayRequirementTransitions` | Return ordered publication envelopes after a revision cursor. |
| `ReconcileRequirementsState` | On startup, repair an interrupted commit/publication, reevaluate time validity, and resume delivery. |

`EvaluateRequirements` and `DecideEntitlements` can be internal collaborators as long as
...
All ports are abstract classes in `3_applications/requirements/ports/`.

```text
IRequirementsPolicySource
  loadCandidate(householdId) -> CandidatePolicyGraph

IRequirementsProjectionRepository
  load(householdId) -> RequirementsProjectionSnapshot?
  commitRevision(householdId, expectedRevision, nextSnapshot, transitionBatch)
    -> committed | revision_conflict

IRequirementsTransitionRepository
...
IRequirementsEventPublisher
  publish(publicationEnvelopes) -> void

IRequirementsAdministrationAuthorizer
...
6. Publish committed envelopes through `IRequirementsEventPublisher`.
7. Mark transition IDs published.

If the process stops after step 5, `ReconcileRequirementsState` republishes the unmarked
...
`YamlRequirementsPolicySource extends IRequirementsPolicySource`:
...
- reports source locations for administrative validation errors; and
- performs no requirement evaluation or fallback selection.
...
`YamlRequirementsProjectionRepository` and
`YamlRequirementsTransitionRepository` extend their application ports and use injected
FileIO capabilities. No raw `fs` or `path` import is allowed.

The domain-first household layout is:

```text
data/household[-{hid}]/requirements/
...
`RequirementsEventBusPublisher extends IRequirementsEventPublisher` wraps the generic
...
arbitrary topic and turn messages into assertions. Requirements ingress is a command
boundary, never a generic bus listener.

### Authenticated ingress

`AuthenticatedRequirementsIngress` is an inbound adapter. It authenticates a service,
...
such as `publisher: school`, `actor: parent`, or `roles: [caregiver]` is ignored or
...
One injected `createRequirementsRouter(deps)` may mount smaller resource handlers. The
...
|---|---|---|
| `GET` | `/api/v1/requirements` | Current declared requirements/evaluations filtered by subject/period. |
| `GET` | `/api/v1/requirements/:requirementId` | One current evaluation and safe definition. |
...
| `GET` | `/api/v1/requirements/transitions?afterRevision=...` | Ordered replay page plus next cursor/current revision. |

### Attestation and administration

| Method | Resource | Use case |
|---|---|---|
| `POST` | `/api/v1/requirements/attestations` | `ObserveManualAttestation`; actor comes from request context. |
| `DELETE` | `/api/v1/requirements/attestations/:assertionId` | Authorized retraction; source revision is required. |
| `POST` | `/api/v1/admin/requirements/policy/activate` | Validate and atomically activate the current candidate source. |
| `GET` | `/api/v1/admin/requirements/assertions` | Raw claim/provenance diagnostics. |
| `GET` | `/api/v1/admin/requirements/policy` | Active digest/revision and last candidate validation result. |

Route names may be adjusted to the repository's API vocabulary during implementation,
but the resources and access boundaries are normative.
...
|---|---:|---:|
| Declared safe requirement definitions | Read | Read |
| Requirement evaluations and progress | Read | Read |
...
semantic permission. HTTP handlers do not infer caregiver/admin status from payloads.
...
`5_composition/modules/requirements.mjs` owns construction:
...
2. Construct YAML policy, projection, transition, event-publisher, and authorization
   adapters.
3. Construct `RequirementsContainer` with only semantic dependencies.
...
enters `3_applications/requirements`, and no policy fallback, claim mapping, or event
...
```text
schema: daylight.requirements-event/v1
transitionId
householdRevision
occurredAt
kind
...
Requirement `school.day-complete` selects the `school` publisher explicitly.
...
real requirement/decision transition; it does not edit the Piano state directly.
...
the next boundary. If evidence expires while inside Saturday, the requirement becomes
indeterminate and the `fail_closed` entitlement is denied with `degraded: true`.

The consumer—not Requirements—chooses which cartoon rail, pause behavior, or explanatory
screen to show.

### 13.3 Role-aware chore attestation

A caregiver submits a manual attestation. The API ignores any actor/role in the body and
uses the authenticated request actor. `ObserveManualAttestation` asks
`IRequirementsAdministrationAuthorizer` for the attestation permission, then the domain
checks the claim type's `actorRequired` and `acceptedActorRoles: [caregiver]` invariant.
...
Composition injects Requirements query/replay dependencies into a screen-framework
...
Requirements publishes no `hide`, `disable`, `pause`, route, component, content ID, or
...
- requirement reference resolution and cycle rejection;
...
- D7 inheritance: every flagship Requirements adapter extends its live application port.
...
## 16. Implementation boundaries for a later plan

A later implementation plan may stage the work as domain, application, adapters, API,
composition, then opt-in producer/consumer migrations. The foundation is not complete
until the pure model, ports, persistence/recovery, access tiers, and architecture tests
exist together; an event-only prototype or a YAML evaluator without durable current
state is not this design.
...
Fitness, chore, or screen-framework behavior into the Requirements bounded context.
...
- Household revisions order Requirements commits; publisher source revisions order one
