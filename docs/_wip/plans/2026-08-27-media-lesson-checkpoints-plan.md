...
- Attempts are counted per item in **instance-scoped, bounded, TTL'd memory** — NOT "for the life of the request chain", which was the original wording here and is NOT implementable: each answer is a separate POST, so a request-scoped counter can only ever report `1`. Built as a map keyed `sessionId\0checkpointId`, LRU-bounded (200) with an 8h TTL, deleted on clear. The durable record is `attempts` on the `checkpoint_cleared` event. Do NOT add a second store, and do NOT let the client supply progress: a page posting `answered: [q1,q2]` beside one correct answer would open a three-question gate with one answer, undetectably — the exact skip this feature exists to prevent.
- **Losing that memory re-asks a checkpoint; it can never skip one.** That is the deliberate direction, and it is why per-item progress is NOT written to the event log: `checkpoint_cleared` is keyed by `checkpointId` and the reducer is first-write-wins, so an event per answered ITEM would let the FIRST item open the gate and the remaining questions would never be asked.
