...
| JOURNEY-QUEUE | Paused real movie → result More actions → Add to queue → same-title queue jump | RED at earlier menu junction: More actions clears query/resumes playback; Add to Queue menu unavailable. No synthetic bypass. | Task4 pending; queue-jump position assertion not reached and not accepted |
...
| TASK-2-REVIEW-FIXES | Known→unknown duration, stalled progress, seek capability, playback generations, actual-position relative seeks and mounted-source ownership | Unit RED for each boundary; full browser rapid Forward→Back exposed stale-position overshoot | Repairs through b587d2226; scoped review clean after3rounds; root focused15files151tests exit0. Final browser6passed1failed4.3min: old paused video restarted in cross-title journey; repair continues. Persistence timer cleanup also restores clean24file/247test exit |
| JOURNEY-CROSS-TITLE | Paused Disclosure Day → ordinary Arrival selection → actual new video | Earlier baselineGREEN; final b587d2226 run RED: old paused source emitted1play event. Logs place old restart after Pause and before Arrival selection, during late seek completion. | Task2 fixround4 pending; actual new-source playback alone does not satisfy no-old-source-restart assertion |
