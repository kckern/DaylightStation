...
|---|---|---|---|
| JOURNEY-M0-SAFETY | Actual detail/playing-item picker → unavailable Move → explicit Keep choice → cancel without dispatch | Idle5537 popup places Keep below viewport; active case first had two harness selection/parent assumptions, corrected without product changes | Active52074 GREEN10.1s on9a07: actual Arrival node retained/advancing through Back, ordinary Stop, zero device commands. Idle portal-only sizing repair has unit evidence; compiled ordinary-click validation pending |
...
| JOURNEY-SEARCH-RETRY-POINTER | Silent search → retryable failure → pointer Retry → real results | Old0c14580 never leaves Searching; compiled9a07 run22807 phone passes but desktop pointer blurs/clears input before second request | Root fresh round4 pointer/focus repair:2 true global/source Retry REDs→214 related GREEN and independent focused acceptance. New compiled browser verification pending; no whole search-story acceptance |
...
| JOURNEY-HLS-SERIAL | Full7 Disclosure HLS cases, actual branch read/mint | Earlier34664 first2 exceeded30s readiness; Plex transaction waits were confirmed separately | Bundled accepted0c run15499: all7 passed together in1.9min, including local aim, duration/bar, deep seek, pause/resume, rapid controls, focus and Stop; observed HLS sessions require successful ordinary-Stop responses. Does not prove the intermittent Plex transaction latency is repaired, physical-device parity, or full-story acceptance |
...
| JOURNEY-NATIVE-MP4 | Actual branch-qualified Arrival original MP4; full-content seek, focus, Stop | Previous global DASH baseline could not start reliably; dev39999 also lost app readiness in2 cases | Bundled accepted0c run96950: all7 passed together in1.2min with actual branch read/mint provenance. Covers Office peek/local aim, seek/bar, deep seek, pause/resume, forward/back, focus and Stop. No whole-story/device parity or HLS acceptance |
