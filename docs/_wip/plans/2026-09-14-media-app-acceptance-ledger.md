...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| PLAY.1a/AC1 | "Play now" on a search result, a browsed item, an item's details, a suggestion, or a recent item all play at the aim shown on screen. | Partial | `JOURNEY-ORDINARY-REMOTE-PLAY-ADD` verifies ordinary aimed Search Play. `JOURNEY-RESULT-MORE-PLAY-NOW` verifies result-More Play now to the explicitly aimed receiver in fork mode; default-mode tray discrepancy and other entry points remain unresolved/unverified. The Browse+Detail run on source `8b338aefd02cc6047b624898574fe06569f6c697` passed native playback/queue checks but Detail’s tray assertion failed. Sol traced it to session buffering despite forwarding native playback; a browser-side fix is pending, so this adds no story acceptance. |
| PLAY.1a/AC2 | The item starts now on that screen; whatever was queued after the old item stays queued (R1). | Partial | `JOURNEY-RESULT-MORE-PLAY-NOW`, source `d49abd2dcc720c892218b55b3474b02a77807fb3`, preview port 39601, explicit fork: actual new receiver visit/revision, Disclosure retained the same `queueItemId` and queue tail, advancing native playback, matching tray (1 passed, 33.9s). Follow-up `17602653f9657f1203f4fa465388c30ed30a5522` preview port 40679: explicit-fork case passed; default-mode case failed because the tray showed “Sent” instead of expected “Playing,” so default-mode behavior remains unverified. |
