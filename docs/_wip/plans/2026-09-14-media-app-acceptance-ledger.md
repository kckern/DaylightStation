...
| JOURNEY-REMOTE-CONTROLS | Actual receiver Pause/Resume/seek/Stop from the fleet Peek panel | The isolated 25.6s run on source `7d22a589c86408849101d22bff3af9630c42574d` verifies Pause/Resume and Stop only as described below. Its `ArrowRight` +5 absolute seek reached the asserted sender value while playback was advancing, so natural progress could satisfy the check; it is insufficient causal evidence of seek control. A subsequent real Forward 10 control run on that same source failed: native video did not reach the requested seek state within 20s. Reverse, drag, and Stop were not reached in that run. The runtime assertions have since been strengthened to exercise paused relative ±10, a track-position absolute seek, a distinct pointer drag, displayed time/native matching, resume, and Stop receipt; that revised runtime still needs execution against its clean built source. | Source `7d22a589c86408849101d22bff3af9630c42574d`, preview port 34351. Initial command `BASE_URL=http://127.0.0.1:34351 npx playwright test tests/live/flow/media/media-app-remote-controls.runtime.test.mjs --workers=1 --reporter=line`: 1 passed (25.6s), raw log `/tmp/daylight-controls-7d22a589c8640884-1789890786.log`; Pause/Resume changed actual native state and sender progress; Stop returned ready/null-current, paused/reset the native element, and preserved currentIndex plus ordered queue identities/content. Causal Forward 10 failure command used the same runtime file against source `7d22a589c86408849101d22bff3af9630c42574d`; 1 failed at native seek-state polling after 20s, raw log `/tmp/daylight-controls-seek-gap-7d22a589c8640884-1789891107.log`. Relative jump buttons beyond the revised but not-yet-run assertions, live media, the two-second deadline, offline delivery/receipt semantics and screen-off remain unverified; no full STEER story acceptance is claimed. |
