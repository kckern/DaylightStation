...
**Status:** In progress. **2 accepted stories / 13 accepted AC; 12 partial stories / 20 partial AC; 68 unverified stories / 255 unverified AC**. No component, test count or API response earns acceptance by itself.
...
| JOURNEY-SEARCH-SCOPES | FIND.2a configured All, Video and parent/child scopes across phone SearchMode, tablet and laptop dock | Initial run against source `18d032ae533731496ae686122fc1bf84bed5e3cc` ended 2 passed / 7 failed. On compiled source `e4904b6c8fe58c347a905763fe4331b85ec3c49f`, the nine-case run ended 4 passed / 5 failed because the phone's valid `Documentary Serie · Episode` kind subtitle was rejected by an overly narrow matcher and desktop requests had not yet been fixed. The focused phone run passed 3/3. On final compiled source `c21e90a0587e65799488a2b748ef1ebc75466980`, the nine-case run passed 7/9 with no skips: AC1 and AC3 passed on phone/tablet/laptop; phone AC2+AC4 passed; desktop AC2 reached kind matching and failed only because the matcher split `·` but the desktop renderer uses `•`. The two-case desktop follow-up with both separators passed the AC2 scoped results/kind assertions, then failed AC4 at reopening: All remained `aria-pressed=false` on both tablet and laptop. Phone AC4 remains green; desktop reset is not. | Initial preview `38317`, raw log `/tmp/daylight-search-scopes-18d032ae-1789893436.log`. Compiled preview `39171`: initial `e4904b6c` run `/tmp/daylight-search-scopes-e4904b6c-20260920-0849.log`; focused phone `e4904b6c` run `/tmp/daylight-search-scopes-phone-e4904b6c-20260920-0852.log`. Final preview `44059`: `BASE_URL=http://127.0.0.1:44059 npx playwright test tests/live/flow/media/media-app-search-scopes.runtime.test.mjs --workers=1 --reporter=line`, log `/tmp/daylight-scopes-final-c21e90a-20260920-0907.log`; focused desktop rerun `/tmp/daylight-scopes-desktop-ac2-c21e90a-20260920-0910.log`. FIND.2a AC1–AC3 are accepted across three viewports. AC4 remains partial because close/reopen resets to All only on phone; desktop retains Video after reopen. |
...
| FIND.2a/AC4 | The kind I chose stays while search is open; the next time search opens it starts at All. Same on every device (Q6). | Partial | `JOURNEY-SEARCH-SCOPES`: phone Video stayed selected while SearchMode was open and reopening reset to All. On tablet and laptop, Video correctly drove scoped results but remained selected after close/reopen instead of resetting to All. |
