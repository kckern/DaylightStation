...
**Status:** In progress. **11 accepted stories / 32 accepted AC; 10 partial stories / 19 partial AC; 61 unverified stories / 237 unverified AC**. No component, test count or API response earns acceptance by itself.
...
|---|---|---|---|
| TASK-2-AIM-TRANSPORT-P0 | First-layout active aim, distinct Previous/restart-current, authoritative Add result/ordinal receipt, seek capability reasons, and stopped-queue handle | Behavioral REDs reproduced in focused Vitest: the brief suite failed 8/131, CastTargetProvider failed 1/10, LocalSessionController failed 4/73, and the Add receipt lacked an ordinal. | Focused implementation suite passes 232/232 across aim lifetime/provider, local/remote controllers, queue operations, controller conformance, transport/seek/mini-player UI, and Add receipt. The required isolated-receiver Playwright journeys were strengthened for active aim and Add continuity/ordinal plus Next/Previous/Stop, but no owned configured acceptance `BASE_URL` is available in this worktree, so they have not run on this source. Story/criterion statuses remain unchanged and `P0_EXTENSION_ENTRIES` remains empty until that exact runtime report exists. |
...
| JOURNEY-REMOTE-CONTROLS-B63 | Fleet Peek Pause/Resume/Seek, offline disable/reconnect, and Stop→queue→Play on virtual Acceptance receiver | Three serial runtime cases passed with no skips on compiled source `b63df3597a5c88b4c8dfe14ebef011ca8222b25a`: one Pause showed paused UI or honest pending feedback within 2s, then correlated ack, receiver-paused state and native pause were verified; ±10, absolute seek and drag reached native positions; disconnect rendered Offline/disabled controls, a tap produced zero transport POSTs for 5s, and reconnecting the actual receiver produced zero POSTs for another 5s. Stop retained queue and correlated receipt; Play resumed the stopped item. The 2s latency assertion applies only to that Pause press, not every control. Runtime did not test `DEVICE_OFFLINE` rejection or “Not sent” error copy. Separate TransportBar unit tests cover explicit `DEVICE_OFFLINE` feedback; uncommitted `SessionControlService.offline.characterization.test.mjs` passes 1/1 for existing zero-send/no-replay behavior and is characterization, not a RED product test. | Preview `41439`: `BASE_URL=http://127.0.0.1:41439 npx playwright test tests/live/flow/media/media-app-remote-controls.runtime.test.mjs --workers=1 --reporter=line --output=/tmp/daylight-transport-controls-b63df359-output`, 3/3 passed in 2.4m; raw log `/tmp/daylight-transport-controls-b63df359-20260920.log`. Focused TransportBar Vitest 22/22. |
| JOURNEY-REMOTE-CONTROLS-RESPONSIVE-B63 | Same actual Pause/Resume/Seek → virtual receiver native state journey from phone and tablet fleet navigation | Test-only responsive follow-up uses the same committed product source `b63df3597a5c88b4c8dfe14ebef011ca8222b25a`; the runtime spec was modified but uncommitted when these runs executed. Tablet (`820×1180`) passed 1/1 on preview `44071`. The first paired run's phone (`390×844`) case failed before device setup because it tried the desktop-only `cast-target-chip`; screenshot/source review confirmed mobile uses the Search launcher and SearchMode DestinationLine instead. A phone-only attempt then failed in test setup with `ReferenceError: mobile is not defined`, before the journey ran. After correcting only the loop destructuring, the phone case passed 1/1 in 44.1s on preview `40501`: visible Search launcher → DestinationLine destination sheet → Acceptance receiver → SearchMode Arrival result → close SearchMode → Devices tab → real Peek Pause/Resume/Seek and native assertions. These results add viewport evidence only; the run does not exercise offline/reconnect or Stop on phone/tablet and does not broaden AC/story acceptance. | Initial phone/tablet run on preview `44071`: `/tmp/daylight-controls-responsive-b63df359-20260920.log` (tablet passed, phone failed at hidden `cast-target-chip`); intervening phone harness failure `/tmp/daylight-controls-phone-b63df359-20260920.log`; corrected phone run `/tmp/daylight-controls-phone-corrected-b63df359-20260920.log`, output `/tmp/daylight-controls-phone-corrected-b63df359-output`, 1/1 passed. Runtime test source remained uncommitted through the run; no product source changed. |
| JOURNEY-REMOTE-CONTROLS-AUDIO-B63 | Same actual remote Pause/Resume/Seek journey for approved audio item Faith (`plex:584614`) | Test-only parameterization adds a source-specific query/result and scopes native observation to the visible `.audio-player` surface and its single `.audio-player audio` node, excluding ambient audio. On compiled product source `b63df3597a5c88b4c8dfe14ebef011ca8222b25a`, desktop passed 1/1 (47.8s) and phone/tablet passed 2/2 (1.4m). Pause/receiver/native state, paused relative ±10 seeks, absolute seek, real drag, and resume/native advancement all ran. The first local desktop attempt stopped at the first relative-seek check with test-harness `ReferenceError: video is not defined` before the native assertion; Terra corrected the helper to use the scoped native locator, then the reported desktop and responsive runs passed. Seek-distance guards use `min(60s, 15% duration)` for absolute movement and `min(60s, 5% duration)` for post-drag movement, so shorter audio uses a smaller threshold than the fixed 60s movie check. This adds audio-kind/viewport evidence only; STEER criteria and story counts remain unchanged and partial. | Desktop log `/tmp/daylight-faith-desktop-b63-final-20260920-1012.log`; phone/tablet log `/tmp/daylight-faith-phone-tablet-b63-20260920-1015.log`. Initial harness-failure log `/tmp/daylight-controls-audio-b63df359-20260920.log`, trace `/tmp/daylight-controls-audio-b63df359-output/live-flow-media-media-app--c135b-al-Faith-audio-while-paused/trace.zip`. Runtime spec was test-only; no product source changed. |
| JOURNEY-REMOTE-QUEUE-SKIP-09B9 | Fleet Peek Next/Previous on an actual two-item virtual receiver queue | On compiled product source `09b9bd7b2f76245694b08a7536940282c42db6dd`, ordinary result Add placed Disclosure after Arrival. `np-next`/`skipNext` returned through the command/ack path; receiver current item/index advanced to Disclosure/1 with unchanged ordered queue IDs, same owner instance and increased playback revision, native source changed and advanced unpaused, and the UI showed Disclosure with Previous enabled/Next disabled. Then `np-prev`/`skipPrev` returned through the command/ack path, but its 30s required predicate did not observe Arrival/index 0 or the expected native item transition. Source/trace diagnosis: `frontend/src/modules/Media/peek/RemoteSessionController.js` maps remote `skipPrev` to `synthBackspace`, whose intentional behavior restarts the current item; the trace shows Disclosure native position moving from ~1797s to 0, while local `skipPrev` directly advances to the prior queue item. This explains the RED without satisfying remote queue Previous. Failure snapshot still showed “Playing Disclosure Day,” Previous enabled and Next disabled. Virtual-only skip routes are now admitted by the ordinary fixture; physical routes remain blocked. Fixture-focused guard tests passed 13/13; full runtime journey remains RED, so STEER.3a/AC2 is Partial for verified Next only, with Previous and “any queue” coverage still unverified. | Runtime test `tests/live/flow/media/media-app-remote-controls.runtime.test.mjs`, `BASE_URL=http://127.0.0.1:40673 npx playwright test tests/live/flow/media/media-app-remote-controls.runtime.test.mjs --workers=1 --reporter=line --grep 'Peek Next and Previous' --output=/tmp/daylight-queue-skip-09b9-output`: 1 failed after 30s at Previous transition. Raw log `/tmp/daylight-queue-skip-09b9-20260920.log`; trace `/tmp/daylight-queue-skip-09b9-output/live-flow-media-media-app--34fdd-eal-ordinary-receiver-queue/trace.zip`. Artifact `/tmp/daylight-media-preview-DXRZvr`; preview stopped after run. |
| JOURNEY-PLAY-NOW-ENTRYPOINTS-REMOTE-CONTROLS | Browse, Detail and Recents Play now plus remote Pause/Resume/Seek/Stop through the virtual receiver | On source `b750538ca82e315e66bfff2d22d7b2d237d2fd4d`, the two-flow run ended 1 passed / 1 failed after 1.7m. All Browse/Detail/Recents Play now journeys passed native new-visit, same-tail and truthful-tray checks. Remote Pause/Resume/Seek passed native-state and sender-progress checks; Stop passed correlated ack, ready/currentItem-null state and native pause/reset before failing on a stale Arrival hint. Exact queue identity/index assertions occur after that failure and did not execute; remote Stop queue retention is not verified by this run. | Preview port 45045. `BASE_URL=http://127.0.0.1:45045 npx playwright test tests/live/flow/media/media-app-play-now-entrypoints.runtime.test.mjs tests/live/flow/media/media-app-remote-controls.runtime.test.mjs --workers=1 --reporter=line`; raw log `/tmp/daylight-remote-runtime-b750538ca82e315e-1789890319.log`. The run does not establish combined-suite GREEN or full-story acceptance. |
| JOURNEY-REMOTE-CONTROLS | Actual receiver Pause/Resume/seek/Stop from the fleet Peek panel | The isolated 25.6s run on source `7d22a589c86408849101d22bff3af9630c42574d` verifies Pause/Resume and Stop only as described below. Its `ArrowRight` +5 absolute seek reached the asserted sender value while playback was advancing, so natural progress could satisfy the check; it is insufficient causal evidence of seek control. A subsequent real Forward 10 control run on that same source failed: native video did not reach the requested seek state within 20s. Reverse, drag, and Stop were not reached in that run. On source `253121dcfc520e7f37ffa9d70aac22fc8412e06a`, paused relative ±10 and 35% track-position absolute seeks passed native verification. The 12% real drag landed at native time ~838s, but the video was no longer paused after the drag (trace diagnosis: stale jolt), so pause-preservation failed. In a separate Stop→Play journey, Stop reached ready/null-current, paused/reset native video and preserved queue identity/index, but the visible queue-kept receipt did not appear; queue reopening and Play were not reached. Root's trace review attributes the missing correlated receipt to the ACK topic route. Neither run establishes full remote seek/Stop acceptance; previous 253121 startup-gate attempt remains inconclusive, not evidence of a product buffering defect. | Source `7d22a589c86408849101d22bff3af9630c42574d`, preview port 34343. Initial command `BASE_URL=http://127.0.0.1:34343 npx playwright test tests/live/flow/media/media-app-remote-controls.runtime.test.mjs --workers=1 --reporter=line`: 1 passed (25.6s), raw log `/tmp/daylight-controls-7d22a589c8640884-1789890786.log`; Pause/Resume changed actual native state and sender progress; Stop returned ready/null-current, paused/reset the native element, and preserved currentIndex plus ordered queue identities/content. Causal Forward 10 failure on that source: 1 failed at native seek-state polling after 20s, raw log `/tmp/daylight-controls-seek-gap-7d22a589c8640884-1789891107.log`. On compiled source `253121dcfc520e7f37ffa9d70aac22fc8412e06a`, command `BASE_URL=http://127.0.0.1:36443 npx playwright test tests/live/flow/media/media-app-remote-controls.runtime.test.mjs --workers=1 --reporter=line`: first run reached paused relative ±10 native verification before test harness failure at immediate aria comparison, log `/tmp/daylight-extended-controls-253121dc-1789891795.log`; rerun with corrected comparison failed the initial snapshot-state gate (`buffering`, expected `playing`) after 5s, before controls, log `/tmp/daylight-extended-controls-253121dc-rerun-1789891871.log`. Latest source `e63b89a2bbe34b3cb1df46839bd1f3c57e599b77`, preview port 42491: `BASE_URL=http://127.0.0.1:42491 npx playwright test tests/live/flow/media/media-app-remote-controls.runtime.test.mjs --workers=1 --reporter=line` failed after paused relative ±10 and 35% absolute native checks passed; 12% drag failed the paused-state predicate after its native seek landed around 838s, raw log `/tmp/daylight-extended-controls-e63b89a2-1789892311.log`. Separate `--grep 'Peek Stop retains the receiver queue and Play resumes the stopped item'` run passed Stop ready/null-current/native reset/full ordered queue identity checks but failed before queue-kept feedback/reopen/Play because `peek-queue-kept` was absent, raw log `/tmp/daylight-stop-play-e63b89a2-1789892544.log`. No full STEER story acceptance is claimed. |
| JOURNEY-RESULT-MORE-PLAY-NOW | Search result More → Play now while a different item is actually playing on the aimed receiver | Historical evidence includes an intermittent default “Sent” instead of “Playing” tray in the `17602653` paired run; its isolated default case passed, but that left the discrepancy unresolved at the time. The latest rerun now passes both parameterized cases together on the same compiled source: explicit fork and fresh/default each completed the actual receiver visit/revision change, preserved Disclosure's queued tail, advanced native playback and showed the matching truthful tray. Earlier temporary teardown assertion was invalid because wildcard WebSocket subscriptions are included in subscriber counts; it was removed. | Runtime file `tests/live/flow/media/media-app-result-play-now.runtime.test.mjs`. Source `d49abd2dcc720c892218b55b3474b02a77807fb3`, preview port 39601, explicit fork: 1 passed (33.9s). Actual Play Arrival → Add Disclosure → More/Play now Arrival; third command reached the receiver as a new visit/revision, Disclosure retained the same `queueItemId` and queue tail, native playback advanced, and the matching tray appeared. Follow-up source `17602653f9657f1203f4fa465388c30ed30a5522`, preview port 40679, paired fork/default run: fork passed; default failed “Playing” tray (`Sent` appeared) despite native advancement and receiver state; isolated default rerun passed (1/1, 28.6s). Final paired verification on compiled source `1cb363b1e73e1b6a3d5d11ee314ab7c3384f215d`, preview port 41963, `BASE_URL=http://127.0.0.1:41963 npx playwright test tests/live/flow/media/media-app-result-play-now.runtime.test.mjs --workers=1 --reporter=line --output=/tmp/daylight-result-play-now-1cb363b1-output`: 2/2 passed in 1.2m. Raw log `/tmp/daylight-result-play-now-1cb363b1-20260920.log`. This resolves the fork/default discrepancy for this result-More journey; broader surfaces remain outside its evidence. PLAY.1a AC2 and PLACE.2a AC6 remain partial, not full acceptance. |
| JOURNEY-SEARCH-SCOPES | FIND.2a configured All, Video and parent/child scopes across phone SearchMode, tablet and laptop dock | Initial run against source `18d032ae533731496ae686122fc1bf84bed5e3cc` ended 2 passed / 7 failed. On compiled source `e4904b6c8fe58c347a905763fe4331b85ec3c49f`, the nine-case run ended 4 passed / 5 failed because the phone's valid `Documentary Serie · Episode` kind subtitle was rejected by an overly narrow matcher and desktop requests had not yet been fixed. The focused phone run passed 3/3. On compiled source `c21e90a0587e65799488a2b748ef1ebc75466980`, the nine-case run passed 7/9: AC1 and AC3 passed all viewports, phone AC2+AC4 passed, and desktop AC2's kind matcher rejected the renderer's `•` separator. After correcting the separator and implementing a production-shaped close/reopen reset through ContentCombobox, final compiled source `1cb363b1e73e1b6a3d5d11ee314ab7c3384f215d` passed all nine cases with no skips: AC1–AC4 passed on phone SearchMode, tablet dock and laptop dock, including unchanged-query scoped result streams, configured parent/child results, and All restored after reopen. | Initial preview `38317`, raw log `/tmp/daylight-search-scopes-18d032ae-1789893436.log`. Compiled preview `39171`: initial `e4904b6c` run `/tmp/daylight-search-scopes-e4904b6c-20260920-0849.log`; focused phone `e4904b6c` run `/tmp/daylight-search-scopes-phone-e4904b6c-20260920-0852.log`. Compiled preview `44059`: nine-case run `/tmp/daylight-scopes-final-c21e90a-20260920-0907.log`; focused desktop rerun `/tmp/daylight-scopes-desktop-ac2-c21e90a-20260920-0910.log`. Final preview `38473`: `BASE_URL=http://127.0.0.1:38473 npx playwright test tests/live/flow/media/media-app-search-scopes.runtime.test.mjs --workers=1 --reporter=line --output=/tmp/daylight-search-scopes-1cb363b1-output`, raw log `/tmp/daylight-search-scopes-1cb363b1-20260920.log`, 9/9 passed in 27.9s. |
| JOURNEY-REMOTE-STOP-PLAY | Actual receiver Stop → retained queue feedback/reopen → Play on the same item | A focused `e4904b6c` run passed Stop receipt/native reset/queue retention/Open queue, then got a matching Play ACK but did not observe receiver playing the same Arrival within 20s; native/receipt-clear follow-up did not run. On final source `c21e90a0587e65799488a2b748ef1ebc75466980`, the complete Stop→Play journey passed, including queue feedback, exact retained queue, Open queue, correlated Play ACK, same-item receiver resume, native advancement and receipt clearance. This one remote receiver journey does not establish whole-story or cross-surface acceptance. | Compiled preview `44059`: `BASE_URL=http://127.0.0.1:44059 npx playwright test tests/live/flow/media/media-app-remote-controls.runtime.test.mjs --workers=1 --reporter=line --output=/tmp/daylight-controls-output-c21e90a-20260920-0907`; 2/2 journeys passed in 57.8s, raw log `/tmp/daylight-controls-final-c21e90a-20260920-0907.log`. Earlier `e4904b6c` Stop→Play run: `/tmp/daylight-stop-only-e4904b6c-1789894323.log`. |
| JOURNEY-AIM-PERSISTENCE-MATRIX | PLACE.2a/AC1–AC3: visible aim selection, navigation/reload persistence, and two-hour idle expiry across phone SearchMode, tablet Browse header/dock, and laptop Browse header/dock | First phone attempt used the desktop-only combobox-option selector in SearchMode; corrected to the existing ordinary SearchMode result-row button. | Compiled source `d5b64d93e937ac0070cafa19b952ff2ec0e93778`, preview `http://127.0.0.1:35415`; `BASE_URL=http://127.0.0.1:35415 npx playwright test tests/live/flow/media/media-app-aim-persistence.runtime.test.mjs --workers=1 --reporter=line`: 3 passed (33.7s). AC1 accepted with this matrix plus the existing ordinary dock chooser and accepted PLACE.2b phone Browse chooser, covering every actual aim-change surface; AC2 navigation/reload persistence and AC3 automatic two-hour expiry verified in all three viewport flows. No device commands. AC4 and AC6 remain unverified; this does not accept the full PLACE.2a story. |
| JOURNEY-AIM-ACTIVE-SENT | PLACE.2a/AC4: aimed ordinary Play while actual receiver playback is active | The earlier clock-jump attempt expired the aim because raw homeline progress omitted payload `deviceId`; DispatchProvider now derives the authoritative ID from the topic. | Compiled source `f2cf4d6fa89dd8a86994d7f644419402c0e9938d`, preview `http://127.0.0.1:43877`; `BASE_URL=http://127.0.0.1:43877 npx playwright test tests/live/flow/media/media-app-active-aim.runtime.test.mjs --workers=1 --reporter=line`: 1 passed (29.0s). Finite exemption timestamps before and after, at least two hours of sender-clock advance, same actual native video unpaused and advancing over 20 seconds, and target still selected in the visible picker. Partial AC4 only: this verifies an ordinary sent item, not the separate actual-steering path. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| FIND.1b/AC1 | There is only one search. Choosing a screen never opens a different search. | Accepted | Exact source `034f3339e`; native phone/tablet/laptop destination-switch journeys passed 3/3 in `media-app-search-scopes.runtime.test.mjs`; clean Sol scoped review approved |
| FIND.1b/AC2 | The actions on a result are the same whichever screen is aimed at; only the stated destination differs. | Accepted | Exact source `034f3339e`; native phone/tablet/laptop remote Play/Add/local Play and complete action-identity journeys passed 3/3 in `media-app-result-play-now.runtime.test.mjs`; clean Sol scoped review approved |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| FIND.2a/AC1 | Kinds are shown as choices next to the search; the current one is always visible. | Accepted | `JOURNEY-SEARCH-SCOPES`: All/configured chips and current selection passed on phone SearchMode, tablet dock and laptop dock on compiled source `c21e90a0587e65799488a2b748ef1ebc75466980`; phone follow-up passed 3/3. |
| FIND.2a/AC2 | Choosing a kind updates results immediately without retyping. | Accepted | `JOURNEY-SEARCH-SCOPES`: on phone, tablet and laptop, selecting configured Video reissued the unchanged `Frozen` query and rendered actual Video-scoped results with kind subtitles. Desktop focused rerun passed this assertion before the separate AC4 reset check failed. |
| FIND.2a/AC3 | Kinds that contain sub-kinds (for example, Music → Hymns) let me choose either level. | Accepted | `JOURNEY-SEARCH-SCOPES`: configured parent and child selections and their real scoped result streams passed on phone SearchMode, tablet dock and laptop dock. |
| FIND.2a/AC4 | The kind I chose stays while search is open; the next time search opens it starts at All. Same on every device (Q6). | Accepted | `JOURNEY-SEARCH-SCOPES`: on final compiled source `1cb363b1e73e1b6a3d5d11ee314ab7c3384f215d`, Video remained selected while search stayed open and close/reopen reset to All on phone SearchMode, tablet dock and laptop dock. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| PLAY.1b/AC1 | With the aim on this device, every "play now" starts on this device; no other screen changes. | Accepted | Exact source `e5f954a8e`; real Browse leaf, Detail and Home Recents Play Now matrix passed phone/tablet/laptop 9/9, while accepted FIND.1b supplies the shared Search result More path. Each proves a distinct local play session, native restart then advancement, zero receiver load/transport and unchanged receiver identity/revisions/queue. Sol review approved. |
| PLAY.1b/AC2 | Playback starts without leaving the part of the app I'm in; a compact handle appears (see `STEER.1`). | Accepted | The same 9/9 exact-artifact matrix preserves each exact URL and route content while Arrival appears in the visible compact player/video dock; Search stays mounted with its query/result/action identity in FIND.1b. Log `/tmp/daylight-play1b-e5f954a8e-runtime.log`, 3.6m serial; review approved. |
...
| PLAY.6a/AC3 | The confirmation names the item, its position ("7th"), and the screen. | Unverified | Task 2 adds an ordinal confirmed receipt backed by authoritative queue revision/length and unit coverage; the strengthened actual-receiver journey is not acceptance evidence until its exact owned-runtime report passes. |
...
| PLACE.2a/AC4 | That clock doesn't run while the aimed screen is playing something this device sent or is steering (R4). | Partial | `JOURNEY-AIM-ACTIVE-SENT`, compiled `f2cf4d6fa89dd8a86994d7f644419402c0e9938d`, preview port 43877: ordinary aimed Play kept the actual native receiver video advancing/unpaused for >20s while the two-hour sender-clock jump retained finite exemption timestamps and the visible selected aim. Task 2 also protects first layout when restored idle time is expired but a matching fresh sent/steered playback is already present; focused provider/lifetime tests pass, while its strengthened exact runtime is pending. Separate actual-steering path remains unverified. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| PLACE.3b/AC1 | With another screen as the aim, or via **Add to queue on…** on the item, the item joins that screen's queue. | Accepted | Exact product `ee0e38db9`, test commit `65270630c`; real phone/tablet/laptop More → Add to Queue targets the aimed Acceptance receiver and proves the same session/owner/current visit/playback revision, increased queue revision, exact `plex:697368` visit, and truthful Added tray. Serial 5/5 in 2.2m; Sol review approved. |
| PLACE.3b/AC2 | That screen's queue visibly includes the new item when I look. | Accepted | The same journeys open real Devices/Fleet → Acceptance Peek and find the exact `queue-item-${disclosureVisit.queueItemId}` visibly titled Disclosure Day. Peek sends no load/transport, preserves aim identity and receiver revisions. Log `/tmp/daylight-place3b-ee0e38db9-final-20260920.log`; review approved. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| STEER.1c/AC1 | The confirmation that something started on a screen offers "steer it" (see `RELY.3`). | Accepted | Two simultaneous confirmed rows prove selecting Office routes/removes only Office, leaving Living Room actionable. Exact-SHA runtime then dispatches to Acceptance receiver, changes preferred aim to This device without altering that dispatch, opens the confirmation's “Steer it” Peek, names Acceptance receiver in that panel, and pauses the actual native receiver through its visible control. Desktop `1440×900`, phone `390×844`, and tablet `820×1180` passed. |
| STEER.1c/AC2 | The same route is available afterwards from the house overview, with no time limit. | Accepted | After the confirmation-opened Peek pauses the native Acceptance receiver and real receiver state becomes paused, Home → Devices independently reopens the same receiver and visibly resumes it; native playback becomes unpaused. This is not the 8-second confirmation route. Exact source `64e08ee4159bf7a75b68a36bc3aa04e672656330`, preview `http://127.0.0.1:45429`, runtime log `/tmp/daylight-steer1c-fix1b-runtime-64e08ee4-20260920.log`, 3/3 passed in 52.7s; Sol scoped re-review approved all findings. |
...
| STEER.3a/AC2 | Skip forward and back are available for any queue. | Partial | `JOURNEY-REMOTE-QUEUE-SKIP-09B9` proves actual `np-next`/`skipNext` advanced the two-item Acceptance queue from Arrival/index 0 to Disclosure/index 1, preserved ordered queue IDs and owner instance, increased playback revision, and changed/advanced the native player with matching UI. The following `np-prev`/`skipPrev` command returned through the command/ack path but restarted Disclosure from ~1797s to 0 instead of returning to Arrival/index 0. Task 2 now maps remote and local Previous to `skipPrev` and exposes restart-current separately as seek-to-zero, with controller tests; the repaired actual-receiver Next/Previous journey is pending an owned runtime and “any queue” coverage remains unverified. Existing ±10 evidence is `seekRel` and belongs to STEER.4a/AC2, not this criterion. Physical routes remain blocked. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| HOUSE.1a/AC1 | One house indicator is visible everywhere in the app, on every device size, shown once. | Accepted | Exact source `a8ee74b7c`; phone/tablet/laptop matrix asserts all six Canvas roots (18 combinations) before proving exactly one visible `house-indicator`. Runtime 3/3 in 1.1m; Sol review approved. |
| HOUSE.1a/AC2 | It states how many screens are playing (and whether any are paused). | Accepted | Real Acceptance receiver/native playback first shows `1 playing`, then visible Peek Pause produces transport 200, receiver/native paused and `0 playing · 1 paused`. Offline-playing snapshots are excluded by focused unit coverage. Exact log `/tmp/daylight-house1a-final-runtime.log`; review approved. |
| HOUSE.1a/AC3 | Opening it shows the full overview in one step. | Accepted | From each verified Canvas root at all three sizes, one activation of the sole indicator reaches visible canonical Fleet with no intermediate surface. Exact artifact `/tmp/daylight-media-preview-5eYbap`; review approved. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| HOUSE.2b/AC1 | This device appears in the overview, marked as "this device". | Accepted | Exact source `bf9edf3d2`; the owning browser's Fleet row is marked **This device**, named `Sender browser`, and shows the actual advancing Arrival session. Clean-detached runtime 1/1; Sol review approved. |
| HOUSE.2b/AC2 | What it shows matches what other people see for it from their devices (checked with two devices side by side). | Accepted | Two isolated browser contexts on the shared `PLAYBACK_STATE_TOPIC` agree on sender name, Arrival title and Playing, then the observer changes to Paused after the sender pauses its real native video without observer navigation. Clean exact artifact SHA-256 `3f0897f4b2e0d8bd0f342d782f540d270c8299a988a152c8360400d1ceb3ebb8`; runtime `/tmp/daylight-house2b-r2-runtime.log`, provenance `/tmp/daylight-house2b-r2-proof-manifest.log`; Sol review approved. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| RELY.9a/AC1 | The current area is always highlighted, including when I'm in the queue or another screen's controls. | Accepted | Exact source `ee0e38db9`; real phone/tablet/laptop journeys assert Browse in Detail, Home in Now Playing/local queue, and Devices in Peek/remote queue. Authoritative runtime log `/tmp/daylight-media-navigation-history-ee0e38db9.log`, 9/9 serial in 1.1m; focused regression 95/95; final Sol review approved. |
| RELY.9a/AC2 | Choosing the area I'm already in takes me to its top, without adding extra steps to going back. | Accepted | Exact source `ee0e38db9`; each viewport reselects Browse from a real nested Detail to canonical Browse, then one real browser Back reaches Home with no duplicate Browse stop. Same 9/9 exact-artifact runtime and approved review. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| RELY.10a/AC1 | Back, whether the device's own back or an on-screen one, goes one step to where I actually came from. | Accepted | Exact source `ee0e38db9`; Detail Back restores the exact nested URL and live `plex:55854` row, Now Playing returns to Browse, Peek returns to Fleet, phone browser Back closes SearchMode before the next Back reaches Home, and depth-one fallback passes on all three viewports. Runtime 9/9; review approved. |
| RELY.10a/AC2 | Any on-screen back is labelled with where it goes, and goes there. | Accepted | Exact source `ee0e38db9`; real journeys assert and click `← Browse` from Detail/Now Playing and `← Devices` from Peek, then verify each named destination. Focused component/provider coverage pins Browse crumb labels. Runtime 9/9; review approved. |
| RELY.10a/AC3 | Closing a pop-up counts as one back. | Accepted | Exact source `ee0e38db9`; phone Escape closes Destination Sheet only and retains SearchMode plus `Frozen`, then separate browser Backs close SearchMode and change route. Tablet/laptop Escape closes only real results, retains `Frozen` and Browse, then browser Back reaches Home. Runtime 9/9; focused regression 95/95; review approved. |
