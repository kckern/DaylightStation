...
| JOURNEY-REMOTE-CONTROLS | Actual receiver Pause/Resume/seek/Stop from the fleet Peek panel | The isolated 25.6s run on source `7d22a589c86408849101d22bff3af9630c42574d` verifies Pause/Resume and Stop only as described below. Its `ArrowRight` +5 absolute seek reached the asserted sender value while playback was advancing, so natural progress could satisfy the check; it is insufficient causal evidence of seek control. A subsequent real Forward 10 control run on that same source failed: native video did not reach the requested seek state within 20s. Reverse, drag, and Stop were not reached in that run. On built source `253121dcfc520e7f37ffa9d70aac22fc8412e06a`, a strengthened run passed paused relative ±10 native seek verification, then failed on a test-side immediate aria value comparison (2443s difference); that assertion was corrected to compare the command's absolute target to the computed target. Its rerun reported `buffering` rather than `playing` at the initial receiver-state poll after 5s, before controls; native readiness had passed. The trace had no native time series, so this is an inconclusive startup-gate observation, not evidence of a product buffering defect. The startup gate was then strengthened (test-only) to require native unpaused advancement >1s and receiver `playing` together, with a 30s readiness poll. There is no post-fix causal absolute/drag/resume/Stop receipt runtime result yet. | Source `7d22a589c86408849101d22bff3af9630c42574d`, preview port 34351. Initial command `BASE_URL=http://127.0.0.1:34351 npx playwright test tests/live/flow/media/media-app-remote-controls.runtime.test.mjs --workers=1 --reporter=line`: 1 passed (25.6s), raw log `/tmp/daylight-controls-7d22a589c8640884-1789890786.log`; Pause/Resume changed actual native state and sender progress; Stop returned ready/null-current, paused/reset the native element, and preserved currentIndex plus ordered queue identities/content. Causal Forward 10 failure on that source: 1 failed at native seek-state polling after 20s, raw log `/tmp/daylight-controls-seek-gap-7d22a589c8640884-1789891107.log`. On compiled source `253121dcfc520e7f37ffa9d70aac22fc8412e06a`, command `BASE_URL=http://127.0.0.1:36443 npx playwright test tests/live/flow/media/media-app-remote-controls.runtime.test.mjs --workers=1 --reporter=line`: first run reached paused relative ±10 native verification before test harness failure at immediate aria comparison, log `/tmp/daylight-extended-controls-253121dc-1789891795.log`; rerun with corrected comparison failed the initial snapshot-state gate (`buffering`, expected `playing`) after 5s, before controls, log `/tmp/daylight-extended-controls-253121dc-rerun-1789891871.log`. Relative jump buttons, causal absolute/drag/runtime Stop receipt on the revised path, live media, the two-second deadline, offline delivery/receipt semantics and screen-off remain unverified; no full STEER story acceptance is claimed. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| STEER.4a/AC1 | A position bar can be dragged; the time is shown while dragging. | Partial | `JOURNEY-PAUSED-SEEK` verifies local Arrival/Disclosure seek completion and visible bar. The earlier remote ArrowRight absolute seek could have passed on natural progress and is not causal seek evidence; the subsequent remote Forward 10 run failed to reach native seek state. Strengthened remote absolute/drag/time assertions await execution against the new build. |
| STEER.4a/AC2 | Jump back and forward buttons are present for every playback, local or on another screen. | Partial | `JOURNEY-LOCAL` verifies actual local ±10-second jumps. `JOURNEY-REMOTE-CONTROLS` does not exercise relative jump buttons; remote parity remains unverified. |
...
| STEER.6a/AC2 | After stopping, I'm told what remains ("Queue kept: 8 items") and can reopen it. | Partial | `JOURNEY-STOP-RESTART` verifies local queue reopen. `JOURNEY-REMOTE-CONTROLS` verifies remote Stop preserves currentIndex and the complete ordered queue identity/content list; visible retained-count feedback and remote reopen remain unverified. |
