...
|---|---|---|---|
| JOURNEY-QUEUE-ESCAPE | Paused Arrival → Add → Escape → second same-title queue entry | Run86990 Escape navigates away with results still open | Run26262 GREEN29.3s after Task9a repair: retained paused Add, first Escape preserves controls/queue, selecting duplicate title actually advances from zero. Independent Escape review remains pending; broader verb/target/device parity still required |
...
| JOURNEY-HLS-SERIAL | Full7 Disclosure HLS cases, actual branch read/mint | Startup first2 exceed30s readiness budget; decisions26168ms/27693ms, retry metadata6287ms/7680ms; no renewed blob corruption | Run34664 5passed/2failed4.8min. Deep seek, pause/resume, rapid controls, focus and Stop pass. All7 attach successful Stop200 evidence for8 exact observed sessions. Startup latency remains unresolved; no full baseline acceptance |
...
| JOURNEY-NATIVE-MP4 | Actual branch-qualified Arrival original MP4; full-content seek, focus, Stop | Previous global DASH baseline could not start reliably | Core run64066 GREEN17.6s; serial run39999 5/7 passed3.2m (seek/bar, deep seek, rapid controls, focus, Stop). Two failures before app readiness under separate trace diagnosis. No whole-story or HLS acceptance |
...
| EMPTY-QUEUE-ADD | Ordinary pointer Add with no playback | Run89426 RED: a video appeared before any explicit Play; trace records only the intended Add action. Existing queue/controller first-add rule loads the new current item | Not repaired. Task4b prepared behind F1 file lease. Earlier instantaneous no-video assertions in keyboard/F2 runs do not exclude delayed startup; route/query/queue observations remain valid, no-autoplay acceptance is withdrawn |
...
| JOURNEY-LOCAL | Disclosure Day search → actual video → seek/pause/stop | Actual video advances; visible seek slider disabled with duration/position zero. Expand video control absent. | Earlier6-journey development GREEN is superseded by latest broader run85106: all7 failed initial native readiness before interaction assertions, despite two isolated decoded-seek greens. Trace has late/repeated mint, MPD200, unfinished init-header requests and no media segments; native readyState0/empty buffer. Corrective Task2b diagnoses startup/recovery and operational gate; no baseline acceptance |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| PLAY.6a/AC1 | "Add to queue" is available on every item wherever it appears and uses the aim. | Failing | JOURNEY-QUEUE: desktop result More actions replays row and does not expose Add to Queue; Task4 |
...
| PLACE.2a/AC5 | Opening the app after that much idle starts on "this device", even though a reload otherwise restores the aim (R7). | Partial | JOURNEY-AIM-IDLE reload after observed expiry remains local; first-layout stale persisted aim covered by review-round unit RED→GREEN. Full closed-app/reopen browser path and review pending |
