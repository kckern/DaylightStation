...
**Status:** In progress. **2 accepted stories / 10 accepted AC; 11 partial stories / 18 partial AC; 69 unverified stories / 260 unverified AC**. No component, test count or API response earns acceptance by itself.
...
| JOURNEY-SEARCH-SCOPES | FIND.2a configured All, Video and parent/child scopes across phone SearchMode, tablet and laptop dock | Initial run against source `18d032ae533731496ae686122fc1bf84bed5e3cc` ended 2 passed / 7 failed. Phone failures were strict-locator ambiguity: hidden dock chips and active SearchMode chips shared test IDs. Desktop AC2/AC3 waited for scoped stream requests that did not arrive; the failure snapshot showed Video pressed while the result dropdown was closed. Source inspection identified the input blur/outside-close path as the likely query-loss cause; the later focus-preservation change and narrowed locators have not been rerun. The Video-kind assertion now reads the dedicated result subtitle, not arbitrary row title text. No FIND story or AC acceptance is claimed by this run. | Preview port 38317. `BASE_URL=http://127.0.0.1:38317 npx playwright test tests/live/flow/media/media-app-search-scopes.runtime.test.mjs --workers=1 --reporter=line`; raw log `/tmp/daylight-search-scopes-18d032ae-1789893436.log`. The passing cases were tablet/laptop AC1; remaining failures were harness ambiguity or the scoped-request gap. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| STEER.6a/AC1 | There is one "stop" control, meaning the same thing everywhere. | Partial | `JOURNEY-STOP-RESTART` verifies local Stop; `JOURNEY-REMOTE-CONTROLS` verifies remote Stop via correlated ack, ready/null-current receiver state, and native pause/reset. Consistent semantics across all surfaces remain unverified. |
| STEER.6a/AC2 | After stopping, I'm told what remains ("Queue kept: 8 items") and can reopen it. | Partial | `JOURNEY-STOP-RESTART` verifies local queue reopen. Remote Stop retains currentIndex and ordered queue identity/content, but the latest `e63b89a2` run failed before the queue-kept receipt appeared, so remote feedback/reopen (and Stop→Play) remain unverified. |
