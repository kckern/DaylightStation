...
**Status:** In progress. **2 accepted stories / 10 accepted AC; 11 partial stories / 17 partial AC; 69 unverified stories / 261 unverified AC**. No component, test count or API response earns acceptance by itself.
...
| PLACE.2a/AC6 | Every play and line-up action then uses it (see `PLAY.1a`). | Partial | `JOURNEY-ORDINARY-REMOTE-PLAY-ADD` verified one ordinary Search Play and result-More Add against the aimed receiver, with native playback retained after Add. Root's independent rerun against compiled `f2cf4d6fa89dd8a86994d7f644419402c0e9938d` at preview port 43877 passed (1 passed, 14.3s), preserving the native/queue/tray flow. Other play/line-up verbs and surfaces remain unverified. |
