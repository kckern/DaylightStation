...
**Status:** In progress. **2 accepted stories / 11 accepted AC; 12 partial stories / 21 partial AC; 68 unverified stories / 256 unverified AC**. No component, test count or API response earns acceptance by itself.
...
| JOURNEY-RESULT-MORE-PLAY-NOW | Search result More → Play now while a different item is actually playing on the aimed receiver | An earlier combined fork/default run intermittently showed “Sent” instead of “Playing” for default mode; the cause remains unexplained. The latest paired run passed both business flows through teardown, then failed only an added `ready` check: its WebSocket subscriber count included wildcard subscriptions, while command liveness remains fresh for 30 seconds. That check was removed; this is not combined-suite GREEN evidence. | Runtime file `tests/live/flow/media/media-app-result-play-now.runtime.test.mjs`. Source `d49abd2dcc720c892218b55b3474b02a77807fb3`, preview port 39601, explicit fork: 1 passed (33.9s). Actual Play Arrival → Add Disclosure → More/Play now Arrival; third command reached the receiver as a new visit/revision, Disclosure retained the same `queueItemId` and queue tail, native playback advanced, and the matching tray appeared. Follow-up build source `17602653f9657f1203f4fa465388c30ed30a5522`, preview port 40679, parameterized fork/default combined run: fork passed; default failed the expected “Playing” tray assertion (`Sent` appeared), despite first-load/native advancement and receiver state. Isolated rerun against the same compiled source/preview, `BASE_URL=http://127.0.0.1:40679 npx playwright test tests/live/flow/media/media-app-result-play-now.runtime.test.mjs --workers=1 --reporter=line --grep 'fresh/default mode'`: 1 passed (28.6s). PLAY.1a AC2 and PLACE.2a AC6 retain partial status, not full acceptance. |
| JOURNEY-SEARCH-SCOPES | FIND.2a configured All, Video and parent/child scopes across phone SearchMode, tablet and laptop dock | Initial run against source `18d032ae533731496ae686122fc1bf84bed5e3cc` ended 2 passed / 7 failed. On compiled source `e4904b6c8fe58c347a905763fe4331b85ec3c49f`, the nine-case run ended 4 passed / 5 failed: AC1 passed on all three viewports; phone AC3 passed; phone AC2 reached the rendered kind assertion but its initial matcher rejected the valid dedicated subtitle `Documentary Serie · Episode`. Tablet/laptop AC2 and AC3 still timed out waiting for scoped requests; the failure snapshots showed the selected scope but no matching request. After narrowing phone locators and correcting the kind-token matcher, the focused phone run passed 3/3 (8.9s), verifying Video results, parent/child selection, and close/reopen reset to All. Desktop scoped-request behavior remains unresolved; FIND.2a is not fully accepted. | Initial preview `38317`, raw log `/tmp/daylight-search-scopes-18d032ae-1789893436.log`. Compiled preview `39171`: `BASE_URL=http://127.0.0.1:39171 npx playwright test tests/live/flow/media/media-app-search-scopes.runtime.test.mjs --workers=1 --reporter=line`, raw log `/tmp/daylight-search-scopes-e4904b6c-20260920-0849.log`; focused phone `BASE_URL=http://127.0.0.1:39171 npx playwright test tests/live/flow/media/media-app-search-scopes.runtime.test.mjs --workers=1 --reporter=line --grep 'phone SearchMode'`, raw log `/tmp/daylight-search-scopes-phone-e4904b6c-20260920-0852.log`. FIND.2a AC1 is accepted across phone/tablet/laptop; AC2/AC3/AC4 remain partial because the phone path passed but desktop behavior is unresolved. |
| JOURNEY-REMOTE-STOP-PLAY | Actual receiver Stop → retained queue feedback/reopen → Play on the same item | Latest compiled-source run passed Stop receipt, native reset, exact retained queue assertions and Open queue; after Play returned a matching ACK, the receiver did not report playing the same Arrival item within 20s. Later native/receipt-clear verification did not run. Remote queue feedback/reopen has evidence, but Stop→Play resume remains unverified; this does not establish STEER story acceptance. | Source `e4904b6c8fe58c347a905763fe4331b85ec3c49f`; one focused runtime case, log `/tmp/daylight-stop-only-e4904b6c-1789894323.log`. This later run supersedes the earlier e63 report that queue-kept feedback was missing; no rerun is claimed. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| FIND.2a/AC1 | Kinds are shown as choices next to the search; the current one is always visible. | Accepted | `JOURNEY-SEARCH-SCOPES`: All/configured chips and current selection passed on phone SearchMode, tablet dock, and laptop dock on compiled source `e4904b6c8fe58c347a905763fe4331b85ec3c49f`; phone follow-up passed 3/3. |
| FIND.2a/AC2 | Choosing a kind updates results immediately without retyping. | Partial | `JOURNEY-SEARCH-SCOPES`: phone Video selection reissued the same `Frozen` query under the configured Video scope, rendered video-kind subtitles, and retained the query. Tablet/laptop scoped requests did not arrive in the compiled run; no cross-device acceptance. |
| FIND.2a/AC3 | Kinds that contain sub-kinds (for example, Music → Hymns) let me choose either level. | Partial | `JOURNEY-SEARCH-SCOPES`: phone configured parent/child selection passed. Tablet/laptop parent/child stream request checks timed out; those viewports remain unverified. |
| FIND.2a/AC4 | The kind I chose stays while search is open; the next time search opens it starts at All. Same on every device (Q6). | Partial | `JOURNEY-SEARCH-SCOPES`: phone Video stayed selected while SearchMode was open and reopening reset to All. Desktop close/reopen was not reached because the scoped request checks timed out. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| PLAY.1a/AC1 | "Play now" on a search result, a browsed item, an item's details, a suggestion, or a recent item all play at the aim shown on screen. | Partial | `JOURNEY-ORDINARY-REMOTE-PLAY-ADD` verifies ordinary aimed Search Play. `JOURNEY-RESULT-MORE-PLAY-NOW` verifies result-More Play now to the explicitly aimed receiver in fork mode; default-mode tray discrepancy remains unresolved. The earlier source `8b338aefd02cc6047b624898574fe06569f6c697` run failed Detail’s tray check due to session buffering despite native advancement; `JOURNEY-PLAY-NOW-ENTRYPOINTS-REMOTE-CONTROLS` on `b750538ca82e315e66bfff2d22d7b2d237d2fd4d` subsequently passed Browse, Detail and Recents native new-visit and truthful-tray checks. Suggestions remain unverified. |
| PLAY.1a/AC2 | The item starts now on that screen; whatever was queued after the old item stays queued (R1). | Partial | `JOURNEY-RESULT-MORE-PLAY-NOW`, source `d49abd2dcc720c892218b55b3474b02a77807fb3`, preview port 39601, explicit fork: actual new receiver visit/revision, Disclosure retained the same `queueItemId` and queue tail, advancing native playback, matching tray (1 passed, 33.9s). Follow-up `17602653f9657f1203f4fa465388c30ed30a5522` preview port 40679: explicit-fork case passed; default-mode case failed because the tray showed “Sent” instead of expected “Playing.” `JOURNEY-PLAY-NOW-ENTRYPOINTS-REMOTE-CONTROLS` later passed native new visits and same-tail checks for Browse, Detail and Recents on source `b750538ca82e315e66bfff2d22d7b2d237d2fd4d`; its remote-control flow failed before the queue identity/index assertions, so it does not verify remote Stop queue retention. |
...
| PLACE.2a/AC6 | Every play and line-up action then uses it (see `PLAY.1a`). | Partial | `JOURNEY-ORDINARY-REMOTE-PLAY-ADD` verified ordinary Search Play and result-More Add against the aimed receiver. `JOURNEY-RESULT-MORE-PLAY-NOW` adds result-More Play now against the explicitly aimed receiver in fork mode, preserving the queue tail and advancing native playback. Its default-mode case showed “Sent” rather than “Playing,” so default-mode behavior and other play/line-up verbs and surfaces remain unverified. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| STEER.3a/AC1 | Play/pause is one control that shows whether it is playing or paused. | Partial | `JOURNEY-LOCAL` verifies local pause/resume. `JOURNEY-REMOTE-CONTROLS` verifies the visible remote Peek toggle changes actual receiver native pause/play state and its displayed action; duplicate-handle/surface parity remains unverified. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| STEER.4a/AC1 | A position bar can be dragged; the time is shown while dragging. | Partial | `JOURNEY-PAUSED-SEEK` verifies local Arrival/Disclosure seek completion and visible bar. Remote 35% track-position absolute seek passed native verification on source `e63b89a2`; the 12% real drag reached ~838s but failed pause preservation after a stale jolt. Remote drag/time behavior is not accepted. |
| STEER.4a/AC2 | Jump back and forward buttons are present for every playback, local or on another screen. | Partial | `JOURNEY-LOCAL` verifies actual local ±10-second jumps. `JOURNEY-REMOTE-CONTROLS` verifies remote paused ±10 commands reached matching native positions on source `e63b89a2`; full parity across controls and surfaces remains unverified. |
...
| Criterion | Observable outcome | Status | Test / evidence |
|---|---|---|---|
| STEER.6a/AC1 | There is one "stop" control, meaning the same thing everywhere. | Partial | `JOURNEY-STOP-RESTART` verifies local Stop; `JOURNEY-REMOTE-CONTROLS` and `JOURNEY-REMOTE-STOP-PLAY` verify remote Stop via correlated ack, ready/null-current receiver state, and native pause/reset. Consistent semantics across all surfaces remain unverified. |
| STEER.6a/AC2 | After stopping, I'm told what remains ("Queue kept: 8 items") and can reopen it. | Partial | `JOURNEY-STOP-RESTART` verifies local queue reopen. `JOURNEY-REMOTE-STOP-PLAY` verifies remote queue-retained receipt and Open queue after exact queue/native Stop checks. Stop→Play did not resume the same item, and broader surface/device parity remains unverified. |
