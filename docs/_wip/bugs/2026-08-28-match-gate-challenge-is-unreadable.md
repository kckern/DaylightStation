...
| # | Issue | What shipped |
|---|---|---|
| **C1** | **"Pass challenge" over a bare exercise title** | **Resolved.** Every host now says why the screen exists: a child who presses "Pass challenge" on a program step reads **"Pass this to finish Exercise 1"**, and one stopped by a video lesson's checkpoint reads **"Pass this to finish Lesson 3"** — where both used to read the eyebrow "Pass challenge" over the exercise bank's own title. `ExerciseRunRoute` mounts `AskSession` (SP1 task 5), which writes the program line from the step it already fetched for the requirement; a checkpoint, which has no step to fetch, carries its lesson's title in the query. Ordinary practice is unchanged by design and keeps the bank title as its headline. Pinned end to end by `Exercises.route.test.jsx`, from the program page's own Pass button and the dashboard's own Continue button. |
