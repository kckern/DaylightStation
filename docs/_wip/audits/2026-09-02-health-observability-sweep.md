...
| Log a meal/template into today | `today/TemplatePicker.jsx` | `template.logged` / `template.log_failed`, `proposal.decided` / `proposal.decide_failed` | info/error | Yes | OK — `today/SavedMealsSheet.jsx` was retired in Phase 10 (PRD F6.3); the picker replaced it and kept the logging |
