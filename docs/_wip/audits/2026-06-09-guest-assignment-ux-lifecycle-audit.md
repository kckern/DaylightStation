...
- Placeholder avatar system: today everything falls back to one image (untagged placeholder `user.jpg`); evaluate tiered placeholders (e.g. untagged placeholder for untagged devices, a second mascot — "alternate guest placeholder" — for tagged-but-anonymous Guests, distinct adult vs kid marks)
...
| **§5 Late-tag untagged placeholder merge** | ✅ Shipped | `sessionBackfill.js:181-198` Rule 1 (`late-identity-claim`, duration-independent); test `PersistenceManager.lateTagMerge.test.js` |
...
| G11 Stray untagged placeholder counts toward governance | ❌ Open | Untagged devices still gate `active: all` immediately |
...
| G18 Named untagged placeholder (photo missing) | ❌ Open | Same fallback as untagged |
...
- ❌ **You cannot tag a second anonymous Guest** while one generic Guest exists (N2). The host taps the second untagged placeholder, scans the picker, and "Guest" simply isn't there — no explanation. This is the single most confusing dead-end in the flow, because the mental model ("everyone can be a Guest") is correct and the UI contradicts it.
...
- ⚠️ **Generic Guests are visually interchangeable.** Both cards say "Guest" with the same untagged placeholder face. With two anonymous guests, neither can tell which zone badge is theirs. (Identity is correctly distinct under the hood — the UI just doesn't show it.)
...
- ⚠️ **Stray straps still take governance hostage** (G11). A forgotten broadcasting strap on a shelf is a untagged placeholder that gates `active: all`. The fix is tribal knowledge ("Remove User it").
...
|---|---|---|
| **Adult** | ✅ Best case: real name + photo after one tap (or zero taps via BLE/mapped strap). Zone overrides possible via config. | ⚠️ untagged placeholder card → tap → "Guest" → correct data, anonymous identity, untagged placeholder face forever. Indistinguishable from a second unregistered adult. |
| **Kid** | ⚠️ Name + photo work; zones only correct if the config entry carries kid `zones:`/`birthyear` AND the kid wears *their own mapped strap*. On a borrowed adult strap: adult thresholds (wrong). | ❌ Worst case: untagged placeholder face, adult zone thresholds, no way to even *say* "this is a child." Coins/governance silently mis-calibrated. |

Two structural observations:

1. **The registered/unregistered axis is an avatar problem.** Registered guests have faces; unregistered ones are all the same untagged placeholder. The system has three *semantically different* anonymous states (untagged device / tagged generic Guest / registered-but-photo-missing) collapsed into one image.
...
Today: every fallback path lands on `user.jpg` (untagged placeholder). Proposal — make the placeholder *encode the identity class*, so a glance at a card tells you its state:

| Identity class | Today | Proposed | Why |
|---|---|---|---|
| Untagged device (nobody claimed it) | untagged placeholder | **untagged placeholder** (keep — it's established household lore for "who is this?") | untagged placeholder = "needs tagging" stays a single unambiguous signal |
| Generic Guest, adult | untagged placeholder | **alternate guest placeholder** (second mascot) or 🧑 silhouette tile | "Claimed but anonymous" must look different from "unclaimed" — right now tagging a Guest produces *no visible change* except the label, which reads as the tap not working |
| Generic Guest, kid | untagged placeholder | **Distinct kid mark** (e.g. child placeholder / 🧒 tile) | Visual confirmation that the kid zone profile is active (see below) |
| Registered user, photo missing | untagged placeholder (with their real name — the "named untagged placeholder" bug-lookalike) | **Initials-on-color avatar** (G18) | A real person with a config entry should never share a face with strangers |
...
**N5. Tagging a generic Guest produces almost no visible change.** *(fixed 2026-06-09, `feature/guest-ux-fixes` — generic Guests resolve to guest-adult / guest-kid placeholder avatars (untagged placeholder fallback until assets exist).)* Card label changes `#10366` → "Guest"; the face stays untagged placeholder. Hosts double-tap because the first tap "didn't seem to work." A distinct claimed-anonymous placeholder (alternate guest placeholder tier) makes success visible.
...
**N9. untagged placeholder menu header is unexplained.** *(fixed 2026-06-09, `feature/guest-ux-fixes` — explainer line in the picker when the device has no base user.)* Header shows `#99999` raw. Add a sentence: "Unrecognized heart-rate strap. Pick who's wearing it — or Guest if they're visiting." (G7 carry-over, one string fix.)
...
**N11. Registered-but-photo-missing = named untagged placeholder** (G18 carry-over). With a tiered placeholder system this becomes an initials avatar for free.
...
7. **Part 4 placeholder tiers** — untagged placeholder (untagged) / alternate guest placeholder (anonymous adult Guest) / kid mark (anonymous kid Guest) / initials (registered, photo missing). Resolves N5, N11/G18, and half of N3 in one coordinated pass. *(◐ partially fixed 2026-06-09, `feature/guest-ux-fixes` — N5 shipped via `guest-adult` / `guest-kid` placeholder image ids, untagged placeholder fallback until assets exist; initials avatar for N11/G18 NOT done)*
...
| **Distinct semantics get distinct visuals** | Missing — one untagged placeholder for three states (Part 4) |
...
- [`unknown-hr-monitors.md`](../../reference/fitness/unknown-hr-monitors.md) — untagged placeholder mechanics, ANT+/BLE admission
