...
2. ~~**`advanced: true` on first observation.**~~ **RETRACTED 2026-08-16 — the premise does not hold.** The claim was that after every remount the baseline is null, so the first tick reports progress and zeroes the retry cap. It isn't: the baseline is `lastSuccessPosRef`, a ref inside `useMediaResilience`, which is called from exactly one place (`Player.jsx:887`) **above** the `singlePlayerKey` boundary. A remount tears down the keyed child; the hook never re-runs and its refs survive. The ref is nulled at one site only (`useMediaResilience.js:112`), inside the `playbackSessionKey`-change effect — which in the same breath calls `releaseSession`, so the false progress always lands on a freshly-emptied ledger with nothing to zero.

    The audit flagged this one as inference (*"that this fired during the incident is inference"*) and the inference was wrong. **Two mechanisms defeat the retry cap, not three.** Both are closed by Task 2. The corresponding plan task was withdrawn before implementation — see Task 4b in the remount-storm plan for the full reasoning.
