...
| **Junk guardrail** | Skip any `.mid` whose estimated render duration exceeds `maxRenderSeconds` (default 20 min) — a stuck note / idle device recording can span 90+ min, rendering a multi-GB scratch WAV. Duration read cheaply from the SMF (pure parser, no render) before fluidsynth. Configurable via `pianoaudio.maxRenderSeconds`. |
| **Order** | Newest-first |
