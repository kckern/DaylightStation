...
function makeLog(grams) {
  return { id: 'log1', userId: 'kckern', status: 'pending',
    items: [{ label: 'Unknown', grams, calories: 0, unit: 'g' }],
    metadata: { source: 'scale', messageId: '900' }, // '900' = bot prompt message
    with(updates) { return { ...this, ...updates, with: this.with }; } };
}
...
describe('LogScaleFoodFromText', () => {
  ...
  beforeEach(() => {
    messaging = { updateMessage: jest.fn().mockResolvedValue({}), deleteMessage: jest.fn().mockResolvedValue({}), sendMessage: jest.fn().mockResolvedValue({ messageId: '901' }) };
  ...
  it('estimates blended density and multiplies by the exact grams', async () => {
    const res = await useCase.execute({
      userId: 'kckern', conversationId: 'telegram:b1_c2', logUuid: 'log1',
      text: 'leftover lasagna', messageId: '555', responseContext: messaging, // '555' = user's inbound text
    });
    ...
    // edits the BOT prompt ('900'), not the user's message; deletes the user's ('555')
    expect(messaging.updateMessage).toHaveBeenCalledWith('900', expect.objectContaining({ choices: expect.any(Array) }));
...
{"label": "<short name>", "density_kcal_per_g": <number>, "protein_per_g": <number>, "carbs_per_g": <number>, "fat_per_g": <number>}`,
  ...
  async execute(input) {
    ...
    const choices = buildConfirmButtons(this.#encodeCallback, logUuid);
    // The describe path runs on a TEXT message, so the inbound messageId is the USER's
    // message (a bot can't edit it). Edit the bot's prompt (metadata.messageId, persisted
    // by LogFoodFromScale and preserved through the container step) and delete the user's.
    const botMessageId = nutriLog.metadata?.messageId;
    if (messageId) { try { await messaging.deleteMessage(messageId); } catch { /* ignore */ } }
    if (botMessageId) {
