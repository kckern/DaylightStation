...
describe("scanCodes: 'qr'", () => {
  ...
  const darkPixelsInCodeArea = (canvas, theme) => {
    ...
    // sample ONLY the code box interior rows (not the token text below, which both modes draw)
    const img = ctx.getImageData(x, 0, size, canvas.height).data; // then refine: locate the box top by scanning for the border row and sample `size` rows from there
    let dark = 0;
...
- [ ] **Step 1: Failing e2e test** — through the harness: (a) `GET .../agenda/preview` for a learner with an assignment → 200, body starts with PNG magic (`89 50 4E 47`), `content-type: image/png`, `content-disposition` contains a slugified filename; (b) DRY-RUN PROOF: after the preview, the harness's session store shows ZERO new sessions for the learner, the token registry has ZERO records, AND no language progress file was written (assert on the harness's language datastore — the fake's `writeProgress` spy is uncalled; this pins `todayStatus` read-only-ness per spec §3); (c) a real tap afterwards still creates sessions/tokens normally; (d) unknown learner → 200 PNG (the notice renders); (e) renderer absent (harness constructs the lifecycle/router without receiptPng) → 501 `{ error: 'agenda preview not configured' }` (spec §5). If the harness serves routes via supertest/express, mirror the existing e2e's route invocation style; if it calls use cases directly, add the router with a minimal express app the way other router tests do (`grep -rln createSchoolLifecycleRouter tests/`).
...
- `SurfaceProgramLauncher`: `{ id, label, surface, action, subject }` from config; `launch({userId})` → `donow.dispatch({ surface, action, learnerId: userId, requestedBy: 'school-program', ref: id, programId: id })`; `status({userId})` → reads the TWO UTC shards spanning the current study day — `listDispatches` for the UTC date of `now` AND of `now − 24h` (the log shards by UTC date, but a 5:01pm PDT dispatch is 00:01Z tomorrow and lands in tomorrow's shard; reading one shard un-serves PE at 5pm) — concatenates, then filters `programId === id && learnerId === userId && isSameStudyDay(Date.parse(row.at), nowMs, { timezone, boundaryHour: 4 })` (from `#domains/school/studyDay.mjs`) → `{ doneToday, progressLabel: null, score: null }` — and spec §9 row 8: a same-surface same-learner same-day row WITHOUT programId is ignored. TEST ROW (required): a dispatch at `2026-07-30T00:01:00Z` (5:01pm PDT July 29) with now `2026-07-30T03:00:00Z` (8pm PDT July 29), timezone America/Los_Angeles → doneToday TRUE.
- **`LanguageProgramLauncher` routes through DoNow (spec §6 last bullet — occupancy applies uniformly):** its `portal` dep is replaced by the DoNow service; `launch({userId})` → `donow.dispatch({ surface: 'portal', action: { target: { kind: 'program', program: 'language' } }, learnerId: userId, requestedBy: 'school-program', ref: 'language', programId: 'language' })` and RETURNS the DoNow result. `status()` unchanged (ladder-owned evidence; the log row's programId is audit, not evidence, for language). Update `programLaunchers.test.mjs`: the launch test asserts the DoNow call shape, and a new test pins the occupancy path — an occupied-by-other portal yields `pending_approval`, never a broadcast (the mid-quiz sibling clobber the busy policy exists to prevent).
- **Update `IProgramLauncher.mjs`'s doc-comment in the same commit:** `launch()` now returns the DoNow `{ decision, ... }` result, not `{ dispatched: boolean }` — a documentation-only port must not become documentation-only fiction.
- **Program-branch slip wording follows the DoNow decision** in `ResolveScanAction`: launchers now return `{ decision, ... }` — dispatched → the existing "Starting on the Portal…" slip; pending_approval → "…is busy — we asked a grown-up."; denied/failed → "Go to the Portal and open it there." (Existing two-wording tests update to three.)
- **The `donow` dep on `ResolveScanAction`/school routing is OPTIONAL-DEGRADING** (the composition supplies it; fakes may not): absent `donow`, the launch arm and program dispatches slip "Ask a grown-up to set this up." — never a crash, never a phantom dispatch. State this in the constructor JSDoc; one test pins it.
...
**Wiring facts:** `WakeAndLoadService`/`SendHubCommand`/`CallHomeAssistantService`/`eventBus`/`thermalPrinterRegistry` all already exist in app.mjs — grep each for the construction site and inject; config knobs under a new `donow:` block in system/household config read via configService (`approvalsToken`, `notifyService`, `pianoKioskDeviceParam`, `livingroomDeviceId` default `livingroom-tv`); school.yml `programs:` list → SurfaceProgramLauncher instances merged into the launchers Map with the collision-is-boot-error check. **Piano registration is conditional on Task 9's outcome:** if the piano hook shipped its no-op fallback branch (content-open plumbing unreachable), `piano-kiosk` is NOT default-registered — config opt-in only (`donow.surfaces.pianoKiosk: true`) — because a surface whose dispatch provably does nothing while the log records `dispatched` and honor-close marks music done stretches fire-and-forget past honesty. Read Task 9's report before wiring. Fail-closed: no eventBus/no seams → the affected adapters degrade (occupancy unknown / dispatch failed), service still constructs; donow router mounts only when the service exists.
