...
`LanguageStudyService.todayStatus({ userId })`: derive today's queue the way the service's existing day methods do (find the method that computes `summary: summarizeQueue(queue)` around line 218 and reuse its internal derivation — do NOT duplicate the queue math; extract a private helper if needed). Return `{ doneToday, progressLabel, score: null }` where **doneToday = zero outstanding entries — vacuously TRUE on an empty queue** (adjudicated: mirrors `shouldRollDay`, rollover.mjs:45-46 — "an empty queue counts as complete: a learner who has retired every available sentence should still advance rather than stall on a vacuous condition"; a fresh learner can never present an empty queue, so empty means mastered, and a `total > 0 &&` guard would nudge a graduated learner forever). progressLabel: `'Day {dayIndex}'`, or `'Course complete'` when the queue is vacuously complete (total === 0). If the user has no corpus/progress at all → `{ doneToday: false, progressLabel: null, score: null }`.
