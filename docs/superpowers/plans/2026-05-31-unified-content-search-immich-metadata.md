...
- **Immich search with `withExif:true, withPeople:true` returns full exif + people/faces in ONE call.** Sample exif keys: `make, model, lensModel, fNumber, focalLength, iso, exposureTime, latitude, longitude, city, state, country, dateTimeOriginal, orientation, rating, ...`. People came back as `[{name:'Milo', faces:1}]`.
