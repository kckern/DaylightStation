...
- **W7 tutor-link** — PARTIALLY closed, not fully: Task 16 ships "Review this lesson" on a failed catalog quiz, but it reopens the Catalog root, not the failed lesson (no deep-link support exists), and it is a link back to the lesson, not the synchronous adaptive-tutor offer the original Wave 7 record described (that needs a server-side remediation-offer id a live quiz close still has no way to mint). Recorded here as upgraded-but-still-open, not closed.
...
**Still open by choice, re-recorded here (not debt — design choices the role advocates themselves accepted):**

- **Tap-to-confirm on multiple-choice items** — deferred since Wave 7 ("it slows every answer to insure rare mis-taps"); the marked-answer fix shipped instead. Unchanged this wave.
- **Paper's "one more?" affordance past the daily serve cap** — deferred since Wave 7 by design: the day cap is the pedagogy, and on-screen materials being uncapped is the intended pressure valve. Unchanged this wave.
- **A third, no-stakes "show me again (doesn't count)" flashcard lane** — deferred since Wave 7 to keep that wave shippable. Unchanged this wave.
- **The certificate's portrait orientation** — recorded as a deviation at Wave 4 ("the certificate ships portrait, not 'landscape-ish'"). Unchanged this wave.
- **The real tutor deep-link off a failed quiz** — see "W7 tutor-link" above: Task 16 shipped the honest fallback (Catalog root) it could ship without new backend surface; the synchronous adaptive-tutor handoff itself remains a backend wiring gap, open for a future wave.

If any of these five are wanted, each is a scoped, one-wave-away addition — none require re-opening this wave's closures.
