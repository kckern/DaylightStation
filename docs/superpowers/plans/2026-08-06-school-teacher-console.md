...
- **Deviations recorded at M4 (historical; certificate orientation superseded by the 2026-08-24 software-only follow-up below):** `/certificate` always renders PDF (the `format` param is accepted but not required); the certificate originally shipped portrait, not "landscape-ish"; the roadmap row's "tutor insights polish" was already delivered by the wave-1 TutorInsights panel — no further work was needed or done.
...
**Wave-9 deviations recorded:** the quiz-sheet 'two answer surfaces' finding was an artifact of a PROOF render — card-backed renders already suppress on-page bubbles (the real printed path is single-surface; the render warning 'rendered without card allocation' already flags proofs). The subject-page 'vacant band' is `justify-content: safe center` seating a sparse subject mid-fold by design; with Practice shelves now present the fold fills, so the centering stays. The claim→guest→who's-here ceremony triangle (flow sin 3) is an identity-architecture change, deferred with this record. 'My Progress' naming with household scope: kidMode already scopes the claimed view; the unclaimed household board keeps its everyone view.

**M9 verdicts applied:** (1) ProgramCard labelizes report.label and unknown state chips; metric units singularize at 1 ('1 day'). (2) the shimmer is REAL now — PosterImg tracks img onLoad and holds `school-poster--loading` (the orphaned class the reviewer caught) until Plex art actually arrives, on both video and audio tiles. (3) the claimed rail renders the Last-active row only when the humane date exists — no more dangling label. (4) the Catalog's EmptyState spans the grid (`grid-column: 1 / -1`, one optical center product-wide) and the Print empty copy stops naming school.yml at a child. Deviation verdicts accepted as recorded (proof-render bubbles withdrawn by the reviewer; centered sparse band kept; identity-ceremony triangle stays on the ledger).

---

# Wave 10 — Production hardening (plan: `2026-08-06-school-production-hardening.md`)

**Contract:** a separate production-readiness assessment (not authored in this file) found two release **blockers**, a nine-item operator **punch list**, and named findings (F1/F2/F3 — mutating GETs and an ungated human sign-off lane; F5 — client-trusted media completion) against the accepted-debt items this ledger had been carrying since Waves 5–9. The wave's own plan is `docs/superpowers/plans/2026-08-06-school-production-hardening.md`; task-by-task detail and review remediation live under `.superpowers/sdd/2026-08-06-school-production-hardening/`. Same cycle: TDD per task, review + fix rounds, school-scoped sweep, deploy gate, live verify, Fable M10 review.

**Shipped (Wave 10, commits `db0330ee3`..`61457b602`):** attempt-shard and assignment-store corrupt-refusal + atomic writes (Blockers 1; Tasks 1–2); chapter banks roll up to gate their parent unit, ALL must pass, "Quiz N of M" (Blocker 2; Task 6); PIN on the human grade/close sign-off lanes, additive over the existing grown-up check (F3; Task 3); `GET /learners/:id/agenda` de-mutated to the existing preview twin, `POST` added for the real mint (F1; Task 4); `GET /print/*id` refuses card-minting query params, `POST /print/render` takes them (and the PIN) in the body instead (F2; Task 5); Print Center capability restoration on the data-volume surface profile (punch 4; Task 7); bounded-parallel materials summary fan-out + 1h units cache (punch 1; Task 8); `school:smoke` 24/24 and `receipts.test.mjs` repinned to the shipped product (punch 3; Task 9); a 410 mid-run now shows a sign, never a silent bounce (punch 9a; Task 10); STALE_SAVE is 409 everywhere (including a live periods-500 bug found and fixed en route), real `traceId`s, and a stale TTL comment corrected (punch 9b–d; Task 11); a reassignment audit trail (`apps/school/reassignments.yml`) merged into `GET /audit` (Task 12); a quota-approval preview route + PrintCenter `Preview` link (Task 13); the teacher-backlog nudge reads a fresh (not boot-cached) teacher list (Task 14); the student panel shows today's plan (Task 15); a failed catalog quiz offers "Review this lesson" plus the `passingPercent` threading fix that made a catalog quiz able to fail at all (Task 16); server-side sittings — mid-quiz resumability across restarts, with an ordered-prefix resume guard, a screen-transport gate, and `fresh` semantics split correctly between "Try again" and "Start again" (Task 17); one identity ceremony — dismissal cancels, guest is an explicit button (Task 18); the teacher console's first live Playwright smoke, structurally gated so a wrong-PIN mutation test cannot silently start writing real curriculum data (Task 19).

**Closed by this wave (superseding the lines below — do not re-open without a new decision):**

- **M5 audit** — Wave 5's recorded deviation ("the promised `apps/school/reassignments.yml` audit log was NOT shipped") is CLOSED by Task 12: the log now exists, append-only, corrupt-refusing, merged into `/audit`.
- **M5 e2e/browser-wiring gap** — Wave 5's other recorded deviation ("the browser↔prod-wiring seam remains the one honestly-open gap") is CLOSED by Task 19: the console is now driven headless against live wiring, not mocked panels.
- **M6 preview** — Wave 6's recorded deviation ("the advocate's #19 print-document PEEK was not delivered") is CLOSED by Task 13: `GET /print/printables/:id/preview` + the PrintCenter `Preview` link.
- **M6 nudge** — Wave 6's recorded deviation ("the nudge teacher list reads the boot-cached config") is CLOSED by Task 14: an hourly fresh reload, falling back to the cached list on failure.
- **W7 resumability** — the Wave 7 deviation record's headline item ("full mid-quiz resumability … is real architecture, deferred") is CLOSED by Task 17.
- **W7 day-plan** — the Wave 7 deviation record's "the day-plan-on-Portal item was NOT built this wave" is CLOSED by Task 15 (the student panel's Today block).
- **W7 tutor-link** — CLOSED on 2026-08-24: a completed failed Catalog quiz now submits only its session/learner identity to `POST /sessions/:sessionId/remediation-offer`; the server verifies ownership and durable completion, re-resolves the exact lesson/module and bank revision, mints/reuses the adaptive-remediation session, and the School shell opens that session. "Review this lesson" remains the fallback when remediation was not authored.
- **W9 identity triangle** — Wave 9's recorded deviation ("the claim→guest→who's-here ceremony triangle … is an identity-architecture change, deferred with this record") is CLOSED by Task 18: dismissal is now a cancel everywhere, guest is reached through exactly one explicit button.
- **F1/F2/F3 (assessment findings, not previously ledger lines in this file)** — F1 (agenda GET minted real sessions) and F2 (print GET minted card allocations via query string, PIN included) are CLOSED by Tasks 4 and 5; F3 (grade/close human lanes carried no PIN) is CLOSED by Task 3.
- **F5 (client-trusted media completion)** — not built out as its own fix; BACKSTOPPED by Task 6's live quiz-gate roll-up, which makes chapter-structured course completion depend on a server-verified passed quiz per chapter rather than a client-reported watch signal.

**Software-only follow-up closed (2026-08-24):** all five choices re-recorded
above are now implemented: two-tap multiple-choice confirmation; an optional
paper **One more?** continuation beyond today's cap; a no-stakes flashcard
rotate-to-back lane; landscape Letter certificates; and the exact-attempt,
exact-lesson adaptive-tutor handoff described under W7 tutor-link. Focused
frontend/application/API/render/lifecycle coverage, the architecture gate,
frontend production build, and a local Docker image build cover the automated
software seams. Deployment, live installation verification, physical-printer
execution, learner/manual trials, and the manual stranded-session decision are
outside this software-only follow-up.

**M10 review:** pending — deploy, live verification, and the Fable production-readiness re-assessment are Task 20 Step 4, owned by the wave's controller, not this documentation task.
