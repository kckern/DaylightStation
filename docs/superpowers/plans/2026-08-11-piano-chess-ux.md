...
- Search timeout is `movetime_ms + 1500`.
- The engine must never block play: on any engine failure or timeout, fall back to `chooseMove` from `shared/gaming/chess/opponent.mjs` and report `engine: 'fallback'`.
...
In `docker/Dockerfile`, after the `npm ci`/`npm install` step that populates `node_modules`, add a layer that keeps only the lite-single build and fails loudly if it is missing:

```dockerfile
# Stockfish ships every build variant (251MB unpacked). Chess only needs the
# single-threaded lite build (~7MB); a missing file must fail the build rather
# than ship an image whose chess silently runs on the fallback engine.
RUN set -eux; \
...
import { createStockfishEngine } from './StockfishEngineAdapter.mjs';
...
let engine = null;
let pending = null; // { id, resolveOn: 'bestmove' }
...
parentPort.on('message', (msg) => {
  if (msg.type !== 'search' || !engine) return;
  pending = { id: msg.id };
...
export function createStockfishEngine({
  ...
  return {
    ...
    async chooseMove({ fen, rung, gameId }) {
      ...
      const fallback = homegrownChooseMove(fen, { difficulty: 'steady', seed: fen.length });
...
export function createChessConfigService({
  ...
  return {
    ...
    async writeUserLayer(userId, patch) {
      ...
      await writeUserConfig(userId, patch);
...
```javascript
import express from 'express';
import { validateFen } from '../../../../../shared/gaming/chess/engine.mjs';
...
export function createChessRouter({ engine, configService, logger = null }) {
  ...
  router.post('/move', asyncHandler(async (req, res) => {
    ...
    if (typeof fen !== 'string' || !validateFen(fen)?.ok) {
      return res.status(400).json({ error: 'invalid_fen' });
    }
...
`validateFen` returns chess.js's `{ ok, error }` shape — `shared/gaming/chess/engine.mjs:53` and `:76` already guard with `validateFen(fen).ok`, so `.ok` is correct here.
...
useEffect(() => {
  ...
  const timer = setTimeout(async () => {
    ...
    const served = await requestOpponentMove({
      fen, rung: rungId, gameId, userId: currentUser?.user_id ?? null,
    });
    // The server is the strong opponent; the bundled engine is what keeps the
    // game playable when it cannot be reached.
    const reply = served
      || chooseMove(fen, { difficulty: 'steady', seed: gameRef.current.history.length });
...
`rungId`, `gameId` and `opponentDelayMs` come from the config loaded in Task 7; until then, default them to `'learner'`, a stable per-mount id, and `700`.
...
```bash
git add frontend/src/modules/Piano/PianoChessGame/chessApi.js frontend/src/modules/Piano/PianoChessGame/chessApi.test.js frontend/src/modules/Piano/PianoChessGame/PianoChessGame.jsx
...
**Interfaces:**
- Consumes: `identifyChord(notes)` and `squareToChord(square, scheme)` from `./chordAddress.js`; the component's existing `cursorChord` and `heldNotes`.
- Produces: `<ChordReadout heldNotes={number[]} chord={{symbol}|null} square={string|null} connected={boolean} />`.
...
describe('ChordReadout', () => {
  ...
  it('says the held set is not a square when it names no square', () => {
    render(<ChordReadout heldNotes={[60, 61, 62]} chord={null} square={null} connected />);
 ...
 */
export default function ChordReadout({ heldNotes = [], chord = null, square = null, connected = true }) {
...
```jsx
{ghost?.square === square && !piece && (
  <img className="chess-board__piece chess-board__piece--ghost" src={pieceSrc(ghost.piece)} alt="" aria-hidden="true" />
)}
```

Also ensure each square button carries `data-square={square}` if it does not already, since the test locates the ghost by it.
...
```javascript
// Only while a piece is held and the cursor names a square the piece can reach.
const ghost = game.origin && cursor && cursor !== game.origin
  ? { square: cursor, piece: pieceAt(game.game.fen, game.origin) }
  : null;
```

`shared/gaming/chess/pieces.mjs` exports `toPieceCode`, `parsePieceCode` and
`resolvePieceAsset`, but **no** square lookup — and `position.mjs` already exports
`fenToPosition`, which projects a FEN into squares. Use that rather than inlining FEN
parsing:

```javascript
import { fenToPosition } from '@shared-gaming/chess/position.mjs';
// ...
const origin = game.origin ? fenToPosition(game.game.fen)[game.origin] : null;
const ghost = origin && cursor && cursor !== game.origin
  ? { square: cursor, piece: origin }
  : null;
```

Check `fenToPosition`'s value shape before wiring — if it yields `{ color, type }` rather
than a `'wP'` code, convert with `toPieceCode` so `ghost.piece` matches what the board's
existing piece renderer accepts.
...
```bash
git add frontend/src/modules/Chess/ frontend/src/modules/Piano/PianoChessGame/ shared/gaming/chess/
...
Derive `opponentDelayMs` from `chessConfig?.opponent_delay_ms ?? 700` and the hint gating from `chessConfig?.feedback?.hint_level`, mapping: `off` → never show sources/targets; `after-mistake` → the current `showLegality` behaviour; `always` → always show.

Add panel styles to `PianoChessGame.scss` following the existing rail conventions (discrete buttons, brass active state).
