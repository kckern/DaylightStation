...
- [ ] **Step 1:** Create stroke-consistent SVG icons for: video, music, score, game, lessons, studio (menu); home, plug/connection (chrome); play, pause, prev, next, shuffle, repeat, volume-down, volume-up, queue, back, close (transport); record, stop, trash (studio). Use `currentColor` and a shared `1.75` stroke.
