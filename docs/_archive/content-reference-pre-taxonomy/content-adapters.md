...
| `backend/src/1_adapters/content/filesystem/FilesystemDriver.mjs` | Generic filesystem driver (singalong/readalong modes) |
| `backend/src/1_adapters/content/filesystem/resolvers/scripture.mjs` | Scripture resolver (standalone plugin) |
| `backend/src/2_domains/content/services/ContentSourceRegistry.mjs` | Adapter registry and ID resolution |
| `backend/src/3_applications/content/ContentIdResolver.mjs` | 5-layer ID resolution chain (see [content-id-resolver.md](./content-id-resolver.md)) |

> **Deprecated adapters (dead code, pending deletion):**
> - `backend/src/1_adapters/content/singing/SingingAdapter.mjs` — legacy alias for SingalongAdapter, no longer imported
> - `backend/src/1_adapters/content/narrated/NarratedAdapter.mjs` — legacy alias for ReadalongAdapter, no longer imported
> - `backend/src/1_adapters/content/narrated/resolvers/scripture.mjs` — superseded by `filesystem/resolvers/scripture.mjs`
