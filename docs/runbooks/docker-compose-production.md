...
Each deployed image contains a `/build.txt` file with build information:

```bash
# Check build info
ssh homeserver.local 'docker exec daylight-station cat /build.txt'
```

Example output:
```
Build Time: 2026-01-14 15:30:45 PST
Commit: abc1234
```

This is generated by `deploy.sh` during the build process:
- **Build Time**: Timestamp in Pacific Standard Time when the image was built
- **Commit**: Short git commit hash from the source repository

**Related code:** `deploy.sh`, `docker/Dockerfile`
