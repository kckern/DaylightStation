...
| `come-follow-me-ot-2026` curriculum work is broken for at least one learner | `school.curriculum.invalid-works` (`progression.mode must be sequential\|module_blocks`, 2026-08-24) and `school.agenda.plan-errors` ("assigned but no published units belong to it", recurring 08-25/26 for `learnerId: felix`) | These are almost certainly one root cause in the work's own definition. Note: a *related but distinct* draft-review-state version of this same course was fixed 2026-08-25 (see the incident below) — verify whether felix's remaining error is the same `progression.mode` schema defect or a fresh recurrence before re-diagnosing from scratch |
