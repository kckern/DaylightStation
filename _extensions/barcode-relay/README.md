...
An **M5Stack ATOM Lite (ESP32)** that reads a **Zebra DS2278** barcode scanner over
**BLE HID** and relays each scan over WiFi/WebSocket to the DaylightStation event bus
(topic **`barcode-relay`**). No host computer in the path.

```
DS2278 (HID-BLE keyboard) --BLE HOGP--> ESP32 ATOM Lite --WiFi WS--> backend event bus
                                        (BLE central + WS client)     topic: barcode-relay
```

Scan payload: `{source:'barcode-relay', type:'scan', device:'ds2278', code, ts}` →
handled by `backend/src/3_applications/hardware/barcodeRelay.mjs`.

For the full build story, protocol gotchas, and one-time scanner setup, see
...
household SSOT (`data/household/config/barcode-relay.yml`). Generate the gitignored
`firmware/include/config.h` from it first, then build/upload:
...
`config.example.h` documents the shape. The backend host is a stable **hostname**
