...
// Off unless explicitly enabled AND given a password. The board's diagnostics
// server on :80 is unauthenticated and read-only by design; an open flash
// endpoint beside it would be a much bigger door, and "I forgot the password
// line" must not be the thing that opens it. Hard-error rather than silently
// downgrading to an unprotected OTA.
const ota = sc.ota || {};
const otaEnabled = ota.enabled === true;
const otaPassword = String(ota.password ?? '');
if (otaEnabled && !otaPassword) {
  console.error(`ERROR: ${src} (reader=${wantId}) sets ota.enabled true with no ota.password.`);
...
// Over-the-air updates (ArduinoOTA, espota on :3232). Requires the dual-slot
// partition table in platformio.ini — min_spiffs.csv, NOT huge_app.csv, which
// has one app slot and cannot OTA at all.
#define OTA_ENABLED           ${otaEnabled ? 1 : 0}
#define OTA_PASSWORD          "${esc(otaPassword)}"
`;

writeFileSync(OUT, h, { mode: 0o600 });
console.log(`[gen-config] ${src} (reader=${wantId})\n[gen-config] -> ${OUT}  backend=${backend.host}:${backend.port}${backend.ws_path || '/ws'}  ${serial.baud ?? 9600}/${framing} sniff=${sc.sniff_mode === false ? 0 : 1}  nfc=${nfcEnabled ? `on(${nfcSda}/${nfcScl})` : 'off'}  buzzer=${buzzerEnabled ? `on(G${buzzerPin},${parseInt(buzzer.ms ?? 4, 10)}ms)` : 'off'}  ota=${otaEnabled ? 'on(espota:3232)' : 'off'}`);
