...
| SC-14.2 | Core shell ≤9 KB and physical execution window | done | done | done | Build gates pass; current Z80 code is 8,985 bytes (9,062-byte `.86p`), leaving 231 bytes in the product ceiling and 415 bytes in the 9,400-byte execution window |
...
| SC-14.5 | Scratch/free reserve 8–12 KB | done | done | done | Sync preserves 8,500 bytes; SCN1 is capped at 4,096 transient bytes and preflight leaves 4,372 after variable overhead; low-memory tests pass |
| SC-14.6 | Standard runtime and downloadable-content budget | done | done | done | Ten-program contract: 60,736-byte planning target, 83,832 independent component ceiling, and 73,786 reserve-safe aggregate maximum. The direct-matrix build is 73,556 bytes, 12,820 above target, leaves 230 aggregate bytes, and leaves 4,838 bytes for content after target state buckets (including the 512-byte-target SCCO continuation index); no more content can be admitted beside maximum state without re-planning state or executable budgets. Every per-program bound remains enforced. |
