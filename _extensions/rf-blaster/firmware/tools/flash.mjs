#!/usr/bin/env node
// =============================================================================
// flash.mjs — regenerate config.h from rf-blasters.yml, then build + upload.
//
// Usage:
//   node tools/flash.mjs <path-to>/config/rf-blasters.yml [blaster-id] [--port /dev/cu.xxx]
//   DAYLIGHT_RF_CONFIG=<path> node tools/flash.mjs [blaster-id] [--port ...]
//
...
const src = rest[0] || process.env.DAYLIGHT_RF_CONFIG;
const blasterId = rest[1] || '';

if (!src) { console.error('ERROR: pass rf-blasters.yml path or set DAYLIGHT_RF_CONFIG.'); process.exit(1); }
