#!/usr/bin/env node
/**
 * cli/school/cardLadder.mjs — `school card-ladder` — card-ladder operations,
 ...
 *   enroll-plan  write an `ops assign` plan file adding the card-ladder
...
const HELP = `school card-ladder — card-ladder operations (any word package)

Usage:
  school.mjs card-ladder quiz --deck <deckId|slug> [--seed N] [--force]
                              [--data-dir P] [--media-dir P] [--source-root P]
  school.mjs card-ladder quiz --learner <id> --package <pkg> [--week <YYYY-Www>]
                              [--rows N] [--seed N] [--force]
                              [--data-dir P] [--media-dir P] [--source-root P]
  school.mjs card-ladder enroll-plan --learner <id> --deck <deckId|slug> --out <file>
                              [--title TEXT] [--base-url URL] [--data-dir P] [--media-dir P]
  school.mjs card-ladder trace --learner <id> [--day YYYY-MM-DD | --sitting ID]
...
plan for 'school ops assign' with the card-ladder program appended (or
replacing that learner's existing card-ladder program). The tile title
defaults to the lexicon's program.title.
trace reads $DAYLIGHT_LOGSTORE (default ${DEFAULT_LOGSTORE}) for that
learner's school.card-ladder.* events and prints a timeline: one header line
...
// ── trace (spec §8 "school card-ladder trace") ─────────────────────────────
// Reads the log store for this learner's school.card-ladder.* events, turns
// them into a timeline via the pure `formatTrace` (backend/…/cardLadder/trace.mjs),
...
/** Every word package this learner has a card-ladder day file under (either folder name), sorted for determinism. */
