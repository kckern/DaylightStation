/**
 * Split a persisted v3 fitness session into two at a timestamp boundary.
 *
 ...
 *
 * Without `--write` it is a DRY RUN: prints the split tick, the two new ids,
 * and all reconciliation invariants, and writes nothing. Even with `--write`
 * it refuses to write if any invariant fails.
 *
 * @module cli/lib/fitness/split
 */
...
import {
  splitDecodedSeries,
  computeSplitTick,
  recomputeSummaryForPart,
  allocateBucketsRedistribute
} from '#domains/fitness/services/sessionSplit.mjs';
...
export const spec = {
  name: 'split',
  summary: 'split one session into two at a timestamp, content, or wall-clock boundary',
  usage: 'fitness session split --file=<session.yml> <SELECTOR> [--write]',
  details: `  --file=PATH             Session YAML to split (required)

  SELECTOR (pick exactly one):
  --split-ts=EPOCH_MS     Split at an exact unix-ms timestamp
  --split-content=ID      Split at the first media event with this contentId
  ...
  --write                 Apply the split (default: dry run)
  ...
  lands in fitness/log/_backups/split/ — deliberately OUTSIDE any
 ...
 */
/**
 * Read a session's frame manifest from the media sidecar.
 *
...
function readSnapshotSidecar(ctx, sessionFile) {
  const mediaDir = ctx?.configService?.getMediaDir?.() ?? ctx?.mediaDir;
...
export async function run(argv, ctx) {
  const { flags } = parseArgs(argv, { valueFlags: ['file', 'split-ts', 'split-content', 'split-time'], booleanFlags: ['write'] });

  const FILE = str(flags, 'file');
  const WRITE = bool(flags, 'write');
  const splitTsArg = str(flags, 'split-ts');
  const splitContent = str(flags, 'split-content');
  const splitTime = str(flags, 'split-time');

  if (!FILE) {
    throw new CliError('Required: --file=<path> and one of --split-ts / --split-content / --split-time');
  }
  ...
  let SPLIT_TS = NaN;
  if (splitTsArg != null) {
    SPLIT_TS = Number(splitTsArg);
  } else if (splitContent) {
    const ev = (doc.timeline?.events || []).find(
      e => e?.type === 'media' && e?.data?.contentId === splitContent && Number.isFinite(Number(e.timestamp))
    );
    if (!ev) throw new CliError(`No media event found for contentId ${splitContent}`);
    SPLIT_TS = Number(ev.timestamp);
    console.log(`resolved --split-content ${splitContent} -> ts ${SPLIT_TS} ("${ev.data?.grandparentTitle || ''} / ${ev.data?.parentTitle || ev.data?.title || ''}")`);
  } else if (splitTime) {
    SPLIT_TS = moment.tz(`${doc.session.date} ${splitTime}`, 'YYYY-MM-DD HH:mm:ss', tz).valueOf();
    console.log(`resolved --split-time ${splitTime} on ${doc.session.date} -> ts ${SPLIT_TS}`);
  }
  if (!Number.isFinite(SPLIT_TS)) {
    throw new CliError('Could not resolve a split timestamp. Provide --split-ts / --split-content / --split-time.');
  }
  ...
  // halves with no frames at all. Inline still wins when present, matching
  // findById, so unmigrated sessions behave exactly as before.
  const caps = doc.snapshots?.captures || readSnapshotSidecar(ctx, FILE)?.captures || [];
  ...
  if (!allOk) throw new CliError('Refusing to write: invariants failed.');
  ...
  // `_backups` sibling does not match the date regex, so it is ignored.
  const sessionsRoot = path.dirname(path.dirname(FILE)); // fitness/log
  const backupDir = path.join(sessionsRoot, '_backups', 'split');
  ...
  return { ...result, written: true, backup, file1, file2 };
}
