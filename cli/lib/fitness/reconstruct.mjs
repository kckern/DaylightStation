 ...
 *
 * @module cli/lib/fitness/reconstruct
 */
...
export const spec = {
  name: 'reconstruct',
  summary: 'rebuild v3 session files from unmatched strava archives that have hr data',
  usage: 'fitness session reconstruct [--write] [--days=N]',
  details: `  --write      Persist session files (default: dry-run)
  --days=N     Look back N days (default: back to 2024-01-01); a bare
               numeric positional is accepted for back-compat`,
};
...
// ------------------------------------------------------------------
function findArchive(date, type, id, { loadYamlSafe, stravaArchiveDir, olderArchiveDir }) {
...
export async function run(argv, ctx) {
  // `--write` declared boolean so the documented `--write 90` form keeps 90 as
  // the day count instead of feeding it to the flag.
  const { positional, flags } = parseArgs(argv, { booleanFlags: ['write'] });
  const writeMode = bool(flags, 'write');

  // Default: back to Jan 2024. `--days` wins over the bare numeric positional
  // (the original script's `args.find(/^\d+$/)` rule).
  const defaultDays = Math.ceil(moment().diff(moment('2024-01-01'), 'days'));
  const numericArg = positional.find(a => /^\d+$/.test(a));
  const positionalDays = numericArg ? parseInt(numericArg, 10) : undefined;
  const daysBack = num(flags, 'days', positionalDays ?? defaultDays);

  // Backend bootstrap. Imported lazily so that merely importing this module
  // (which the dispatcher does to read `spec`) initializes nothing.
  ...
  for (const { date, entry } of entries) {
    ...
    const found = findArchive(date, entry.type, entry.id, archiveDeps);
  ...
  return { processed, matched, skipped, daysBack, write: writeMode };
}
