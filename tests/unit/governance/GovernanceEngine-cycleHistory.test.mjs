...
describe('GovernanceEngine cycle history and cooldown', () => {
  ...
  it('records cycle success history with full fields and applies cooldown', () => {
    ...
    nowRef.value = 101500;
    engine.evaluate({ activeParticipants: ['user_2'], userZoneMap: { user_2: 'active' },
    ...
    nowRef.value = 103000;
    engine.evaluate({ activeParticipants: ['user_2'], userZoneMap: { user_2: 'warm' },
      zoneRankMap: { cool:0, active:1, warm:2, hot:3, fire:4 },
    ...
    expect(entry.rider).toBe('user_2');
    expect(entry.ridersUsed).toEqual(['user_2']);
    ...
    // Cooldown applied
    expect(engine._cycleCooldowns.user_2).toBe(106000 + 300 * 1000);
  });
  ...
  it('abandonActiveChallenge() records abandoned status and applies cooldown', () => {
    ...
    expect(engine._cycleCooldowns.user_2).toBe(203000 + 300 * 1000);
  });
