/**
 * TV App - Watchlist Submenu Navigation Test
 *
 * Verifies watchlist items open submenus instead of routing to Player.
 */
...
test.describe('TV Watchlist Submenu Navigation', () => {

  test('API returns list action for watchlist items', async ({ request }) => {
    console.log('[TEST] Checking API response for FHE watchlist...');
    ...
    // play should be undefined or null for watchlist items
    expect(fhe.play).toBeFalsy();  // Watchlist items should have no play action
  ...
  test('Selecting FHE opens submenu (not Player)', async ({ browser }) => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 20; attempt++) {
        ...
        // If video player appeared, this is the bug - watchlist went to player
        if (videoCount > 0) {
          break;
        }
      }
      ...
      // Should NOT have video player (that would mean it incorrectly played the watchlist)
      if (videoCount > 0) {
        throw new Error('FHE watchlist incorrectly opened in player instead of submenu');
      }
