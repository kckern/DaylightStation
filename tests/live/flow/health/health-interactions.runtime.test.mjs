...
test('one voice control owns its spinner; capture keeps its original day during navigation', async ({ page }) => {
  ...
  await expect(page.locator('.health-meal__add-action[data-loading]')).toHaveCount(1);
  ...
  release();
  await expect(page.locator('.health-meal__add-action[data-loading]')).toHaveCount(0);
