...
test('one voice control owns its spinner; capture keeps its original day during navigation', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Capture meal', exact: true }).click();
  await page.getByRole('option', { name: 'Add to Breakfast', exact: true }).click();
  await page.getByRole('button', { name: 'Quick voice log to Breakfast', exact: true }).click();
  ...
  release();
  await expect(page.locator('.health-quickbar__btn[data-loading]')).toHaveCount(0);
