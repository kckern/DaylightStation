...
describe('FileTransport', () => {
  ...
  describe('rotation', () => {
    // NOTE: these tests pin CURRENT behavior, which has a known off-by-one:
    // the rotation loop renames the live file to `.2` directly (the `i === 1 ?
    // filename : ...` branch), so `.1` is never created and only maxFiles - 1
    // rotated generations are actually retained. See file.mjs:66-80.
    // The tests pre-seed the file on disk and wait for the stream to open,
    // because rotation uses existsSync and createWriteStream opens lazily.
    const streamOpenDelay = () => new Promise((resolve) => setTimeout(resolve, 50));

    test('rotates when maxSize is exceeded: live file is renamed aside, new file starts at size 0', async () => {
      const filename = path.join(tmpDir, 'app.log');
      ...
      await streamOpenDelay();

      transport.send(makeEvent({ event: 'trigger-rotate' }));

      // Current behavior: rotated content lands in `.2`, never `.1`
      expect(fs.existsSync(`${filename}.2`)).toBe(true);
      expect(fs.existsSync(`${filename}.1`)).toBe(false);
      expect(transport.getStatus().currentSize).toBe(0);

      await transport.flush();
      expect(fs.readFileSync(`${filename}.2`, 'utf-8')).toContain('x'.repeat(250));
    });

    test('oldest rotated generation is deleted at the maxFiles cap', async () => {
      const filename = path.join(tmpDir, 'app.log');
      fs.writeFileSync(filename, 'x'.repeat(250));
      fs.writeFileSync(`${filename}.2`, 'gen-old\n'); // at cap for maxFiles: 2
      ...
      const rotated = fs.readFileSync(`${filename}.2`, 'utf-8');
    ...
    test('maxFiles: 1 is the only configuration that produces a .1 file (post-loop rename)', async () => {
      const filename = path.join(tmpDir, 'app.log');
      fs.writeFileSync(filename, 'x'.repeat(250));
