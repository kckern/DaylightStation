// tests/unit/adapters/content/MediaAdapter.test.mjs
...
describe('MediaAdapter', () => {
  ...
  beforeEach(() => {
    adapter = new MediaAdapter({ mediaBasePath: fixturesPath });
  });

  test('has correct source and prefixes', () => {
    expect(adapter.source).toBe('files');
    expect(adapter.prefixes).toContainEqual({ prefix: 'files' });
  ...
  test('getItem returns item for existing file', async () => {
    ...
    expect(item.id).toBe('files:audio/test.mp3');
    expect(item.source).toBe('files');
  ...
  test('getList returns directory contents', async () => {
    ...
    // MediaAdapter.getList returns PlayableItem instances for files in flat
    // directories (no itemType field); container directories carry
    // itemType: 'container'.
    expect(list[0].mediaUrl).toBeDefined();
  });
  ...
  test('throws error when mediaBasePath is missing', () => {
    expect(() => new MediaAdapter({})).toThrow('MediaAdapter requires mediaBasePath');
    expect(() => new MediaAdapter({ mediaBasePath: '' })).toThrow('MediaAdapter requires mediaBasePath');
  });
  ...
  describe('getCoverArt', () => {
    test('should return cover art for valid image type and size', async () => {
      const adapter = new MediaAdapter({ mediaBasePath: fixturesPath });
  ...
  describe('image MIME types', () => {
    it('should detect SVG files', () => {
      const adapter = new MediaAdapter({ mediaBasePath: '/test' });
  ...
  // NOTE: household-scoped watch state suite removed — MediaAdapter no longer
  // owns the householdsBasePath/_loadWatchState code path. Watch state is now
  // sourced via the configService injection pathway.
});
