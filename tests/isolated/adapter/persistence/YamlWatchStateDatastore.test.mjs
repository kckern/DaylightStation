...
describe('YamlWatchStateDatastore', () => {
  ...
  test('set and get watch state', async () => {
    ...
    expect(retrieved.contentId).toBe('plex:12345');
  ...
  test('getAll returns all states for storage path', async () => {
    await store.set(new MediaProgress({ contentId: 'plex:1', playhead: 100, duration: 1000 }), 'plex');
    await store.set(new MediaProgress({ contentId: 'plex:2', playhead: 200, duration: 2000 }), 'plex');
