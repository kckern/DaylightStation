...
import {
  Assertion, PolicyGraph, PeriodRef, SubjectRef, decideEntitlement, evaluateRequirement, fourState,
...
function candidate(expression = { kind: 'claim', claimTypeId: 'school.done', publisherId: 'school', subject: '$subject', period: '$period', nodeId: 'claim' }) {
  return {
    ...
    entitlements: {
      'piano.games': { requirementId: 'school.required', failurePosture: 'fail_closed' },
    },
  };
}
...
describe('Requirements domain', () => {
  ...
  it('keeps publisher sources independent and treats missing evidence as indeterminate', () => {
    const policy = graph();
    const missing = evaluateRequirement({ graph: policy, assertions: [], requirementId: 'school.required', subject: learner, period, now, timezone });
    ...
    const evaluation = evaluateRequirement({ graph: policy, assertions: [assertion], requirementId: 'school.required', subject: learner, period, now, timezone });
    ...
    const stale = evaluateRequirement({ graph: policy, assertions: [assertion], requirementId: 'school.required', subject: learner, period, now: now + 86_400_000, timezone });
  ...
  it('does not let a different accepted publisher satisfy a qualified selector', () => {
    ...
    expect(evaluateRequirement({ graph: policy, assertions: [fitnessAssertion], requirementId: 'school.required', subject: learner, period, now, timezone }).state).toBe('indeterminate');
  });

  it('applies fail-open and fail-closed only to indeterminate evidence', () => {
    const value = candidate();
    value.entitlements['piano.preview'] = { requirementId: 'school.required', failurePosture: 'fail_open' };
    const policy = graph(value);
    const evaluation = evaluateRequirement({ graph: policy, assertions: [], requirementId: 'school.required', subject: learner, period, now, timezone });
  ...
  it('retraction restores uncertainty without deleting provenance', () => {
    ...
    expect(evaluateRequirement({ graph: policy, assertions: [assertion], requirementId: 'school.required', subject: learner, period, now: now + 1, timezone }).state).toBe('indeterminate');
  });
  ...
  it('uses count bounds so unresolved members matter only when they can change the result', () => {
    ...
    value.requirements['school.required'].subjectKinds = ['household'];
    ...
    expect(evaluateRequirement({ graph: policy, assertions, requirementId: 'school.required', subject: household, period, now, timezone }).state).toBe('satisfied');
  });
  ...
  it('projects explicit numeric progress without inventing aggregate weights', () => {
    ...
    value.requirements['school.required'].expression = { kind: 'all', children: [{ kind: 'schedule', days: ['sat'], start: '00:00', end: '24:00', nodeId: 'school.required/expression/0' }, comparison], nodeId: 'school.required/expression' };
    value.requirements['school.required'].progress = { basisNodeId: comparison.nodeId };
    const policy = graph(value);
  ...
  it('rejects reference cycles and exposes readonly policy maps', () => {
    const value = candidate({ kind: 'reference', requirementId: 'school.other', nodeId: 'a' });
    value.requirements['school.other'] = { schemaVersion: 1, subjectKinds: ['learner'], periodKinds: ['local_day'], expression: { kind: 'reference', requirementId: 'school.required', nodeId: 'b' } };
    expect(() => graph(value)).toThrowError(expect.objectContaining({ code: 'REQUIREMENT_CYCLE' }));
    const valid = graph();
    expect(() => valid.requirements.set('school.extra', {})).toThrow(/ReadonlyMap/);
  });
});
