...
const loaded = {};
for (const facet of facets) loaded[facet] = await import('@daylight/fitness/' + facet + '/value');
...
await assert.rejects(import('@daylight/fitness/server/value.mjs'),{code:'ERR_PACKAGE_PATH_NOT_EXPORTED'});
console.log(JSON.stringify({facets,versions,separateInstances:true,privateImportRejected:true}));
